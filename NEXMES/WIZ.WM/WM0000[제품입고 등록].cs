...
namespace WIZ.WM
{
    public partial class WM0000 : WIZ.Forms.BaseMDIChildForm
    {
        #region < MEMBER AREA > 
        ...
        #region < FORM LOAD >
        private void WM0000_Load(object sender, EventArgs e)
        {
            #region GRID SETTING
            ...
            _GridUtil.InitColumnUltraGrid(grid1, "MAKER", "작업자", false, GridColDataType_emu.VarChar, 90, 100, Infragistics.Win.HAlign.Center, true, false);
        ...
        public override void DoInquire()
        {
            ...
            try
            {
                ...
                rtnDtTemp = helper.FillTable("USP_WM0000_S1", CommandType.StoredProcedure
        ...
        private void btnBoxIn_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < grid1.Rows.Count; i++)
            {
                ...
                {
                    iCnt++;
                    _dLotQty += Convert.ToDouble(grid1.Rows[i].Cells["NOWQTY"].Value);
                }
            }
            ...
            try
            {
                for (int i = 0; i < grid1.Rows.Count; i++)
                {
                    ...
                    if (sChkFlag == "TRUE")
                    {
                        ...
                        float fQty = Convert.ToSingle(grid1.Rows[i].Cells["NOWQTY"].Value);

                        helper.ExecuteNoneQuery("USP_WM0000_I1", CommandType.StoredProcedure
                                                     , helper.CreateParameter("AS_PLANTCODE", LoginInfo.PlantCode, DbType.String, ParameterDirection.Input)
                                                     , helper.CreateParameter("AS_ITEMCODE", _sItemCode, DbType.String, ParameterDirection.Input)
                                                     , helper.CreateParameter("AS_LOTNO", sLotNo, DbType.String, ParameterDirection.Input)
                                                     , helper.CreateParameter("AF_NOWQTY", fQty, DbType.String, ParameterDirection.Input)
            ...
            try
            {
                ...
                helper.ExecuteNoneQuery("USP_WM0000_I2", CommandType.StoredProcedure
                                                      , helper.CreateParameter("AS_PLANTCODE", LoginInfo.PlantCode, DbType.String, ParameterDirection.Input)
                                                      , helper.CreateParameter("AS_ITEMCODE", _sItemCode, DbType.String, ParameterDirection.Input)
