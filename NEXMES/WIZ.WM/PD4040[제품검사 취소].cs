...
//   Edited Date  : 
//   Edit By      :
//   Description  : 자재재고 상태(검사품)의 LOT을 제품검사 대기 상태로 변경.
// *---------------------------------------------------------------------------------------------*
#endregion
...
namespace WIZ.WM
{
    public partial class PD4040 : WIZ.Forms.BaseMDIChildForm
    {
        #region < MEMBER AREA >
        ...
        private void PD4040_Load(object sender, EventArgs e)
        {
        ...
        private void ScanLogic()
        {
            try
            {
                ...
                _dt = USP_PD4040_S1(gBarCode, ref RS_CODE, ref RS_MSG);
        ...
        private DataTable USP_PD4040_S1(string sBarCode, ref string RS_CODE, ref string RS_MSG)
        {
            DBHelper helper = new DBHelper(false);

            try
            {
                rtnDtTemp = helper.FillTable("USP_PD4040_S1"
        ...
        private void USP_PD4040_I1(string sBarCode, ref string RS_CODE, ref string RS_MSG)
        {
            DBHelper helper = new DBHelper(false);

            try
            {
                helper.ExecuteNoneQuery("USP_PD4040_I1"
                                       , CommandType.StoredProcedure
                                       , helper.CreateParameter("AS_PLANTCODE", LoginInfo.PlantCode, DbType.String, ParameterDirection.Input)
                                       , helper.CreateParameter("AS_BARCODE", sBarCode, DbType.String, ParameterDirection.Input)
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            DialogResult result = MessageBox.Show("[" + gBarCode + "] 제품검사 취소 하시겠습니까?", "제품검사 취소", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1);
            ...
            try
            {
                USP_PD4040_I1(gBarCode, ref RS_CODE, ref RS_MSG);

                if (RS_CODE == "S")
                {
                    ControlClear();
                    ShowDialog(Common.getLangText("제품검사 취소 완료", "MSG"), DialogForm.DialogType.OK);
                }
                else if (RS_CODE == "E")
                {
                    ShowDialog(RS_MSG, DialogForm.DialogType.OK);
