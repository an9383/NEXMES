...
namespace WIZ.WM
{
    public partial class WM0030 : WIZ.Forms.BaseMDIChildForm
    {
        #region < MEMBER AREA > 
        ...
        #region < FORM LOAD >
        private void WM0030_Load(object sender, EventArgs e)
        {
            #region GRID SETTING
            //그리드 객체 생성
        ...
        public override void DoInquire()
        {
            ...
            try
            {
                ...
                string sEndDate = DBHelper.nvlDateTime(cbo_ENDDATE_H.Value).ToString("yyyy-MM-dd");     // 끝일자
                string sLotNo = DBHelper.nvlString(txt_LOTNO_H.Text.Trim());                          // LOTNO
                string sTotInspResult = DBHelper.nvlString(cbo_TOTINSPRESULT_H.Value);                        // 종합검사결과
