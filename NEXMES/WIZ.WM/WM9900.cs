...
namespace WIZ.WM
{
    public partial class WM9900 : WIZ.Forms.BaseMDIChildForm
    {
        #region [ 선언자 ]
            ...
            try
            {
                ...
                if (helper.RSCODE == "S")
                {
                    if (rtnDtTemp.Rows.Count > 0)
                    {
                        ...
                        {
                            if (Convert.ToString(grid1.Rows[i].Cells["DEPTCODE"].Value) != "WM")
                            {
        ...
        private void ComboBoxCloseYear()
        {
            ...
            try
            {
                ...
                DataTable dtComboBox = helper.FillTable("USP_MM9900_S2", CommandType.StoredProcedure
                                                               , helper.CreateParameter("@PLANTCODE", sPlantCode, DbType.String, ParameterDirection.Input)
                                                               , helper.CreateParameter("@DEPTCODE", "WM", DbType.String, ParameterDirection.Input));
