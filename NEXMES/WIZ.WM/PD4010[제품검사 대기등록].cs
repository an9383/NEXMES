...
namespace WIZ.WM
{
    public partial class PD4010 : WIZ.Forms.BaseMDIChildForm
    {
        #region < MEMBER AREA >
        ...
        private void PD4010_Load(object sender, EventArgs e)
        {
        ...
        private void ControlClear()
        {
            ...
            txtInspFlagNm.Text = string.Empty;
        ...
        private void ScanLogic()
        {
            try
            {
                ...
                string sInspFlagNm = string.Empty;

                DataTable _dt = new DataTable();
                _dt = USP_PD4010_S1(gBarCode, ref RS_CODE, ref RS_MSG);
                ...
                if (RS_CODE == "S")
                {
                    if (_dt.Rows.Count > 0)
                    {
                        ...
                        txtInspFlagNm.Text = sInspFlagNm;
                    }
                    else
                    {
                        ControlClear();
        ...
        private DataTable USP_PD4010_S1(string sBarCode, ref string RS_CODE, ref string RS_MSG)
        {
            DBHelper helper = new DBHelper(false);

            try
            {
                rtnDtTemp = helper.FillTable("USP_PD4010_S1"
        ...
        private void USP_PD4010_I1(string sBarCode, string sWhCode, string sLocCode, ref string RS_CODE, ref string RS_MSG)
        {
            DBHelper helper = new DBHelper(false);

            try
            {
                helper.ExecuteNoneQuery("USP_PD4010_I1"
                                       , CommandType.StoredProcedure
                                       , helper.CreateParameter("AS_PLANTCODE", LoginInfo.PlantCode, DbType.String, ParameterDirection.Input)
                                       , helper.CreateParameter("AS_BARCODE", sBarCode, DbType.String, ParameterDirection.Input)
        ...
        private void USP_PD4010_I2(string sBarCode, ref string RS_CODE, ref string RS_MSG)
        {
            DBHelper helper = new DBHelper(false);

            try
            {
                helper.ExecuteNoneQuery("USP_PD4010_I2"
                                       , CommandType.StoredProcedure
                                       , helper.CreateParameter("AS_PLANTCODE", LoginInfo.PlantCode, DbType.String, ParameterDirection.Input)
                                       , helper.CreateParameter("AS_BARCODE", sBarCode, DbType.String, ParameterDirection.Input)
                                       , helper.CreateParameter("AS_MAKER", LoginInfo.UserID, DbType.String, ParameterDirection.Input));
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            DialogResult result = MessageBox.Show("[" + gBarCode + "] 제품입고 등록을 하시겠습니까?", "제품입고 등록", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1);
            ...
            try
            {
                if (sInspFlag == "I")
                {
                    USP_PD4010_I2(gBarCode, ref RS_CODE, ref RS_MSG);

                    if (RS_CODE == "S")
                    {
                        ControlClear();
                        ShowDialog(Common.getLangText("제품검사 대기 정상 등록", "MSG"), DialogForm.DialogType.OK);
                    }
                    else if (RS_CODE == "E")
                    {
                        ShowDialog(RS_MSG, DialogForm.DialogType.OK);
                ...
                {
                    ShowDialog(Common.getLangText("창고와 위치를 설정하세요.", "MSG"), DialogForm.DialogType.OK);

                    //PD6040 PD6040 = new PD6040("WH002");
                    //PD6040.ShowDialog();

                    //if (PD6040.pWhCode == string.Empty || PD6040.pStorageLocCode == string.Empty)
                    //{
                    //    ShowDialog(Common.getLangText("창고와 위치를 선택하세요.", "MSG"), DialogForm.DialogType.OK);
                    //    return;
                    //}

                    //string sWhCode  = PD6040.pWhCode;
                    //string sLocCode = PD6040.pStorageLocCode;

                    ////무검사항목은 재고생성 및 이력생성
                    //USP_PD4010_I1(gBarCode, sWhCode, sLocCode, ref RS_CODE, ref RS_MSG);

                    //if (RS_CODE == "S")
                    //{
                    //    ControlClear();
                    //    ShowDialog(Common.getLangText("제품창고 정상입고", "MSG"), DialogForm.DialogType.OK);
                    //}
                    //else if (RS_CODE == "E")
                    //{
                    //    ShowDialog(RS_MSG, DialogForm.DialogType.OK);
