...
namespace WIZ.WM
{
    public partial class WM3100 : WIZ.Forms.BaseMDIChildForm
    {
        #region [ 선언자 ]
        //그리드 객체 생성
        ...
        #region [ Form Load ]
        private void WM3100_Load(object sender, EventArgs e)
        {
            #region [ Grid1 셋팅 ]
        ...
        public override void DoInquire()
        {
            ...
            try
            {
                ...
                if (rbtnTaking.Checked == true)
                {
                    ...
                    rtnDtTemp = helper.FillTable("USP_WM3100_S1", CommandType.StoredProcedure
        ...
        public override void DoSave()
        {
            ...
            {
                this.Focus();
                if (this.ShowDialog(Common.getLangText("제품 재고 실사 등록을 하시겠습니까?", "MSG")) == System.Windows.Forms.DialogResult.Cancel)
                {
                    CancelProcess = true;
                    DoInquire();
                    return;
                ...
                for (int i = 0; i < this.grid1.Rows.Count; i++)
                {
                    if (Convert.ToString(grid1.Rows[i].Cells["CHK"].Value).ToUpper() == "TRUE")
                    {
                        ...
                        {
                            this.ClosePrgFormNew();
                            this.ShowDialog(Common.getLangText("실사일자와 반영일자가 다릅니다.\r\n실사반영을 할 수 없습니다.", "MSG"), DialogForm.DialogType.OK);
                            DoInquire();
                            return;
                        }
                        ...
                        helper.ExecuteNoneQuery("USP_WM3100_I1"
