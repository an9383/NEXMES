...
namespace WIZ.PM
{
    public partial class PM0200 : WIZ.Forms.BaseMDIChildForm
    {
        #region <MEMBER AREA>
        ...
        private void PM0200_Load(object sender, EventArgs e)
        {
            #region Grid 셋팅
            //그리드 객체 생성
            ...
            _GridUtil.InitColumnUltraGrid(grid1, "MONSTOPNAME", "공수", false, GridColDataType_emu.VarChar, 100, 100, Infragistics.Win.HAlign.Left, true, false, null, null, null, null, null);
            _GridUtil.InitColumnUltraGrid(grid1, "MONSTOPEFFECT", "공수코드", false, GridColDataType_emu.VarChar, 80, 100, Infragistics.Win.HAlign.Center, false, false, null, null, null, null, null);
            ...
            string[] sMergeColumn2 = { "MINORCODE", "CODENAME", "MONSTOPNAME", "MONSTOPEFFECT", "LASTAVG", "THISAVG" };
            string[] sMergeColumn3 = { "CODENAME", "MONSTOPNAME" };
            string[] sMergeColumn4 = { "LASTAVG", "THISAVG" };
            string[] sHeadColumn = { "MINORCODE", "CODENAME", "MONSTOPNAME", "MONSTOPEFFECT", "LASTAVG", "THISAVG", "01", "02", "03", "04", "05", "06", "07", "08", "09", "10", "11", "12" };
        ...
        public override void DoInquire()
        {
            ...
            try
            {
                ...
                rtnDsTemp = helper.FillDataSet("USP_PM0200_S1", CommandType.StoredProcedure, param);
                ...
                {
                    hash.Add(DBHelper.gGetCode(drRow["MINORCODE"]) + "|" + DBHelper.gGetCode(drRow["MONSTOPEFFECT"]), drRow);
                }
                if (rtnDsTemp.Tables[0].Rows.Count > 0)
                {
                    ...
                    {
                        double result_Tm1 = Convert.ToDouble(DBHelper.nvlDouble(((DataRow)hash[steamcode + "|" + "MonthOutST"])[i].ToString()));
                        double result2_Tm1 = Convert.ToDouble(DBHelper.nvlDouble(((DataRow)hash[steamcode + "|" + "MonthOutIN"])[i].ToString()));
                        double result_Tm2 = Convert.ToDouble(DBHelper.nvlDouble(((DataRow)hash[steamcode2 + "|" + "MonthOutST"])[i].ToString()));
                        double result2_Tm2 = DBHelper.nvlDouble(((DataRow)hash[steamcode2 + "|" + "MonthOutIN"])[i]);
                        ((DataRow)hash[steamcode + "|" + "MonthOutSUM"])[i] = (result_Tm1 == 0 || result2_Tm1 == 0) ? 0 : DBHelper.nvlDouble((result_Tm1 / result2_Tm1) * 100);
                        ((DataRow)hash[steamcode2 + "|" + "MonthOutSUM"])[i] = (result_Tm2 == 0 || result2_Tm2 == 0) ? 0 : DBHelper.nvlDouble((result_Tm2 / result2_Tm2) * 100);
                        rtnDsTemp.Tables[0].Rows[lastrow][i] = (result_Tm1 == 0 || result2_Tm2 == 0) ? 0 : DBHelper.nvlDouble((((result_Tm1 / result2_Tm1) + (result_Tm2 / result2_Tm2)) / 2) * 100);
                    ...
                    for (int i = grid1.Columns.IndexOf("LASTAVG"); i < grid1.Columns.Count; i++)
                    {
                        ...
                        grid1.DisplayLayout.Rows[((DataRow)hash[steamcode + "|" + "MonthOutSUM"]).Table.Rows.IndexOf(((DataRow)hash[steamcode + "|" + "MonthOutSUM"]))].Cells[i].Appearance.FontData.Bold = Infragistics.Win.DefaultableBoolean.True;
                        grid1.DisplayLayout.Rows[((DataRow)hash[steamcode2 + "|" + "MonthOutSUM"]).Table.Rows.IndexOf(((DataRow)hash[steamcode2 + "|" + "MonthOutSUM"]))].Cells[i].Appearance.FontData.Bold = Infragistics.Win.DefaultableBoolean.True;
                        ...
                        grid1.Rows[((DataRow)hash[steamcode + "|" + "MonthOutSUM"]).Table.Rows.IndexOf(((DataRow)hash[steamcode + "|" + "MonthOutSUM"]))].Activation = Activation.NoEdit;
                        grid1.Rows[((DataRow)hash[steamcode2 + "|" + "MonthOutSUM"]).Table.Rows.IndexOf(((DataRow)hash[steamcode2 + "|" + "MonthOutSUM"]))].Activation = Activation.NoEdit;
                ...
                if (rtnDsTemp.Tables[0].Rows.Count > 0)
                {
                    ...
                    if (grid1.Rows.Count != 0)
                    {
                        ...
                        mainDt.Rows.Add(new object[] {"전년도",
                            grid1.Rows[((DataRow)hash[steamcode + "|" + "MonthOutSUM"]).Table.Rows.IndexOf(((DataRow)hash[steamcode + "|" + "MonthOutSUM"]))].Cells["LASTAVG"].Value.ToString(),
                            grid1.Rows[((DataRow)hash[steamcode2 + "|" + "MonthOutSUM"]).Table.Rows.IndexOf(((DataRow)hash[steamcode2 + "|" + "MonthOutSUM"]))].Cells["LASTAVG"].Value.ToString(),
                        ...
                        mainDt.Rows.Add(new object[] {"현재",
                            grid1.Rows[((DataRow)hash[steamcode + "|" + "MonthOutSUM"]).Table.Rows.IndexOf(((DataRow)hash[steamcode + "|" + "MonthOutSUM"]))].Cells["THISAVG"].Value.ToString(),
                            grid1.Rows[((DataRow)hash[steamcode2 + "|" + "MonthOutSUM"]).Table.Rows.IndexOf(((DataRow)hash[steamcode2 + "|" + "MonthOutSUM"]))].Cells["THISAVG"].Value.ToString(),
                        ...
                        for (int i = 1; i < 13; i++)
                        {
                            mainDt.Rows.Add(new object[] {i+"월",grid1.Rows[((DataRow)hash[steamcode + "|" + "MonthOutSUM"]).Table.Rows.IndexOf(((DataRow)hash[steamcode + "|" + "MonthOutSUM"]))].Cells[i+5].Value.ToString(),
                                grid1.Rows[((DataRow)hash[steamcode2 + "|" + "MonthOutSUM"]).Table.Rows.IndexOf(((DataRow)hash[steamcode2 + "|" + "MonthOutSUM"]))].Cells[i+5].Value.ToString(),
            ...
            foreach (DataRow drRow in dt.Rows)
            {
                switch (drRow.RowState)
                {
                    case DataRowState.Modified:
                        ...
                        if (DBHelper.gGetCode(drRow["MONSTOPEFFECT"]) == "MonthOutSUM")
                        {
                            continue;
                        }
                        ...
                        for (int i = 6; i < 18; i++)
                        {
                            ...
                            if (DBHelper.gGetCode(drRow[i]) != string.Empty)
                            {
                                ...
                                try
                                {
                                    ...
                                    param[1] = helper.CreateParameter("@deptcode", DBHelper.gGetCode(drRow["MINORCODE"]), DbType.String, ParameterDirection.Input);
                                    param[2] = helper.CreateParameter("@MONSTOPEFFECT", DBHelper.gGetCode(drRow["MONSTOPEFFECT"]), DbType.String, ParameterDirection.Input);     // 등록자\
                                    param[3] = helper.CreateParameter("@Y00", save_Y00, DbType.String, ParameterDirection.Input);
                                    param[4] = helper.CreateParameter("@M00", grid1.Columns[i].Key, DbType.String, ParameterDirection.Input);
                                    param[5] = helper.CreateParameter("@RstValue", DBHelper.gGetCode(drRow[i]), DbType.String, ParameterDirection.Input);
                                    param[6] = helper.CreateParameter("@USER", WIZ.LoginInfo.UserID, DbType.String, ParameterDirection.Input);
                                    ...
                                    helper.ExecuteNoneQuery("USP_PM0200_I1", CommandType.StoredProcedure, param);
            ...
            if (grid2.Rows.Count >= 1)
            {
                ...
                if (grid2.Rows[0].Cells["M00"].Value.ToString() != string.Empty)
                {
                    ...
                    try
                    {
                        ...
                        param[0] = helper.CreateParameter("@plantcode", save_plantcode, DbType.String, ParameterDirection.Input);
                        param[1] = helper.CreateParameter("@targetclass", "CL02", DbType.String, ParameterDirection.Input);
                        ...
                        helper.ExecuteNoneQuery("USP_PM0200_I2", CommandType.StoredProcedure, param);
        ...
        private void grid1_ClickCell(object sender, ClickCellEventArgs e)
        {
            ...
            if (e.Cell.Column.Header.Column.ToString() != "")
            {
                ...
                try
                {
                    ...
                    param[4] = helper.CreateParameter("@targetclass", "CL02", DbType.String, ParameterDirection.Input);
                    ...
                    DataTable rtnDtTemp = helper.FillTable("USP_PM0200_S2", CommandType.StoredProcedure, param);
