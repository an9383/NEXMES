...
namespace WIZ.PM
{
    public partial class PM0300 : WIZ.Forms.BaseMDIChildForm
    {
        #region <MEMBER AREA>
        ...
        private void PM0300_Load(object sender, EventArgs e)
        {
            #region Grid 셋팅
            //그리드 객체 생성
            ...
            _GridUtil.InitColumnUltraGrid(grid1, "MONSTOPNAME", "구분", false, GridColDataType_emu.VarChar, 120, 100, Infragistics.Win.HAlign.Left, true, false, null, null, null, null, null);
            ...
            string[] sMergeColumn2 = { "MONSTOPNAME", "MONSTOPEFFECT", "LASTAVG", "THISAVG" };
            string[] sMergeColumn3 = { "LASTAVG", "THISAVG" };
            string[] sMergeColumn4 = { "prod" };
            string[] sHeadColumn = { "prod", "MONSTOPNAME", "MONSTOPEFFECT", "LASTAVG", "THISAVG", "01", "02", "03", "04", "05", "06", "07", "08", "09", "10", "11", "12" };
        ...
        public override void DoInquire()
        {
            ...
            try
            {
                ...
                rtnDsTemp = helper.FillDataSet("USP_PM0300_S1", CommandType.StoredProcedure, param);
                ...
                {
                    hash.Add(DBHelper.gGetCode(drRow["MONSTOPEFFECT"]), drRow);
                }
                ...
                if (rtnDsTemp.Tables[0].Rows.Count > 0)
                {
                    ...
                    {
                        double result_Tm1 = Convert.ToDouble(DBHelper.nvlDouble(((DataRow)hash["MonthMIPout"])[i].ToString()));
                        double result_Tm2 = Convert.ToDouble(DBHelper.nvlDouble(((DataRow)hash["MonthMIPin"])[i].ToString()));
                        double result_Tm3 = Convert.ToDouble(DBHelper.nvlDouble(((DataRow)hash["MonthLS"])[i].ToString()));
                        ((DataRow)hash["MonthFailsum"])[i] = DBHelper.nvlDouble((result_Tm1 + result_Tm2 + result_Tm3));


                        rtnDsTemp.Tables[0].Rows[lastrow][i] = (DBHelper.nvlDouble(((DataRow)hash["Monthfail"])[i].ToString()) == 0) ? 0 : DBHelper.nvlDouble(DBHelper.nvlDouble((result_Tm1 + result_Tm2 + result_Tm3)) / DBHelper.nvlDouble(((DataRow)hash["Monthfail"])[i].ToString()) * 100);
                    ...
                    for (int i = grid1.Columns.IndexOf("LASTAVG"); i < grid1.Columns.Count; i++)
                    {
                        ...
                        grid1.Rows[((DataRow)hash["MonthFailsum"]).Table.Rows.IndexOf(((DataRow)hash["MonthFailsum"]))].Activation = Activation.NoEdit;
                        //grid1.Rows[((DataRow)hash["MonthOutSUM"]).Table.Rows.IndexOf(((DataRow)hash["MonthOutSUM"]))].Activation = Activation.NoEdit;
                ...
                if (rtnDsTemp.Tables[0].Rows.Count > 0)
                {
                    ...
                    mainDt.Columns.Add("MIP외주", typeof(double));
                    mainDt.Columns.Add("MIP사내", typeof(double));
                    mainDt.Columns.Add("LINESTOP", typeof(double));
                    ...
                    if (grid1.Rows.Count != 0)
                    {
                        ...
                        mainDt.Rows.Add(new object[] {"전년도",
                            DBHelper.nvlDouble(grid1.Rows[((DataRow)hash["MonthMIPout"]).Table.Rows.IndexOf(((DataRow)hash["MonthMIPout"]))].Cells["LASTAVG"].Value.ToString()),
                            DBHelper.nvlDouble(grid1.Rows[((DataRow)hash["MonthMIPin"]).Table.Rows.IndexOf(((DataRow)hash["MonthMIPin"]))].Cells["LASTAVG"].Value.ToString()),
                            DBHelper.nvlDouble(grid1.Rows[((DataRow)hash["MonthLS"]).Table.Rows.IndexOf(((DataRow)hash["MonthLS"]))].Cells["LASTAVG"].Value.ToString()),
                            DBHelper.nvlDouble(grid1.Rows[0].Cells[1].Value.ToString() == "목표(매출당 실패비용)"  ? grid1.Rows[0].Cells["LASTAVG"].Value : 0)});


                        mainDt.Rows.Add(new object[] {"현재",
                            DBHelper.nvlDouble(grid1.Rows[((DataRow)hash["MonthMIPout"]).Table.Rows.IndexOf(((DataRow)hash["MonthMIPout"]))].Cells["THISAVG"].Value.ToString()),
                            DBHelper.nvlDouble(grid1.Rows[((DataRow)hash["MonthMIPin"]).Table.Rows.IndexOf(((DataRow)hash["MonthMIPin"]))].Cells["THISAVG"].Value.ToString()),
                            DBHelper.nvlDouble(grid1.Rows[((DataRow)hash["MonthLS"]).Table.Rows.IndexOf(((DataRow)hash["MonthLS"]))].Cells["THISAVG"].Value.ToString()),
                            DBHelper.nvlDouble(grid1.Rows[0].Cells[1].Value.ToString() == "목표(매출당 실패비용)"  ? grid1.Rows[0].Cells["THISAVG"].Value : 0)});


                        for (int i = 1; i < 13; i++)
                        {
                            mainDt.Rows.Add(new object[] {i+"월",
                            DBHelper.nvlDouble(grid1.Rows[((DataRow)hash["MonthMIPout"]).Table.Rows.IndexOf(((DataRow)hash["MonthMIPout"]))].Cells[i+4].Value.ToString()),
                            DBHelper.nvlDouble(grid1.Rows[((DataRow)hash["MonthMIPin"]).Table.Rows.IndexOf(((DataRow)hash["MonthMIPin"]))].Cells[i+4].Value.ToString()),
                            DBHelper.nvlDouble(grid1.Rows[((DataRow)hash["MonthLS"]).Table.Rows.IndexOf(((DataRow)hash["MonthLS"]))].Cells[i+4].Value.ToString()),
                            DBHelper.nvlDouble(grid1.Rows[0].Cells[1].Value.ToString() == "목표(매출당 실패비용)"  ? grid1.Rows[0].Cells[i+4].Value : 0)});
                        }
                    }
                    ...
                    chart.Series.AddRange(new Infragistics.UltraChart.Data.Series.ISeries[] { seriesLine2, seriesColumn, seriesColumn2, seriesColumn3 });
            ...
            foreach (DataRow drRow in dt.Rows)
            {
                switch (drRow.RowState)
                {
                    case DataRowState.Modified:
                        ...
                        if (DBHelper.gGetCode(drRow["MONSTOPEFFECT"]) == "MonthFailsum")
                        {
                            continue;
                        }
                        ...
                        for (int i = 5; i < 17; i++)
                        {
                            ...
                            if (DBHelper.gGetCode(drRow[i]) != string.Empty)
                            {
                                ...
                                try
                                {
                                    ...
                                    param[1] = helper.CreateParameter("@MONSTOPEFFECT", DBHelper.gGetCode(drRow["MONSTOPEFFECT"]), DbType.String, ParameterDirection.Input);     // 등록자\
                                    ...
                                    helper.ExecuteNoneQuery("USP_PM0300_I1", CommandType.StoredProcedure, param);
            ...
            if (grid2.Rows.Count >= 1)
            {
                ...
                if (grid2.Rows[0].Cells["M00"].Value.ToString() != string.Empty)
                {
                    ...
                    try
                    {
                        ...
                        param[0] = helper.CreateParameter("@plantcode", save_plantcode, DbType.String, ParameterDirection.Input);
                        param[1] = helper.CreateParameter("@targetclass", "CL03", DbType.String, ParameterDirection.Input);
                        param[2] = helper.CreateParameter("@Y00", save_Y00, DbType.String, ParameterDirection.Input);
                        ...
                        helper.ExecuteNoneQuery("USP_PM0300_I2", CommandType.StoredProcedure, param);
        ...
        private void grid1_ClickCell(object sender, ClickCellEventArgs e)
        {
            ...
            if (e.Cell.Column.Header.Column.ToString() != "")
            {
                ...
                try
                {
                    ...
                    param[4] = helper.CreateParameter("@targetclass", "CL03", DbType.String, ParameterDirection.Input);
                    ...
                    DataTable rtnDtTemp = helper.FillTable("USP_PM0300_S2", CommandType.StoredProcedure, param);
