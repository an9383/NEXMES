...
//   Edited Date  : 
//   Edit By      :
//   Description  : 공정 검사항목 품목별 스펙을 관리 
// *---------------------------------------------------------------------------------------------*
#endregion
...
namespace WIZ.BM
{
    public partial class BM0415 : WIZ.Forms.BaseMDIChildForm
    {
        #region < MEMBER AREA >
        ...
        #region < FORM LOAD >
        private void BM0415_Load(object sender, EventArgs e)
        {
            try
            {
                #region GRID SETTING
        ...
        public override void DoInquire()
        {
            ...
            string sItemCode = DBHelper.nvlString(txt_INSPCODE_H.Text.Trim());
            string sInspCode = DBHelper.nvlString(txt_ITEMCODE_H.Text.Trim());
            ...
            try
            {
                ...
                rtnDtTemp = helper.FillTable("USP_BM0410_S2", CommandType.StoredProcedure
            ...
            try
            {
                ...
                rtnDtTemp = helper.FillTable("USP_BM0410_S3", CommandType.StoredProcedure
                                                            , helper.CreateParameter("AS_PLANTCODE", sPlantCode, DbType.String, ParameterDirection.Input)
                ...
                foreach (DataRow drRow in rtnDtTemp.Rows)
                {
                    ...
                    switch (drRow.RowState)
                    {
                        case DataRowState.Deleted:
                            ...
                            helper.ExecuteNoneQuery("USP_BM0410_D1"
                        ...
                        case DataRowState.Modified:
                            #region 수정
                            helper.ExecuteNoneQuery("USP_BM0410_U1"
                                                     , CommandType.StoredProcedure
                                                     , helper.CreateParameter("AS_PLANTCODE", DBHelper.nvlString(drRow["PLANTCODE"]), DbType.String, ParameterDirection.Input)
                                                     , helper.CreateParameter("AS_ITEMCODE", DBHelper.nvlString(drRow["ITEMCODE"]), DbType.String, ParameterDirection.Input)
                                                     , helper.CreateParameter("AS_INSPCODE", DBHelper.nvlString(drRow["INSPCODE"]), DbType.String, ParameterDirection.Input)
        ...
        public override void DoImportExcel()
        {
            ...
            BM0410_EXCEL BM0410_excel = new BM0410_EXCEL();
            BM0410_excel.ShowDialog();
        ...
        private void grid2_ClickCell(object sender, ClickCellEventArgs e)
        {
            ...
            try
            {
                ...
                rtnDtTemp = helper.FillTable("USP_BM0410_S1", CommandType.StoredProcedure
                                                            , helper.CreateParameter("AS_PLANTCODE", sPlantCode, DbType.String, ParameterDirection.Input)
                                                            , helper.CreateParameter("AS_ITEMCODE", sItemCode, DbType.String, ParameterDirection.Input)
                                                            , helper.CreateParameter("AS_INSPCODE", sInspCode, DbType.String, ParameterDirection.Input)
                                                            , helper.CreateParameter("AS_USEFLAG", sUseFlag, DbType.String, ParameterDirection.Input));
        ...
        private void btn_UPLOAD_B_Click(object sender, EventArgs e)
        {
            ...
            openFileDlg.Filter = "비트맵 (*.bmp)|*.bmp|GIF (*.gif)|*.gif|PDF (*.pdf)|*.pdf|JPEG (*.jpg)|*.jpg|PNG (*.png)|*.png|사용 가능한 이미지 파일|*.bmp;*.pdf;;*.gif;*.jpg;*.png";
            ...
            if (drResult == DialogResult.OK)
            {
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        try
                        {
                            helper2.ExecuteNoneQuery("USP_BM0410_U2", CommandType.StoredProcedure
        ...
        private byte[] GetImage(string sPlantCode, string sItemCode, string sInspCode)
        {
            ...
            try
            {
                rtnDtTemp = helper.FillTable("USP_BM0410_S4", CommandType.StoredProcedure
                          , helper.CreateParameter("AS_PLANTCODE", sPlantCode, DbType.String, ParameterDirection.Input)
                          , helper.CreateParameter("AS_ITEMCODE", sItemCode, DbType.String, ParameterDirection.Input)
                          , helper.CreateParameter("AS_INSPCODE", sInspCode, DbType.String, ParameterDirection.Input));
        ...
        public byte[] ConvertImageToByteArray(Image theImage)
        {
            ...
            pByte = new byte[ms.Length];  //무명함수적용 (메모리스트림에 있는 length값을 byte 배열로 변환)
            ms.Position = 0;
