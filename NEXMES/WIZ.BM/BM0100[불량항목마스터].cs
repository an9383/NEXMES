...
namespace WIZ.BM
{
    public partial class BM0100 : WIZ.Forms.BaseMDIChildForm
    {
        #region < MEMBER AREA >
        ...
        private void BM0100_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                _GridUtil.InitColumnUltraGrid(grid1, "ERRORCODE", "불량코드", true, GridColDataType_emu.VarChar, 90, 90, Infragistics.Win.HAlign.Center, true, false);
                _GridUtil.InitColumnUltraGrid(grid1, "ERRORDESC", "불량명", true, GridColDataType_emu.VarChar, 150, 100, Infragistics.Win.HAlign.Left, true, true);
                _GridUtil.InitColumnUltraGrid(grid1, "ERRORTYPE", "불량구분", true, GridColDataType_emu.VarChar, 160, 100, Infragistics.Win.HAlign.Left, true, true);
                _GridUtil.InitColumnUltraGrid(grid1, "ERRORCLASS", "불량유형", true, GridColDataType_emu.VarChar, 100, 100, Infragistics.Win.HAlign.Left, true, true);
                ...
                grid1.DisplayLayout.Bands[0].Columns["ERRORTYPE"].Header.Appearance.ForeColor = Color.SkyBlue;
                #endregion
                ...
                rtnDtTemp = _Common.GET_BM0000_CODE("ERRORTYPE");  //불량 구분
                ...
                //불량항목
                btbManager.PopUpAdd(txt_ERRORCODE_H, txt_ERRORDESC_H, "BM0100", new object[] { cbo_ERRORTYPE_H, cbo_ERRORTYPE_H, cbo_USEFLAG_H });
        ...
        public override void DoInquire()
        {
            ...
            try
            {
                ...
                string sErrorDecs = DBHelper.nvlString(txt_ERRORDESC_H.Text.Trim());
                string sErrorType = DBHelper.nvlString(cbo_ERRORTYPE_H.Value);
                string sUseFlag = DBHelper.nvlString(cbo_USEFLAG_H.Value);

                rtnDtTemp = helper.FillTable("USP_BM0100_S1", CommandType.StoredProcedure
                                                            , helper.CreateParameter("AS_PLANTCODE", sPlantCode, DbType.String, ParameterDirection.Input)
                                                            , helper.CreateParameter("AS_ERRORCODE", sErrorCode, DbType.String, ParameterDirection.Input)
                                                            , helper.CreateParameter("AS_ERRORDECS", sErrorDecs, DbType.String, ParameterDirection.Input)
        ...
        public override void DoNew()
        {
            try
            {
                ...
                this.grid1.ActiveRow.Cells["ERRORCODE"].Value = Common.getLangText("자동채번", "TEXT");
            ...
            try
            {
                ...
                foreach (DataRow drRow in rtnDtTemp.Rows)
                {
                    ...
                    if (drRow.RowState != DataRowState.Deleted)
                    {
                        ...
                        if (Convert.ToString(drRow["ERRORTYPE"]) == string.Empty)
                        {
                            this.ClosePrgFormNew();
                            this.ShowDialog(Common.getLangText("불량 구분은 필수 입력항목입니다..", "MSG"), Forms.DialogForm.DialogType.OK);
                            return;
                        }
                    ...
                    switch (drRow.RowState)
                    {
                        ...
                        case DataRowState.Added:
                            #region 추가                            
                            helper.ExecuteNoneQuery("USP_BM0100_I1"
                                                    , CommandType.StoredProcedure
                                                    , helper.CreateParameter("AS_PLANTCODE", DBHelper.nvlString(drRow["PLANTCODE"]), DbType.String, ParameterDirection.Input)
                                                    , helper.CreateParameter("AS_ERRORDESC", DBHelper.nvlString(drRow["ERRORDESC"]), DbType.String, ParameterDirection.Input)
                                                    , helper.CreateParameter("AS_ERRORTYPE", DBHelper.nvlString(drRow["ERRORTYPE"]), DbType.String, ParameterDirection.Input)
                                                    , helper.CreateParameter("AS_ERRORCLASS", DBHelper.nvlString(drRow["ERRORCLASS"]), DbType.String, ParameterDirection.Input)
                        ...
                        case DataRowState.Modified:
                            #region 수정
                            helper.ExecuteNoneQuery("USP_BM0100_U1"
                                                    , CommandType.StoredProcedure
                                                    , helper.CreateParameter("AS_PLANTCODE", DBHelper.nvlString(drRow["PLANTCODE"]), DbType.String, ParameterDirection.Input)
        ...
        public override void DoImportExcel()
        {
            ...
            BM0100_EXCEL bm0100_excel = new BM0100_EXCEL();
            bm0100_excel.ShowDialog();
