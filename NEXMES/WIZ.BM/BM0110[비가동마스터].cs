...
namespace WIZ.BM
{
    public partial class BM0110 : WIZ.Forms.BaseMDIChildForm
    {
        #region < MEMBER AREA >
        ...
        #region < FORM LOAD >
        private void BM0110_Load(object sender, EventArgs e)
        {
            try
            {
                #region GRID SETTING
                ...
                _GridUtil.InitColumnUltraGrid(grid1, "STOPDESC", "비가동명", true, GridColDataType_emu.VarChar, 150, 100, Infragistics.Win.HAlign.Left, true, true);
                ...
                _GridUtil.InitColumnUltraGrid(grid1, "STOPCLASS", "비가동유형", true, GridColDataType_emu.VarChar, 120, 100, Infragistics.Win.HAlign.Left, true, true);
                _GridUtil.InitColumnUltraGrid(grid1, "STOPMH", "인당공수집계구분", true, GridColDataType_emu.VarChar, 120, 100, Infragistics.Win.HAlign.Left, true, true);
                _GridUtil.InitColumnUltraGrid(grid1, "STOPMCH", "설비공수집계구분", true, GridColDataType_emu.VarChar, 120, 100, Infragistics.Win.HAlign.Left, true, true);
                _GridUtil.InitColumnUltraGrid(grid1, "STOPCL", "프로세스집계구분", true, GridColDataType_emu.VarChar, 120, 100, Infragistics.Win.HAlign.Left, true, true);
                _GridUtil.InitColumnUltraGrid(grid1, "REPORTFLAG", "레포트 집계유무", true, GridColDataType_emu.VarChar, 120, 100, Infragistics.Win.HAlign.Left, true, true);
                _GridUtil.InitColumnUltraGrid(grid1, "VIEWFLAG", "현장단말기\r\n표시유무", true, GridColDataType_emu.VarChar, 90, 100, Infragistics.Win.HAlign.Left, true, true);
                ...
                WIZ.Common.FillComboboxMaster(cbo_STOPTYPE_H, rtnDtTemp, rtnDtTemp.Columns["CODE_ID"].ColumnName, rtnDtTemp.Columns["CODE_NAME"].ColumnName, "ALL", "");
        ...
        public override void DoInquire()
        {
            ...
            try
            {
                ...
                rtnDtTemp = helper.FillTable("USP_BM0110_S1", CommandType.StoredProcedure
        ...
        public override void DoNew()
        {
            try
            {
                ...
                this.grid1.ActiveRow.Cells["STOPCODE"].Value = Common.getLangText("자동채번", "TEXT");
            ...
            try
            {
                ...
                foreach (DataRow drRow in rtnDtTemp.Rows)
                {
                    ...
                    if (drRow.RowState != DataRowState.Deleted)
                    {
                        ...
                        if (Convert.ToString(drRow["STOPTYPE"]) == string.Empty)
                        {
                            this.ClosePrgFormNew();
                            this.ShowDialog(Common.getLangText("비가동 구분은 필수 입력항목입니다..", "MSG"), Forms.DialogForm.DialogType.OK);
                            return;
                        }
                    ...
                    switch (drRow.RowState)
                    {
                        ...
                        case DataRowState.Added:
                            #region 추가                            
                            helper.ExecuteNoneQuery("USP_BM0110_I1"
                                                    , CommandType.StoredProcedure
                                                    , helper.CreateParameter("AS_PLANTCODE", DBHelper.nvlString(drRow["PLANTCODE"]), DbType.String, ParameterDirection.Input)
                                                    , helper.CreateParameter("AS_STOPTYPE", DBHelper.nvlString(drRow["STOPTYPE"]), DbType.String, ParameterDirection.Input)
                                                    , helper.CreateParameter("AS_STOPDESC", DBHelper.nvlString(drRow["STOPDESC"]), DbType.String, ParameterDirection.Input)
                                                    , helper.CreateParameter("AS_STOPCLASS", DBHelper.nvlString(drRow["STOPCLASS"]), DbType.String, ParameterDirection.Input)
                                                    , helper.CreateParameter("AS_STOPMH", DBHelper.nvlString(drRow["STOPMH"]), DbType.String, ParameterDirection.Input)
                                                    , helper.CreateParameter("AS_STOPMCH", DBHelper.nvlString(drRow["STOPMCH"]), DbType.String, ParameterDirection.Input)
                                                    , helper.CreateParameter("AS_STOPCL", DBHelper.nvlString(drRow["STOPCL"]), DbType.String, ParameterDirection.Input)
                                                    , helper.CreateParameter("AS_REPORTFLAG", DBHelper.nvlString(drRow["REPORTFLAG"]), DbType.String, ParameterDirection.Input)
                                                    , helper.CreateParameter("AS_VIEWFLAG", DBHelper.nvlString(drRow["VIEWFLAG"]), DbType.String, ParameterDirection.Input)
                        ...
                        case DataRowState.Modified:
                            #region 수정
                            helper.ExecuteNoneQuery("USP_BM0110_U1"
                                                    , CommandType.StoredProcedure
                                                    , helper.CreateParameter("AS_PLANTCODE", DBHelper.nvlString(drRow["PLANTCODE"]), DbType.String, ParameterDirection.Input)
                                                    , helper.CreateParameter("AS_STOPTYPE", DBHelper.nvlString(drRow["STOPTYPE"]), DbType.String, ParameterDirection.Input)
        ...
        public override void DoImportExcel()
        {
            ...
            BM0110_EXCEL bm0110_excel = new BM0110_EXCEL();
            bm0110_excel.ShowDialog();
