...
namespace WIZ.BM
{
    public partial class BM0610 : WIZ.Forms.BaseMDIChildForm
    {
        #region < MEMBER AREA >
        ...
        #region < FORM LOAD >
        private void BM0610_Load(object sender, EventArgs e)
        {
            try
            {
                #region GRID SETTING
                ...
                _GridUtil.InitColumnUltraGrid(grid1, "DASPOINTNAME", "하드웨어 명", false, GridColDataType_emu.VarChar, 200, 100, Infragistics.Win.HAlign.Left, true, true);
                _GridUtil.InitColumnUltraGrid(grid1, "DASTYPE", "하드웨어 유형", false, GridColDataType_emu.VarChar, 120, 100, Infragistics.Win.HAlign.Left, true, false);
                _GridUtil.InitColumnUltraGrid(grid1, "IPADDRESS", "할당 I.P", false, GridColDataType_emu.VarChar, 160, 100, Infragistics.Win.HAlign.Center, true, true);
                _GridUtil.InitColumnUltraGrid(grid1, "LOCATION", "설치 장소", false, GridColDataType_emu.VarChar, 120, 100, Infragistics.Win.HAlign.Left, true, true);
                _GridUtil.InitColumnUltraGrid(grid1, "SERIALNO", "시리얼번호", false, GridColDataType_emu.VarChar, 120, 100, Infragistics.Win.HAlign.Center, true, true);
                ...
                WIZ.Common.FillComboboxMaster(this.cbo_DASTYPE_H, rtnDtTemp, rtnDtTemp.Columns["CODE_ID"].ColumnName, rtnDtTemp.Columns["CODE_NAME"].ColumnName, "ALL", "");
        ...
        public override void DoInquire()
        {
            ...
            try
            {
                ...
                string sDasType = DBHelper.nvlString(cbo_DASTYPE_H.Value);                  // 하드웨어 종류
                string sIpAddress = DBHelper.nvlString(txt_IPADDRESS_H.Text.Trim());          // 할당IP
                string sUseFlag = DBHelper.nvlString(cbo_USEFLAG_H.Value);

                rtnDtTemp = helper.FillTable("USP_BM0610_S1", CommandType.StoredProcedure
            ...
            try
            {
                ...
                foreach (DataRow drRow in rtnDtTemp.Rows)
                {
                    ...
                    if (drRow.RowState != DataRowState.Deleted)
                    {
                        ...
                        if (Convert.ToString(drRow["DASTYPE"]) == string.Empty)
                        {
                            this.ClosePrgFormNew();
                            this.ShowDialog(Common.getLangText("하드웨어 종류는 필수 입력항목입니다.", "MSG"), Forms.DialogForm.DialogType.OK);
                            return;
                        }


                        if (Convert.ToString(drRow["DASPOINTID"]) == string.Empty)
                        {
                            this.ClosePrgFormNew();
                            this.ShowDialog(Common.getLangText("하드웨어 ID는 필수 입력항목입니다.", "MSG"), Forms.DialogForm.DialogType.OK);
                            return;
                        }
                    ...
                    switch (drRow.RowState)
                    {
                        ...
                        case DataRowState.Added:
                            #region 추가                            
                            helper.ExecuteNoneQuery("USP_BM0610_I1"
                                                    , CommandType.StoredProcedure
                                                    , helper.CreateParameter("AS_PLANTCODE", DBHelper.nvlString(drRow["PLANTCODE"]), DbType.String, ParameterDirection.Input)
                                                    , helper.CreateParameter("AS_DASTYPE", DBHelper.nvlString(drRow["DASTYPE"]), DbType.String, ParameterDirection.Input)
                                                    , helper.CreateParameter("AS_DASPOINTID", DBHelper.nvlString(drRow["DASPOINTID"]), DbType.String, ParameterDirection.Input)
                                                    , helper.CreateParameter("AS_DASPOINTNAME", DBHelper.nvlString(drRow["DASPOINTNAME"]), DbType.String, ParameterDirection.Input)
                                                    , helper.CreateParameter("AS_IPADDRESS", DBHelper.nvlString(drRow["IPADDRESS"]), DbType.String, ParameterDirection.Input)
                                                    , helper.CreateParameter("AS_LOCATION", DBHelper.nvlString(drRow["LOCATION"]), DbType.String, ParameterDirection.Input)
                        ...
                        case DataRowState.Modified:
                            #region 수정
                            helper.ExecuteNoneQuery("USP_BM0610_U1"
                                                     , CommandType.StoredProcedure
                                                     , helper.CreateParameter("AS_PLANTCODE", DBHelper.nvlString(drRow["PLANTCODE"]), DbType.String, ParameterDirection.Input)
                                                     , helper.CreateParameter("AS_DASTYPE", DBHelper.nvlString(drRow["DASTYPE"]), DbType.String, ParameterDirection.Input)
                                                     , helper.CreateParameter("AS_DASPOINTID", DBHelper.nvlString(drRow["DASPOINTID"]), DbType.String, ParameterDirection.Input)
        ...
        public override void DoImportExcel()
        {
            ...
            BM0610_EXCEL bm0610_excel = new BM0610_EXCEL();
            bm0610_excel.ShowDialog();
