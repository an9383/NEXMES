...
// *---------------------------------------------------------------------------------------------*                                                                                                  
//   Form ID      : BM3640                                                                                                                                                                          
//   Form Name    : 완성품검사항목 품목별 스펙 관리 (PD)                                                                                                                                                                    
//   Name Space   : WIZ.BM                                                                                                                                                                        
...
namespace WIZ.BM
{
    ...
    public partial class BM3640 : WIZ.Forms.BaseMDIChildForm
    {
        #region <MEMBER AREA>
        ...
        #region BM3640_Load
        private void BM3640_Load(object sender, EventArgs e)
        {
            #region Grid 셋팅
            ...
            _GridUtil.InitColumnUltraGrid(grid1, "DisplayNo", "표시순서", true, GridColDataType_emu.VarChar, 100, 100, Infragistics.Win.HAlign.Center, true, true, null, null, null, null, null);          // 표시순서                                                
            ...
            */
            ...
            rtnDtTemp = _Common.GET_BM0000_CODE("InspCase", " MinorCode ='PD'");    //검사구분 완성검사)                                                                                                                                                                                 
        ...
        public override void DoInquire()
        {
            ...
            try
            {
                ...
                grid1.DataSource = helper.FillTable("USP_BM3640_S1N", CommandType.StoredProcedure
                ...
                foreach (DataRow drRow in dt.Rows)
                {
                    ...
                    switch (drRow.RowState)
                    {
                        case DataRowState.Deleted:
                            ...
                            helper.ExecuteNoneQuery("USP_BM3640_D1N", CommandType.StoredProcedure
                                                    ...
                                                    , helper.CreateParameter("InspCode", Convert.ToString(drRow["InspCode"]), DbType.String, ParameterDirection.Input));           // 검사항목코드
                        ...
                        case DataRowState.Added:
                            ...
                            sPlantCode = Convert.ToString(drRow["PlantCode"]);
                            helper.ExecuteNoneQuery("USP_BM3640_I1N", CommandType.StoredProcedure
                                                    , helper.CreateParameter("PlantCode", Convert.ToString(drRow["PlantCode"]), DbType.String, ParameterDirection.Input)            // 공장코드
                                                    , helper.CreateParameter("ItemCode", Convert.ToString(drRow["ItemCode"]), DbType.String, ParameterDirection.Input)            // 품목
                                                    , helper.CreateParameter("InspCode", Convert.ToString(drRow["InspCode"]), DbType.String, ParameterDirection.Input)            // 검사항목코드
                                                    , helper.CreateParameter("DisplayNo", Convert.ToString(drRow["DisplayNo"]), DbType.String, ParameterDirection.Input)            // 표시순서
                                                    , helper.CreateParameter("SpecLSL", Convert.ToString(drRow["SpecLSL"]), DbType.String, ParameterDirection.Input)            // 하한치
                                                    , helper.CreateParameter("SpecUSL", Convert.ToString(drRow["SpecUSL"]), DbType.String, ParameterDirection.Input)            // 상한치
                                                    , helper.CreateParameter("InspRequired", Convert.ToString(drRow["InspRequired"]), DbType.String, ParameterDirection.Input)            // 검사필수여부
                                                    , helper.CreateParameter("InspPeriod", Convert.ToString(drRow["InspPeriod"]), DbType.String, ParameterDirection.Input)            // 검사주기(일/주/월)
                                                    , helper.CreateParameter("InspCycle", Convert.ToString(drRow["InspCycle"]), DbType.String, ParameterDirection.Input)            // 검사주기(별)
                                                    , helper.CreateParameter("InspMethod", Convert.ToString(drRow["InspMethod"]), DbType.String, ParameterDirection.Input)            // 검사수집장비
                                                    , helper.CreateParameter("InspCount", Convert.ToString(drRow["InspCount"]), DbType.String, ParameterDirection.Input)            // 검사횟수
                                                    , helper.CreateParameter("InspValType", Convert.ToString(drRow["InspValType"]), DbType.String, ParameterDirection.Input)            // 검사정보구분(양호/불량 OK/NOK 확인 값입력)
                        ...
                        case DataRowState.Modified:
                            ...
                            //param = new System.Data.Common.DbParameter[16];
                            sPlantCode = Convert.ToString(drRow["PlantCode"]);
                            helper.ExecuteNoneQuery("USP_BM3640_U1N", CommandType.StoredProcedure
                                                    , helper.CreateParameter("PlantCode", Convert.ToString(drRow["PlantCode"]), DbType.String, ParameterDirection.Input)            // 공장코드
                                                    , helper.CreateParameter("ItemCode", Convert.ToString(drRow["ItemCode"]), DbType.String, ParameterDirection.Input)            // 품목
                                                    , helper.CreateParameter("InspCode", Convert.ToString(drRow["InspCode"]), DbType.String, ParameterDirection.Input)            // 검사항목코드
                                                    , helper.CreateParameter("DisplayNo", Convert.ToString(drRow["DisplayNo"]), DbType.String, ParameterDirection.Input)            // 표시순서
