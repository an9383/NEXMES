...
// *---------------------------------------------------------------------------------------------*
//   Form ID      : BM0330
//   Form Name    : 작업장별 비가동내역
//   Name Space   : WIZ.BM
//   Created Date : 2018-01-09
//   Made By      : WIZCORE 남부사무소 사원 김병수
//   Edited Date  : 
//   Edit By      :
//   Description  : 작업장별로 발생할수 있는 비가동을 관리                 
// *---------------------------------------------------------------------------------------------*
#endregion
...
namespace WIZ.BM
{
    public partial class BM0330 : WIZ.Forms.BaseMDIChildForm
    {
        #region < MEMBER AREA >
        ...
        #region < FORM LOAD >
        private void BM0330_Load(object sender, EventArgs e)
        {
            try
            {
                #region GRID SETTING

                //GRID1 작업장별 비가동내역 (GRID1은 우측에 위치한 MAIN GRID!)
                ...
                _GridUtil.InitColumnUltraGrid(grid1, "WORKCENTERNAME", "작업장명", true, GridColDataType_emu.VarChar, 135, 130, Infragistics.Win.HAlign.Left, true, false);
        ...
        public override void DoInquire()
        {
            ...
            try
            {
                ...
                rtnDtTemp = helper.FillTable("USP_BM0330_S2", CommandType.StoredProcedure
            ...
            try
            {
                ...
                rtnDtTemp = helper.FillTable("USP_BM0330_S3", CommandType.StoredProcedure
                                                            , helper.CreateParameter("AS_PLANTCODE", sPlantCode, DbType.String, ParameterDirection.Input)
            ...
            try
            {
                ...
                foreach (DataRow drRow in rtnDtTemp.Rows)
                {
                    ...
                    if (drRow.RowState != DataRowState.Deleted)
                    {
                        ...
                        {
                            this.ClosePrgFormNew();
                            this.ShowDialog(Common.getLangText("작업장코드는 필수 입력항목입니다..", "MSG"), Forms.DialogForm.DialogType.OK);
                            return;
                        }
                    ...
                    switch (drRow.RowState)
                    {
                        ...
                        case DataRowState.Added:
                            #region 추가                            
                            helper.ExecuteNoneQuery("USP_BM0330_I1"
                        ...
                        case DataRowState.Modified:
                            #region 수정
                            helper.ExecuteNoneQuery("USP_BM0330_U1"
                                                     , CommandType.StoredProcedure
                                                     , helper.CreateParameter("AS_PLANTCODE", DBHelper.nvlString(drRow["PLANTCODE"]), DbType.String, ParameterDirection.Input)
                                                     , helper.CreateParameter("AS_WORKCENTERCODE", DBHelper.nvlString(drRow["WORKCENTERCODE"]), DbType.String, ParameterDirection.Input)
                                                     , helper.CreateParameter("AS_STOPCODE", DBHelper.nvlString(drRow["STOPCODE"]), DbType.String, ParameterDirection.Input)
        ...
        public override void DoImportExcel()
        {
            ...
            BM0330_EXCEL bm0330_excel = new BM0330_EXCEL();
            bm0330_excel.ShowDialog();
        ...
        private void grid2_DoubleClickRow(object sender, DoubleClickRowEventArgs e)
        {
            ...
            try
            {
                ...
                rtnDtTemp = helper.FillTable("USP_BM0330_S1", CommandType.StoredProcedure
                                                            , helper.CreateParameter("AS_PLANTCODE", sPlantCode, DbType.String, ParameterDirection.Input)
                                                            , helper.CreateParameter("AS_WORKCENTERCODE", sWorkcenterCode, DbType.String, ParameterDirection.Input)
                                                            , helper.CreateParameter("AS_STOPCODE", sStopCode, DbType.String, ParameterDirection.Input)
                                                            , helper.CreateParameter("AS_USEFLAG", sUseFlag, DbType.String, ParameterDirection.Input));
