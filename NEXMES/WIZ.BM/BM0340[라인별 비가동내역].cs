...
//   Edited Date  : 
//   Edit By      :
//   Description  : 라인별로 발생할수 있는 비가동을 관리                   
// *---------------------------------------------------------------------------------------------*
#endregion
...
namespace WIZ.BM
{
    public partial class BM0340 : WIZ.Forms.BaseMDIChildForm
    {
        #region < MEMBER AREA >
        ...
        #region < FORM LOAD >
        private void BM0340_Load(object sender, EventArgs e)
        {
            try
            {
                #region GRID SETTING

                //GRID1 라인별 비가동내역 (GRID1은 우측에 위치한 MAIN GRID!)
                ...
                _GridUtil.InitColumnUltraGrid(grid1, "LINECODE", "라인코드", true, GridColDataType_emu.VarChar, 90, 100, Infragistics.Win.HAlign.Center, true, false);
                _GridUtil.InitColumnUltraGrid(grid1, "LINENAME", "라인명", true, GridColDataType_emu.VarChar, 150, 130, Infragistics.Win.HAlign.Left, true, false);
                _GridUtil.InitColumnUltraGrid(grid1, "STOPCODE", "비가동코드", true, GridColDataType_emu.VarChar, 90, 90, Infragistics.Win.HAlign.Center, true, false);
                ...
                //GRID2 라인 (GRID2는 좌측 상단 첫번쨰 GRID) (MAIN GRID를 제외한 GRID는 사업장 / 코드 / 코드명 만 출력) (GRID EDITABLE 은 FALSE)
                ...
                _GridUtil.InitColumnUltraGrid(grid2, "LINECODE", "라인코드", true, GridColDataType_emu.VarChar, 100, 100, Infragistics.Win.HAlign.Center, true, false);
                _GridUtil.InitColumnUltraGrid(grid2, "LINENAME", "라인명", true, GridColDataType_emu.VarChar, 175, 130, Infragistics.Win.HAlign.Left, true, false);

                _GridUtil.SetInitUltraGridBind(grid2);


                //GRID3 비가동항목 (GRID3는 좌측 상단 두번쨰 GRID, GRID를 추가시 좌측상단부터 GRID 채번) (GRID EDITABLE 은 FALSE)
                ...
                _GridUtil.InitColumnUltraGrid(grid3, "STOPCODE", "비가동코드", true, GridColDataType_emu.VarChar, 100, 90, Infragistics.Win.HAlign.Center, true, false);
                _GridUtil.InitColumnUltraGrid(grid3, "STOPDESC", "비가동명", true, GridColDataType_emu.VarChar, 175, 100, Infragistics.Win.HAlign.Left, true, false);
        ...
        public override void DoInquire()
        {
            ...
            try
            {
                ...
                rtnDtTemp = helper.FillTable("USP_BM0340_S2", CommandType.StoredProcedure
            ...
            try
            {
                ...
                rtnDtTemp = helper.FillTable("USP_BM0340_S3", CommandType.StoredProcedure
                                                            , helper.CreateParameter("AS_PLANTCODE", sPlantCode, DbType.String, ParameterDirection.Input)
        ...
        public override void DoNew()
        {
            ...
            {
                this.ShowDialog(Common.getLangText("라인을 선택해주세요.", "MSG"), Forms.DialogForm.DialogType.OK);
                return;
            }
            else if (grid3.ActiveRow == null)
            {
                this.ShowDialog(Common.getLangText("비가동코드를 선택해주세요.", "MSG"), Forms.DialogForm.DialogType.OK);
                return;
            }
            ...
            try
            {
                ...
                this.grid1.ActiveRow.Cells["LINECODE"].Value = DBHelper.nvlString(grid2.ActiveRow.Cells["LINECODE"].Value);
                this.grid1.ActiveRow.Cells["LINENAME"].Value = DBHelper.nvlString(grid2.ActiveRow.Cells["LINENAME"].Value);
                //GRID3의 정보
                this.grid1.ActiveRow.Cells["STOPCODE"].Value = DBHelper.nvlString(grid3.ActiveRow.Cells["STOPCODE"].Value);
                this.grid1.ActiveRow.Cells["STOPDESC"].Value = DBHelper.nvlString(grid3.ActiveRow.Cells["STOPDESC"].Value);
                ...
                grid1.ActiveRow.Cells["LINECODE"].Activation = Activation.NoEdit;
                grid1.ActiveRow.Cells["LINENAME"].Activation = Activation.NoEdit;
                grid1.ActiveRow.Cells["STOPCODE"].Activation = Activation.NoEdit;
                grid1.ActiveRow.Cells["STOPDESC"].Activation = Activation.NoEdit;
            ...
            try
            {
                ...
                foreach (DataRow drRow in rtnDtTemp.Rows)
                {
                    ...
                    if (drRow.RowState != DataRowState.Deleted)
                    {
                        ...
                        {
                            this.ClosePrgFormNew();
                            this.ShowDialog(Common.getLangText("라인은 필수 입력항목입니다..", "MSG"), Forms.DialogForm.DialogType.OK);
                            return;
                        }
                        if (Convert.ToString(drRow["STOPCODE"]) == string.Empty)
                        {
                            this.ClosePrgFormNew();
                            this.ShowDialog(Common.getLangText("비가동코드는 필수 입력항목입니다..", "MSG"), Forms.DialogForm.DialogType.OK);
                            return;
                        }
                    ...
                    switch (drRow.RowState)
                    {
                        ...
                        case DataRowState.Added:
                            #region 추가                            
                            helper.ExecuteNoneQuery("USP_BM0340_I1"
                        ...
                        case DataRowState.Modified:
                            #region 수정
                            helper.ExecuteNoneQuery("USP_BM0340_U1"
                                                     , CommandType.StoredProcedure
                                                     , helper.CreateParameter("AS_PLANTCODE", DBHelper.nvlString(drRow["PLANTCODE"]), DbType.String, ParameterDirection.Input)
                                                     , helper.CreateParameter("AS_LINECODE", DBHelper.nvlString(drRow["LINECODE"]), DbType.String, ParameterDirection.Input)
                                                     , helper.CreateParameter("AS_STOPCODE", DBHelper.nvlString(drRow["STOPCODE"]), DbType.String, ParameterDirection.Input)
        ...
        public override void DoImportExcel()
        {
            ...
            BM0340_EXCEL bm0340_excel = new BM0340_EXCEL();
            bm0340_excel.ShowDialog();
        ...
        private void grid2_DoubleClickRow(object sender, DoubleClickRowEventArgs e)
        {
            ...
            try
            {
                ...
                string sLineCode = DBHelper.nvlString(grid2.ActiveRow.Cells["LINECODE"].Value);
                string sUseFlag = DBHelper.nvlString(cbo_USEFLAG_H.Value);
                ...
                rtnDtTemp = helper.FillTable("USP_BM0340_S1", CommandType.StoredProcedure
                                                            , helper.CreateParameter("AS_PLANTCODE", sPlantCode, DbType.String, ParameterDirection.Input)
                                                            , helper.CreateParameter("AS_LINECODE", sLineCode, DbType.String, ParameterDirection.Input)
                                                            , helper.CreateParameter("AS_STOPCODE", sStopCode, DbType.String, ParameterDirection.Input)
                                                            , helper.CreateParameter("AS_USEFLAG", sUseFlag, DbType.String, ParameterDirection.Input));
