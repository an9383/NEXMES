...
namespace WIZ.BM
{
    public partial class BM8000 : WIZ.Forms.BaseMDIChildForm
    {
        #region < MEMBER AREA >
        ...
        #region < FORM LOAD >
        private void BM8000_Load(object sender, EventArgs e)
        {
            try
            {
                #region GRID SETTING
                ...
                _GridUtil.InitColumnUltraGrid(grid1, "ULLAGECODE", "결감코드", true, GridColDataType_emu.VarChar, 90, 90, Infragistics.Win.HAlign.Center, true, false);
                _GridUtil.InitColumnUltraGrid(grid1, "ULLAGENAME", "결감명", true, GridColDataType_emu.VarChar, 150, 100, Infragistics.Win.HAlign.Left, true, true);
                _GridUtil.InitColumnUltraGrid(grid1, "ULLAGETYPE", "결감구분", true, GridColDataType_emu.VarChar, 160, 100, Infragistics.Win.HAlign.Left, true, true);
                ...
                grid1.DisplayLayout.Bands[0].Columns["ULLAGECODE"].Header.Appearance.ForeColor = Color.SkyBlue;
                #endregion
                ...
                rtnDtTemp = _Common.GET_BM0000_CODE("ULLAGETYPE"); //결감 유형
        ...
        public override void DoInquire()
        {
            ...
            try
            {
                ...
                string sUllageType = DBHelper.nvlString(cbo_ULLAGETYPE_H.Value);

                rtnDtTemp = helper.FillTable("USP_BM8000_S1", CommandType.StoredProcedure
        ...
        public override void DoNew()
        {
            try
            {
                ...
                this.grid1.ActiveRow.Cells["ULLAGECODE"].Value = Common.getLangText("자동채번", "TEXT");

                //사용자 입력이 필요하지 않은 부분은 행 추가시 수정이 안되도록 조치
                grid1.ActiveRow.Cells["ULLAGECODE"].Activation = Activation.NoEdit;
                ...
                foreach (DataRow drRow in rtnDtTemp.Rows)
                {
                    switch (drRow.RowState)
                    {
                        ...
                        case DataRowState.Added:
                            #region 추가                            
                            helper.ExecuteNoneQuery("USP_BM8000_I1"
                                                    , CommandType.StoredProcedure
                                                    , helper.CreateParameter("AS_PLANTCODE", DBHelper.nvlString(drRow["PLANTCODE"]), DbType.String, ParameterDirection.Input)
                                                    , helper.CreateParameter("AS_ULLAGECODE", DBHelper.nvlString(drRow["ULLAGECODE"]), DbType.String, ParameterDirection.Input)
                                                    , helper.CreateParameter("AS_ULLAGENAME", DBHelper.nvlString(drRow["ULLAGENAME"]), DbType.String, ParameterDirection.Input)
                                                    , helper.CreateParameter("AS_ULLAGETYPE", DBHelper.nvlString(drRow["ULLAGETYPE"]), DbType.String, ParameterDirection.Input)
                        ...
                        case DataRowState.Modified:
                            #region 수정
                            helper.ExecuteNoneQuery("USP_BM8000_U1"
                                                    , CommandType.StoredProcedure
                                                    , helper.CreateParameter("AS_PLANTCODE", DBHelper.nvlString(drRow["PLANTCODE"]), DbType.String, ParameterDirection.Input)
                                                    , helper.CreateParameter("AS_ULLAGECODE", DBHelper.nvlString(drRow["ULLAGECODE"]), DbType.String, ParameterDirection.Input)
                                                    , helper.CreateParameter("AS_ULLAGENAME", DBHelper.nvlString(drRow["ULLAGENAME"]), DbType.String, ParameterDirection.Input)
