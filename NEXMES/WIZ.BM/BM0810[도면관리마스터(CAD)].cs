...
namespace WIZ.BM
{
    public partial class BM0810 : WIZ.Forms.BaseMDIChildForm
    {
        #region < MEMBER AREA >
        ...
        #region < FORM LOAD >
        private void BM0810_Load(object sender, EventArgs e)
        {
            try
            {
                #region GRID SETTING
                ...
                _GridUtil.InitColumnUltraGrid(grid2, "DRAWID", "도면코드", false, GridColDataType_emu.VarChar, 120, 200, HAlign.Left, true, false, null, null, null, null, null);
                _GridUtil.InitColumnUltraGrid(grid2, "REMARK", "비고", false, GridColDataType_emu.VarChar, 310, 180, HAlign.Left, true, true, null, null, null, null, null);
        ...
        public override void DoDelete()
        {
            ...
            helper.ExecuteNoneQuery("USP_BM0810_D1"
                ...
                foreach (DataRow drRow in rtnDtTemp.Rows)
                {
                    ...
                    switch (drRow.RowState)
                    {
                        ...
                        case DataRowState.Added:
                            #region 추가                            
                            helper.ExecuteNoneQuery("USP_BM0810_I1"
                                                    , CommandType.StoredProcedure
                        ...
                        case DataRowState.Modified:
                            #region 수정
                            helper.ExecuteNoneQuery("USP_BM0810_U1"
                                                     , CommandType.StoredProcedure
                                                    , helper.CreateParameter("AS_PLANTCODE", DbType.String, DBHelper.nvlString(drRow["PLANTCODE"]))
                                                    , helper.CreateParameter("AS_DRAWID", DbType.String, DBHelper.nvlString(drRow["DRAWID"]))
                                                    , helper.CreateParameter("AS_ITEMCODE", DbType.String, DBHelper.nvlString(drRow["ITEMCODE"]))
        ...
        private void grid1_ClickCell(object sender, ClickCellEventArgs e)
        {
            ...
            try
            {
                ...
                {
                    rtnDtTemp = helper.FillTable("USP_BM0810_S2", CommandType.StoredProcedure
                                                            , helper.CreateParameter("AS_PLANTCODE", DbType.String, sPlantCode)
                                                            , helper.CreateParameter("AS_ITEMCODE", DbType.String, sItemCode));
                    if (rtnDtTemp.Rows.Count > 0)
                    {
        ...
        private void grid2_DoubleClickCell(object sender, DoubleClickCellEventArgs e)
        {
            try
            {
                ...
                //this.InitateDirectory(@"C:\DRAW");
                byte[] fileimage = new byte[1024];
                fileimage = (byte[])e.Cell.Row.Cells["FILE"].Value;
                string fileName = e.Cell.Row.Cells["FILE"].Value.ToString();
                string Dwid = e.Cell.Row.Cells["DRAWID"].Value.ToString();

                // 더블클릭시 파일 보기 위한 임시파일 명(현재일시로 계속 생기게 함)
                //string fileName = Application.StartupPath + @"\DRAW\temp.pdf";
                //fileName = Application.StartupPath + @"\DRAW\" + DateTime.Now.ToString("yyyy-MM-dd hhmmss") + " _ " + fileName + @".dwg";
                //fileName = Application.StartupPath + @"\DRAW\" + DateTime.Now.ToString("yyyy-MM-dd hhmmss") + " _ " + Dwid + @".dwg";
                fileName = @"C:\DRAW\" + DateTime.Now.ToString("yyyy-MM-dd hhmmss") + " _ " + Dwid + @".dwg";
                ...
                //MemoryStream ms = new MemoryStream(fileimage, 0, fileimage.Length);
                //BinaryReader br = new BinaryReader(ms);
                fs.Write(fileimage, 0, fileimage.Length);
                fs.Close();
                //br.Read(fileimage, 0, fileimage.Length);
                

                pcSys.DoIExplore_SiteShow(fileName, true);
                //ms.Close();
            ...
            //    fileimage = (byte[])e.Cell.Row.Cells["FileData"].Value;
            //    string fileName = e.Cell.Row.Cells["FileName"].Value.ToString();   //2021-01-14 add

            //    // 더블클릭시 파일 보기 위한 임시파일 명(현재일시로 계속 생기게 함)
            //    //string fileName = Application.StartupPath + @"\DRAW\temp.pdf";
            //    fileName = Application.StartupPath + @"\DRAW\" + DateTime.Now.ToString("yyyy-MM-dd hhmmss") + " _ " + fileName;// + ".pdf";
        ...
        /*private byte[] GetImage(string sPlantCode, string sItemCode, string sDrawId)
        {
            ...
            try
            {
                rtnDtTemp = helper.FillTable("USP_BM0810_S3", CommandType.StoredProcedure
