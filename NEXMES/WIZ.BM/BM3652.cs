...
namespace WIZ.BM
{
    public partial class BM3652 : WIZ.Forms.BaseForm
    {
        string PlantCode;
        string ItemCode;
        string InspCode;
        ...
        public BM3652(string sPlantCode, string sItemCode, string sInspCode)
        {
            InitializeComponent();
        ...
        private void BM3652_Load(object sender, EventArgs e)
        {
        ...
        private void search()
        {
            ...
            System.Data.Common.DbParameter[] param = new System.Data.Common.DbParameter[3];
            try
            {
                DataSet ds;

                string sPlantCode = PlantCode;  // 공장코드
                string sItemCode = ItemCode;      // 품목
                string sInspCode = InspCode;      // 검사항목

                param[0] = helper.CreateParameter("@pPlantCode", sPlantCode, DbType.String, ParameterDirection.Input);
                param[1] = helper.CreateParameter("@pItemCode", sItemCode, DbType.String, ParameterDirection.Input);
                param[2] = helper.CreateParameter("@pInspCode", sInspCode, DbType.String, ParameterDirection.Input);

                ds = helper.FillDataSet("USP_BM3652_S1", CommandType.StoredProcedure, param);
                ...
                for (int i = 0; i < ds.Tables.Count; i++)
                {
                    switch (i)
                    {
                        case 0:
                            ...
                            txtInspCode.Text = InspCode;
                            txtItemName.Text = DBHelper.nvlString(ds.Tables[i].Rows[0]["ItemName"]);
        ...
        private void DoSave()
        {
            ...
            try
            {
                ...
                param[1] = helper.CreateParameter("@pItemCode", sItemCode, DbType.String, ParameterDirection.Input);
                param[2] = helper.CreateParameter("@pInspCode", sInspCode, DbType.String, ParameterDirection.Input);
                param[3] = helper.CreateParameter("@pValue", sArrValue, DbType.String, ParameterDirection.Input);
                param[4] = helper.CreateParameter("@pMaker", this.WorkerID, DbType.String, ParameterDirection.Input);
                param[5] = helper.CreateParameter("@pSeq1", "", DbType.String, ParameterDirection.Input);
                param[6] = helper.CreateParameter("@pSeq2", "", DbType.String, ParameterDirection.Input);
                param[7] = helper.CreateParameter("@pSeq3", "", DbType.String, ParameterDirection.Input);

                param[8] = helper.CreateParameter("@pRetValue", DbType.Int32, ParameterDirection.Output, null, 200);

                helper.ExecuteNoneQuery("USP_BM3652_U1", CommandType.StoredProcedure, param);

                iCount = DBHelper.nvlInt(param[8].Value);
