...
namespace WIZ.BM
{
    public partial class BM0710 : WIZ.Forms.BaseMDIChildForm
    {
        #region < MEMBER AREA >
        ...
        #region < FORM LOAD >
        private void BM0710_Load(object sender, EventArgs e)
        {
            try
            {
                #region GRID SETTING
                ...
                _GridUtil.InitColumnUltraGrid(grid1, "MACODE", "설비보전", true, GridColDataType_emu.VarChar, 90, 90, Infragistics.Win.HAlign.Center, true, false);
                _GridUtil.InitColumnUltraGrid(grid1, "MANAME", "설비보전명", true, GridColDataType_emu.VarChar, 150, 100, Infragistics.Win.HAlign.Left, true, true);
                ...
                grid1.DisplayLayout.Bands[0].Columns["MACODE"].Header.Appearance.ForeColor = Color.SkyBlue;
                grid1.DisplayLayout.Bands[0].Columns["MANAME"].Header.Appearance.ForeColor = Color.SkyBlue;
                ...
                //설비보전
                btbManager.PopUpAdd(txt_MACODE_H, txt_MANAME_H, "BM0710", new object[] { cbo_PLANTCODE_H, "", "" });
                #endregion
            }
            catch (Exception ex)
            {
        ...
        public override void DoInquire()
        {
            ...
            try
            {
                ...
                string sMaCode = DBHelper.nvlString(txt_MACODE_H.Text.Trim());
                string sMaName = DBHelper.nvlString(txt_MANAME_H.Value);
                string sUseFlag = DBHelper.nvlString(cbo_USEFLAG_H.Value);

                rtnDtTemp = helper.FillTable("USP_BM0710_S1", CommandType.StoredProcedure
        ...
        public override void DoNew()
        {
            try
            {
                ...
                this.grid1.ActiveRow.Cells["MACODE"].Value = Common.getLangText("자동채번", "TEXT");

                //사용자 입력이 필요하지 않은 부분은 행 추가시 수정이 안되도록 조치
                grid1.ActiveRow.Cells["MACODE"].Activation = Activation.NoEdit;
            ...
            try
            {
                ...
                foreach (DataRow drRow in rtnDtTemp.Rows)
                {
                    ...
                    if (drRow.RowState != DataRowState.Deleted)
                    {
                        ...
                        if (Convert.ToString(drRow["MANAME"]) == string.Empty)
                        {
                            this.ClosePrgFormNew();
                            this.ShowDialog(Common.getLangText("사업장은 필수 입력항목입니다.", "MSG"), Forms.DialogForm.DialogType.OK);
                            return;
                    ...
                    switch (drRow.RowState)
                    {
                        ...
                        case DataRowState.Added:
                            #region 추가                            
                            helper.ExecuteNoneQuery("USP_BM0710_I1"
                                                    , CommandType.StoredProcedure
                                                    , helper.CreateParameter("AS_PLANTCODE", DBHelper.nvlString(drRow["PLANTCODE"]), DbType.String, ParameterDirection.Input)
                                                    , helper.CreateParameter("AS_MANAME", DBHelper.nvlString(drRow["MANAME"]), DbType.String, ParameterDirection.Input)
                        ...
                        case DataRowState.Modified:
                            #region 수정
                            helper.ExecuteNoneQuery("USP_BM0710_U1"
                                                     , CommandType.StoredProcedure
                                                     , helper.CreateParameter("AS_PLANTCODE", DBHelper.nvlString(drRow["PLANTCODE"]), DbType.String, ParameterDirection.Input)
                                                     , helper.CreateParameter("AS_MACODE", DBHelper.nvlString(drRow["MACODE"]), DbType.String, ParameterDirection.Input)
                                                     , helper.CreateParameter("AS_MANAME", DBHelper.nvlString(drRow["MANAME"]), DbType.String, ParameterDirection.Input)
                                                     , helper.CreateParameter("AS_USEFLAG", DBHelper.nvlString(drRow["USEFLAG"]), DbType.String, ParameterDirection.Input)
                                                     , helper.CreateParameter("AS_REMARK", DBHelper.nvlString(drRow["REMARK"]), DbType.String, ParameterDirection.Input)
        ...
        public override void DoImportExcel()
        {
            ...
            BM0710_EXCEL bm0710_excel = new BM0710_EXCEL();
            bm0710_excel.ShowDialog();
