...
namespace WIZ.PopUp
{
    public partial class BM0110_EXCEL : WIZ.Forms.BasePopupForm
    {
        #region [ 선언자 ]
        ...
        #region [ Form Load ]
        private void BM0110_EXCEL_Load(object sender, EventArgs e)
        {
            ...
            _GridUtil.InitColumnUltraGrid(grid1, "STOPDESC", "비가동명", true, GridColDataType_emu.VarChar, 120, 100, Infragistics.Win.HAlign.Left, true, false);
            _GridUtil.InitColumnUltraGrid(grid1, "STOPTYPE", "비가동구분", true, GridColDataType_emu.VarChar, 140, 90, Infragistics.Win.HAlign.Left, true, false);
            _GridUtil.InitColumnUltraGrid(grid1, "STOPCLASS", "비가동유형", true, GridColDataType_emu.VarChar, 120, 100, Infragistics.Win.HAlign.Left, true, false);
            _GridUtil.InitColumnUltraGrid(grid1, "STOPMH", "인당공수집계구분", true, GridColDataType_emu.VarChar, 120, 100, Infragistics.Win.HAlign.Left, true, false);
            _GridUtil.InitColumnUltraGrid(grid1, "STOPMCH", "설비공수집계구분", true, GridColDataType_emu.VarChar, 120, 100, Infragistics.Win.HAlign.Left, true, false);
            _GridUtil.InitColumnUltraGrid(grid1, "STOPCL", "프로세스집계구분", true, GridColDataType_emu.VarChar, 120, 100, Infragistics.Win.HAlign.Left, true, false);
            _GridUtil.InitColumnUltraGrid(grid1, "REPORTFLAG", "레포트 집계유무", true, GridColDataType_emu.VarChar, 120, 100, Infragistics.Win.HAlign.Left, true, false);
            _GridUtil.InitColumnUltraGrid(grid1, "VIEWFLAG", "현장단말기\r\n표시유무", true, GridColDataType_emu.VarChar, 90, 100, Infragistics.Win.HAlign.Left, true, false);
        ...
        private void INSERT_SAVE()
        {
            ...
            string sStopClass = string.Empty;
            string sStopMh = string.Empty;
            string sStopMch = string.Empty;
            string sStopCl = string.Empty;
            string sReportFlag = string.Empty;
            string sViewFlag = string.Empty;
            ...
            try
            {
                for (int i = 0; i < grid1.Rows.Count; i++)
                {
                    ...
                    sStopType = Convert.ToString(grid1.Rows[i].Cells["STOPTYPE"].Value);
                    sStopDesc = Convert.ToString(grid1.Rows[i].Cells["STOPDESC"].Value);
                    sStopClass = Convert.ToString(grid1.Rows[i].Cells["STOPCLASS"].Value);
                    sStopMh = Convert.ToString(grid1.Rows[i].Cells["STOPMH"].Value);
                    sStopMch = Convert.ToString(grid1.Rows[i].Cells["STOPMCH"].Value);
                    sStopCl = Convert.ToString(grid1.Rows[i].Cells["STOPCL"].Value);
                    sReportFlag = Convert.ToString(grid1.Rows[i].Cells["REPORTFLAG"].Value);
                    sViewFlag = Convert.ToString(grid1.Rows[i].Cells["VIEWFLAG"].Value);
                ...
                DataTable excelUpType = helper_Excel.FillTable("USP_EXCELUPLOADTYPE_S1", CommandType.StoredProcedure
                                                         , helper.CreateParameter("AS_FORMID", "BM0110", DbType.String, ParameterDirection.Input));
                ...
                if (excelUploadType == "R")
                {
                    ...
                    for (int i = 0; i < grid1.Rows.Count; i++)
                    {
                        helper.ExecuteNoneQuery("USP_BM0110_EXCEL_I1", CommandType.StoredProcedure
                                               ...
                                               , helper.CreateParameter("AS_STOPTYPE", Convert.ToString(grid1.Rows[i].Cells["STOPTYPE"].Value), DbType.String, ParameterDirection.Input)
                                               , helper.CreateParameter("AS_STOPDESC", Convert.ToString(grid1.Rows[i].Cells["STOPDESC"].Value), DbType.String, ParameterDirection.Input)
                                               , helper.CreateParameter("AS_STOPCLASS", Convert.ToString(grid1.Rows[i].Cells["STOPCLASS"].Value), DbType.String, ParameterDirection.Input)
                                               , helper.CreateParameter("AS_STOPMH", Convert.ToString(grid1.Rows[i].Cells["STOPMH"].Value), DbType.String, ParameterDirection.Input)
                                               , helper.CreateParameter("AS_STOPMCH", Convert.ToString(grid1.Rows[i].Cells["STOPMCH"].Value), DbType.String, ParameterDirection.Input)
                                               , helper.CreateParameter("AS_STOPCL", Convert.ToString(grid1.Rows[i].Cells["STOPCL"].Value), DbType.String, ParameterDirection.Input)
                                               , helper.CreateParameter("AS_REPORTFLAG", Convert.ToString(grid1.Rows[i].Cells["REPORTFLAG"].Value), DbType.String, ParameterDirection.Input)
                                               , helper.CreateParameter("AS_VIEWFLAG", Convert.ToString(grid1.Rows[i].Cells["VIEWFLAG"].Value), DbType.String, ParameterDirection.Input)
            ...
            try
            {
                ...
                if (openfiledialog.ShowDialog() == System.Windows.Forms.DialogResult.OK)
                {
                    ...
                    for (int i = 1; i < dtExcelImport.Rows.Count; i++)
                    {
                        ...
                        drTarget["STOPTYPE"] = Convert.ToString(dtExcelImport.Rows[i][1]);
                        drTarget["STOPDESC"] = Convert.ToString(dtExcelImport.Rows[i][2]);
                        drTarget["STOPCLASS"] = Convert.ToString(dtExcelImport.Rows[i][3]);
                        drTarget["STOPMH"] = Convert.ToString(dtExcelImport.Rows[i][4]);
                        drTarget["STOPMCH"] = Convert.ToString(dtExcelImport.Rows[i][5]);
                        drTarget["STOPCL"] = Convert.ToString(dtExcelImport.Rows[i][6]);
                        drTarget["REPORTFLAG"] = Convert.ToString(dtExcelImport.Rows[i][7]);
                        drTarget["VIEWFLAG"] = Convert.ToString(dtExcelImport.Rows[i][8]);
                        drTarget["USEFLAG"] = Convert.ToString(dtExcelImport.Rows[i][9]).Trim();
                        drTarget["REMARK"] = Convert.ToString(dtExcelImport.Rows[i][10]);
