...
namespace WIZ.PopUp
{
    public partial class BM0100_EXCEL : WIZ.Forms.BasePopupForm
    {
        #region [ 선언자 ]
        ...
        #region [ Form Load ]
        private void BM0100_EXCEL_Load(object sender, EventArgs e)
        {
            #region Grid 셋팅
            //GRID1 품목
            ...
            _GridUtil.InitColumnUltraGrid(grid1, "ERRORTYPE", "불량구분", true, GridColDataType_emu.VarChar, 100, 100, Infragistics.Win.HAlign.Left, true, false);
            _GridUtil.InitColumnUltraGrid(grid1, "ERRORCLASS", "불량유형", true, GridColDataType_emu.VarChar, 80, 100, Infragistics.Win.HAlign.Left, true, false);
        ...
        private void INSERT_SAVE()
        {
            ...
            string sErrorDecs = string.Empty;
            string sErrorType = string.Empty;
            string sErrorClass = string.Empty;
            ...
            try
            {
                for (int i = 0; i < grid1.Rows.Count; i++)
                {
                    ...
                    sErrorDecs = Convert.ToString(grid1.Rows[i].Cells["ERRORDESC"].Value);
                    sErrorType = Convert.ToString(grid1.Rows[i].Cells["ERRORTYPE"].Value);
                    sErrorClass = Convert.ToString(grid1.Rows[i].Cells["ERRORCLASS"].Value);
                    ...
                    if (sErrorType == string.Empty)
                    {
                        MessageBox.Show(Common.getLangText(Convert.ToString(i + 1) + "번째 열의 불량구분을 확인하세요.", "MSG"));
                        return;
                    }
                }

                DataTable excelUpType = helper_Excel.FillTable("USP_EXCELUPLOADTYPE_S1", CommandType.StoredProcedure
                                                         , helper.CreateParameter("AS_FORMID", "BM0100", DbType.String, ParameterDirection.Input));
                ...
                if (excelUploadType == "R")
                {
                    ...
                    for (int i = 0; i < grid1.Rows.Count; i++)
                    {
                        helper.ExecuteNoneQuery("USP_BM0100_I1"
                                                    ...
                                                    , helper.CreateParameter("AS_ERRORCLASS", Convert.ToString(grid1.Rows[i].Cells["ERRORCLASS"].Value), DbType.String, ParameterDirection.Input)
            ...
            try
            {
                ...
                if (openfiledialog.ShowDialog() == System.Windows.Forms.DialogResult.OK)
                {
                    ...
                    for (int i = 1; i < dtExcelImport.Rows.Count; i++)
                    {
                        ...
                        drTarget["ERRORDESC"] = Convert.ToString(dtExcelImport.Rows[i][1]);
                        drTarget["ERRORTYPE"] = Convert.ToString(dtExcelImport.Rows[i][2]);
                        drTarget["ERRORCLASS"] = Convert.ToString(dtExcelImport.Rows[i][3]);
