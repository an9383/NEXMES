...
namespace WIZ.PopUp
{
    public partial class BM0520_EXCEL : WIZ.Forms.BasePopupForm
    {
        #region [ 선언자 ]
        ...
        #region [ Form Load ]
        private void BM0520_EXCEL_Load(object sender, EventArgs e)
        {
            #region Grid 셋팅
            //GRID1 품목
            ...
            _GridUtil.InitColumnUltraGrid(grid1, "MESSAGEDESC", "메시지 내용", true, GridColDataType_emu.VarChar, 250, 130, Infragistics.Win.HAlign.Left, true, false);
            _GridUtil.InitColumnUltraGrid(grid1, "MESSAGETYPE", "메시지 유형", true, GridColDataType_emu.VarChar, 130, 250, Infragistics.Win.HAlign.Left, true, false);
        ...
        private void INSERT_SAVE()
        {
            ...
            string sMessageType = string.Empty;
            string sMessageDesc = string.Empty;
            ...
            try
            {
                for (int i = 0; i < grid1.Rows.Count; i++)
                {
                    ...
                    sMessageType = Convert.ToString(grid1.Rows[i].Cells["MESSAGETYPE"].Value);
                    sMessageDesc = Convert.ToString(grid1.Rows[i].Cells["MESSAGEDESC"].Value);
                    ...
                    if (sMessageType == string.Empty)
                    {
                        MessageBox.Show(Common.getLangText(Convert.ToString(i + 1) + "번째 열의 메시지 유형을 확인하세요.", "MSG"));
                        return;
                    }
                    if (sMessageDesc == string.Empty)
                    {
                        MessageBox.Show(Common.getLangText(Convert.ToString(i + 1) + "번째 열의 메시지 내용을 확인하세요.", "MSG"));
                        return;
                    }
                ...
                DataTable excelUpType = helper_Excel.FillTable("USP_EXCELUPLOADTYPE_S1", CommandType.StoredProcedure
                                                         , helper.CreateParameter("AS_FORMID", "BM0520", DbType.String, ParameterDirection.Input));
                ...
                if (excelUploadType == "R")
                {
                    ...
                    for (int i = 0; i < grid1.Rows.Count; i++)
                    {
                        helper.ExecuteNoneQuery("USP_BM0520_I1"
                                                    ...
                                                    , helper.CreateParameter("AS_MSGDESC", Convert.ToString(grid1.Rows[i].Cells["MESSAGEDESC"].Value), DbType.String, ParameterDirection.Input)
                                                    , helper.CreateParameter("AS_MSGTYPE", Convert.ToString(grid1.Rows[i].Cells["MESSAGETYPE"].Value), DbType.String, ParameterDirection.Input)
            ...
            try
            {
                ...
                if (openfiledialog.ShowDialog() == System.Windows.Forms.DialogResult.OK)
                {
                    ...
                    for (int i = 1; i < dtExcelImport.Rows.Count; i++)
                    {
                        ...
                        drTarget["MESSAGEDESC"] = Convert.ToString(dtExcelImport.Rows[i][1]);
                        drTarget["MESSAGETYPE"] = Convert.ToString(dtExcelImport.Rows[i][2]);
