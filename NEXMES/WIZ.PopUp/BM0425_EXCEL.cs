...
namespace WIZ.PopUp
{
    public partial class BM0425_EXCEL : WIZ.Forms.BasePopupForm
    {
        #region [ 선언자 ]
        ...
        #region [ Form Load ]
        private void BM0425_EXCEL_Load(object sender, EventArgs e)
        {
            #region Grid 셋팅
            //GRID1 품목
        ...
        private void INSERT_SAVE()
        {
            ...
            try
            {
                ...
                DataTable excelUpType = helper_Excel.FillTable("USP_EXCELUPLOADTYPE_S1", CommandType.StoredProcedure
                                                         , helper.CreateParameter("AS_FORMID", "BM0425", DbType.String, ParameterDirection.Input));
            ...
            try
            {
                ...
                if (openfiledialog.ShowDialog() == System.Windows.Forms.DialogResult.OK)
                {
                    ...
                    for (int i = 1; i < dtExcelImport.Rows.Count; i++)
                    {
                        ...
                        drTarget["INSPCODE"] = Convert.ToString(dtExcelImport.Rows[i][5]);
                        drTarget["INSPNAME"] = Convert.ToString(dtExcelImport.Rows[i][6]);
                        drTarget["VALUETYPE"] = Convert.ToString(dtExcelImport.Rows[i][7]);
                        drTarget["SPECTYPE"] = Convert.ToString(dtExcelImport.Rows[i][8]);
                        drTarget["UNITCODE"] = Convert.ToString(dtExcelImport.Rows[i][9]);
                        drTarget["SPECNOL"] = Convert.ToString(dtExcelImport.Rows[i][10]);
                        drTarget["SPECLSL"] = Convert.ToString(dtExcelImport.Rows[i][11]);
                        drTarget["SPECUSL"] = Convert.ToString(dtExcelImport.Rows[i][12]);
