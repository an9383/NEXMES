...
namespace WIZ.PopUp
{
    public partial class POP_TBM0300_EXCEL : WIZ.Forms.BasePopupForm
    {
        #region [ 선언자 ]
        ...
        #region [ Form Load ]
        private void POP_TBM0300_EXCEL_Load(object sender, EventArgs e)
        {
            #region Grid 셋팅
            ...
            _GridUtil.InitColumnUltraGrid(grid1, "CUSTTYPE", "거래처구분ⓒ", true, GridColDataType_emu.VarChar, 100, 120, Infragistics.Win.HAlign.Center, true, false, null, null, null, null, null);
            _GridUtil.InitColumnUltraGrid(grid1, "CUSTCODE", "거래처코드", true, GridColDataType_emu.VarChar, 100, 120, Infragistics.Win.HAlign.Center, true, true, null, null, null, null, null);
            _GridUtil.InitColumnUltraGrid(grid1, "CUSTNAME", "거래처명", true, GridColDataType_emu.VarChar, 300, 140, Infragistics.Win.HAlign.Left, true, true, null, null, null, null, null);
            _GridUtil.InitColumnUltraGrid(grid1, "BIZREQNO", "사업자등록번호", true, GridColDataType_emu.VarChar, 140, 140, Infragistics.Win.HAlign.Left, true, true, null, null, null, null, null);
            _GridUtil.InitColumnUltraGrid(grid1, "PHONE", "전화번호", true, GridColDataType_emu.VarChar, 120, 140, Infragistics.Win.HAlign.Left, true, true, null, null, null, null, null);
            _GridUtil.InitColumnUltraGrid(grid1, "ADDRESS", "주소", true, GridColDataType_emu.VarChar, 400, 140, Infragistics.Win.HAlign.Left, true, true, null, null, null, null, null);
        ...
        private void INSERT_SAVE()
        {
            ...
            try
            {
                for (int i = 0; i < grid1.Rows.Count; i++)
                {
                    ...
                    CUSTTYPE = Convert.ToString(grid1.Rows[i].Cells["CUSTTYPE"].Value);
                    CUSTCODE = Convert.ToString(grid1.Rows[i].Cells["CUSTCODE"].Value);
                    CUSTNAME = Convert.ToString(grid1.Rows[i].Cells["CUSTNAME"].Value);
                    BIZREQNO = Convert.ToString(grid1.Rows[i].Cells["BIZREQNO"].Value);
                    PHONE = Convert.ToString(grid1.Rows[i].Cells["PHONE"].Value);
                    ADDRESS = Convert.ToString(grid1.Rows[i].Cells["ADDRESS"].Value);
                    ...
                    {
                        MessageBox.Show(Convert.ToString(i + 1) + "번째 열의 거래처 구분을 확인하세요.");
                        return;
                    }
                    if (CUSTCODE == string.Empty)
                    {
                        MessageBox.Show(Convert.ToString(i + 1) + "번째 열의 거래처 코드를 확인하세요.");
                        return;
                    }
                ...
                if (excelUploadType == "R")
                {
                    ...
                    for (int i = 0; i < grid1.Rows.Count; i++)
                    {
                        helper.ExecuteNoneQuery("USP_POP_BM0300_I1"
            ...
            try
            {
                ...
                if (openfiledialog.ShowDialog() == System.Windows.Forms.DialogResult.OK)
                {
                    ...
                    for (int i = 1; i < dtExcelImport.Rows.Count; i++)
                    {
                        ...
                        drTarget["CUSTTYPE"] = Convert.ToString(dtExcelImport.Rows[i][1]);
                        drTarget["CUSTCODE"] = Convert.ToString(dtExcelImport.Rows[i][2]);
                        drTarget["CUSTNAME"] = Convert.ToString(dtExcelImport.Rows[i][3]);
                        drTarget["BIZREQNO"] = Convert.ToString(dtExcelImport.Rows[i][4]);
                        drTarget["PHONE"] = Convert.ToString(dtExcelImport.Rows[i][5]);
                        drTarget["ADDRESS"] = Convert.ToString(dtExcelImport.Rows[i][6]);
