...
namespace WIZ.PopUp
{
    public partial class POP_TBM6100_EXCEL : WIZ.Forms.BasePopupForm
    {
        #region [ 선언자 ]
        ...
        #region [ Form Load ]
        private void POP_TBM6100_EXCEL_Load(object sender, EventArgs e)
        {
            #region Grid 셋팅
            ...
            _GridUtil.InitColumnUltraGrid(grid1, "MESRCODE", "측정항목", true, GridColDataType_emu.VarChar, 120, 100, Infragistics.Win.HAlign.Center, true, false, null, null, null, null, null);
            _GridUtil.InitColumnUltraGrid(grid1, "MESRNAME", "측정항목명", true, GridColDataType_emu.VarChar, 180, 100, Infragistics.Win.HAlign.Left, true, false, null, null, null, null, null);
        ...
        private void INSERT_SAVE()
        {
            ...
            string MESRCODE = string.Empty;
            string MESRNAME = string.Empty;
            ...
            try
            {
                for (int i = 0; i < grid1.Rows.Count; i++)
                {
                    ...
                    MESRCODE = Convert.ToString(grid1.Rows[i].Cells["MESRCODE"].Value);
                    MESRNAME = Convert.ToString(grid1.Rows[i].Cells["MESRNAME"].Value);
                    ...
                    if (MESRNAME == string.Empty)
                    {
                        MessageBox.Show(Convert.ToString(i + 1) + "번째 열의 측정항목명을 확인하세요.");
                        return;
                    }
                ...
                if (excelUploadType == "R")
                {
                    ...
                    for (int i = 0; i < grid1.Rows.Count; i++)
                    {
                        helper.ExecuteNoneQuery("USP_POP_BM6100_I1"
                                          , CommandType.StoredProcedure
                                               ...
                                               , helper.CreateParameter("AS_MESRCODE", Convert.ToString(grid1.Rows[i].Cells["MESRCODE"].Value), DbType.String, ParameterDirection.Input)
                                               , helper.CreateParameter("AS_MESRNAME", Convert.ToString(grid1.Rows[i].Cells["MESRNAME"].Value), DbType.String, ParameterDirection.Input)
            ...
            try
            {
                ...
                if (openfiledialog.ShowDialog() == System.Windows.Forms.DialogResult.OK)
                {
                    ...
                    for (int i = 1; i < dtExcelImport.Rows.Count; i++)
                    {
                        ...
                        drTarget["MESRCODE"] = Convert.ToString(dtExcelImport.Rows[i][1]);
                        drTarget["MESRNAME"] = Convert.ToString(dtExcelImport.Rows[i][2]);
                        drTarget["USEFLAG"] = Convert.ToString(dtExcelImport.Rows[i][3]);
