...
namespace WIZ.PopUp
{
    public partial class AP0126_POP : WIZ.Forms.BasePopupForm
    {
        #region < MEMBER AREA >
        private string sPlantCode = string.Empty;                   //사업장
        private string sContractNo = string.Empty;                   //수주번호(CONTRACTNO)     
        private string sShipNo2 = string.Empty;                   //출하번호(SHIPNO)  
        private string sItemCode = string.Empty;                   //품목
        private string sItemName = string.Empty;                   //품목명
        private string sPackQty = string.Empty;                   //포장수량

        //private string sSeq             = string.Empty;                   //거래순번(POSEQ)     
        ...
        public AP0126_POP(DataRow drRow, string sShipNo)
        {
            InitializeComponent();

            //sPlantCode     = Convert.ToString(drRow["PLANTCODE"]);
            sContractNo = Convert.ToString(drRow["CONTRACTNO"]);
            sShipNo2 = Convert.ToString(drRow["SHIPNO"]);
            sItemCode = Convert.ToString(drRow["ITEMCODE"]);
            //sItemName = Convert.ToString(drRow["ITEMNAME"]);
            //sSeq       = Convert.ToString(drRow["SEQ"]);
            ...
            txt_PACKQTY_B.Text = sPackQty;         //발주수량(A)
        ...
        #region < FORM EVENT >
        private void AP0126_POP_Load(object sender, EventArgs e)
        {
            #region < GRID SETTING >
        ...
        {
            int iPoQty = Convert.ToInt32(txt_PACKQTY_B.Text);            //발주수량
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    MessageBox.Show(Common.getLangText("삭제사유를 입력하세요.", "MSG"));
                    return;
                }
                ...
                helper.ExecuteNoneQuery("USP_SA0000_D1"
                                       , CommandType.StoredProcedure
                                        ...
                                        , helper.CreateParameter("AS_SHIPNO", sShipNo2, DbType.String, ParameterDirection.Input)
                                        , helper.CreateParameter("AS_ITEMCODE", sItemCode, DbType.String, ParameterDirection.Input)
        ...
        private void btnProc_Click(object sender, EventArgs e)
        {
            ...
            if (btn != null)
            {
                ...
                while (true)
                {
                    ...
                    //{
                    //    grid1.ActiveRow.Cells["CUSTLOTNO"].Value = txtRemark.Text.Trim();
                    //}
