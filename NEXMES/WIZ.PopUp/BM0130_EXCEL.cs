...
namespace WIZ.PopUp
{
    public partial class BM0130_EXCEL : WIZ.Forms.BasePopupForm
    {
        #region [ 선언자 ]
        ...
        #region [ Form Load ]
        private void BM0130_EXCEL_Load(object sender, EventArgs e)
        {
            #region Grid 셋팅
            //GRID1 품목

            _GridUtil.InitializeGrid(this.grid1, false, true, false, "", false);
            _GridUtil.InitColumnUltraGrid(grid1, "UNITCODE", "단위코드", true, GridColDataType_emu.VarChar, 80, 10, Infragistics.Win.HAlign.Left, true, false);
            _GridUtil.InitColumnUltraGrid(grid1, "UNITNAME", "단위이름", true, GridColDataType_emu.VarChar, 80, 50, Infragistics.Win.HAlign.Left, true, false);
            _GridUtil.InitColumnUltraGrid(grid1, "DISPLAYNO", "표시순번", true, GridColDataType_emu.Integer, 50, 100, Infragistics.Win.HAlign.Right, true, false);
        ...
        private void INSERT_SAVE()
        {
            ...
            try
            {
                for (int i = 0; i < grid1.Rows.Count; i++)
                {
                    ...
                    sUnitName = Convert.ToString(grid1.Rows[i].Cells["UNITNAME"].Value);
                ...
                DataTable excelUpType = helper_Excel.FillTable("USP_EXCELUPLOADTYPE_S1", CommandType.StoredProcedure
                                                         , helper.CreateParameter("AS_FORMID", "BM0130", DbType.String, ParameterDirection.Input));
                ...
                if (excelUploadType == "R")
                {
                    ...
                    for (int i = 0; i < grid1.Rows.Count; i++)
                    {
                        helper.ExecuteNoneQuery("USP_BM0130_I1"
                                                    ...
                                                    , helper.CreateParameter("AS_UNITNAME", Convert.ToString(grid1.Rows[i].Cells["UNITNAME"].Value), DbType.String, ParameterDirection.Input)
            ...
            try
            {
                ...
                if (openfiledialog.ShowDialog() == System.Windows.Forms.DialogResult.OK)
                {
                    ...
                    for (int i = 1; i < dtExcelImport.Rows.Count; i++)
                    {
                        ...
                        drTarget["UNITCODE"] = Convert.ToString(dtExcelImport.Rows[i][0]);
                        drTarget["UNITNAME"] = Convert.ToString(dtExcelImport.Rows[i][1]);
                        drTarget["DISPLAYNO"] = Convert.ToString(dtExcelImport.Rows[i][2]);
