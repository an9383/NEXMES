...
namespace WIZ.PopUp
{
    public partial class BM0080_POP : WIZ.Forms.BasePopupForm
    {
        #region < MEMBER AREA >
        ...
        public BM0080_POP(string[] param)
        {
            InitializeComponent();
            ...
            for (int i = 0; i < param.Length; i++)
            {
                ...
                switch (i)
                {
                    ...
                    case 1:
                        cbo_WHTYPE_H.Text = argument[1].ToUpper() == "" ? "ALL" : argument[1].ToUpper(); //창고유형
                        break;

                    case 2:
                        txt_WHCODE_H.Text = argument[2].ToUpper(); //설비코드
                        break;

                    case 3:
                        txt_WHNAME_H.Text = argument[3].ToUpper(); //설비명
                        break;
                    case 4:
        ...
        private void InitGrid()
        {
            try
            {
                ...
                _GridUtil.InitColumnUltraGrid(grid1, "WHTYPE", "창고유형", false, GridColDataType_emu.VarChar, 130, 100, Infragistics.Win.HAlign.Left, true, false);
                _GridUtil.InitColumnUltraGrid(grid1, "WHCODE", "창고코드", false, GridColDataType_emu.VarChar, 100, 100, Infragistics.Win.HAlign.Center, true, false);
        ...
        private void Search()
        {
            ...
            sWhType = Convert.ToString(cbo_WHTYPE_H.Value);
            sWhCode = txt_WHCODE_H.Text.Trim();
            sWhName = txt_WHNAME_H.Text.Trim();
            sUseFlag = Convert.ToString(cbo_USEFLAG_H.Value);

            _DtTemp = _biz.SEL_BM0080(sPlantCode, sWhType, sWhCode, sWhName, sUseFlag);
        ...
        private void txt_WHCODE_H_KeyPress(object sender, KeyPressEventArgs e)
        {
        ...
        private void txt_WHNAME_H_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (e.KeyChar == (char)Keys.Enter)
            {
                Search();
