...
namespace WIZ.PopUp
{
    public partial class BM0300_EXCEL : WIZ.Forms.BasePopupForm
    {
        #region [ 선언자 ]
        ...
        #region [ Form Load ]
        private void BM0300_EXCEL_Load(object sender, EventArgs e)
        {
            #region Grid 셋팅
            ...
            //라인
            gridManager.PopUpAdd("LINECODE", "LINENAME", "BM0050", new string[] { "PLANTCODE", "", "Y" });
            //불량항목
            gridManager.PopUpAdd("ERRORCODE", "ERRORDESC", "BM0100", new string[] { "PLANTCODE", "", "Y" });
            #endregion
        }
        #endregion
        ...
        private void INSERT_SAVE()
        {
            ...
            try
            {
                for (int i = 0; i < grid1.Rows.Count; i++)
                {
                    ...
                    sErrorCode = Convert.ToString(grid1.Rows[i].Cells["ERRORCODE"].Value);
                    ...
                    {
                        MessageBox.Show(Common.getLangText(Convert.ToString(i + 1) + "번째 열의 라인을 확인하세요.", "MSG"));
                        return;
                    }
                    if (sErrorCode == string.Empty)
                    {
                        MessageBox.Show(Common.getLangText(Convert.ToString(i + 1) + "번째 열의 불량항목을 확인하세요.", "MSG"));
                        return;
                    }
                ...
                DataTable excelUpType = helper_Excel.FillTable("USP_EXCELUPLOADTYPE_S1", CommandType.StoredProcedure
                                                         , helper.CreateParameter("AS_FORMID", "BM0300", DbType.String, ParameterDirection.Input));
            ...
            try
            {
                ...
                if (openfiledialog.ShowDialog() == System.Windows.Forms.DialogResult.OK)
                {
                    ...
                    for (int i = 1; i < dtExcelImport.Rows.Count; i++)
                    {
                        ...
                        drTarget["LINECODE"] = Convert.ToString(dtExcelImport.Rows[i][1]);
                        drTarget["LINENAME"] = Convert.ToString(dtExcelImport.Rows[i][2]);
                        drTarget["ERRORCODE"] = Convert.ToString(dtExcelImport.Rows[i][3]);
                        drTarget["ERRORDESC"] = Convert.ToString(dtExcelImport.Rows[i][4]);
