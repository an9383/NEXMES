...
namespace WIZ.PopUp
{
    public partial class BM0530_EXCEL : WIZ.Forms.BasePopupForm
    {
        #region [ 선언자 ]
        ...
        #region [ Form Load ]
        private void BM0530_EXCEL_Load(object sender, EventArgs e)
        {
            ...
            _GridUtil.InitColumnUltraGrid(grid1, "RESNAME", "사유명", true, GridColDataType_emu.VarChar, 120, 100, Infragistics.Win.HAlign.Left, true, false);
        ...
        private void INSERT_SAVE()
        {
            ...
            try
            {
                ...
                {
                    sResType = Convert.ToString(grid1.Rows[i].Cells["RESTYPE"].Value);
                    sResName = Convert.ToString(grid1.Rows[i].Cells["RESNAME"].Value);
                    sRemark = Convert.ToString(grid1.Rows[i].Cells["REMARK"].Value);
                    sUseFlag = Convert.ToString(grid1.Rows[i].Cells["USEFLAG"].Value);

                    if (sResType == string.Empty)
                    {
                        MessageBox.Show(Common.getLangText(Convert.ToString(i + 1) + "번째 열의 사유유형을 확인하세요.", "MSG"));
                        return;
                    }
                ...
                DataTable excelUpType = helper_Excel.FillTable("USP_EXCELUPLOADTYPE_S1", CommandType.StoredProcedure
                                                         , helper.CreateParameter("AS_FORMID", "BM0530", DbType.String, ParameterDirection.Input));
                ...
                if (excelUploadType == "R")
                {
                    ...
                    for (int i = 0; i < grid1.Rows.Count; i++)
                    {
                        helper.ExecuteNoneQuery("USP_BM0530_EXCEL_I1", CommandType.StoredProcedure
                                               , helper.CreateParameter("AS_RESTYPE", Convert.ToString(grid1.Rows[i].Cells["RESTYPE"].Value), DbType.String, ParameterDirection.Input)
                                               , helper.CreateParameter("AS_RESNAME", Convert.ToString(grid1.Rows[i].Cells["RESNAME"].Value), DbType.String, ParameterDirection.Input)
            ...
            try
            {
                ...
                if (openfiledialog.ShowDialog() == System.Windows.Forms.DialogResult.OK)
                {
                    ...
                    for (int i = 1; i < dtExcelImport.Rows.Count; i++)
                    {
                        ...
                        drTarget["RESTYPE"] = Convert.ToString(dtExcelImport.Rows[i][0]);
                        drTarget["RESNAME"] = Convert.ToString(dtExcelImport.Rows[i][1]);
