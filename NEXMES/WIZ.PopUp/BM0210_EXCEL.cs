...
namespace WIZ.PopUp
{
    public partial class BM0210_EXCEL : WIZ.Forms.BasePopupForm
    {
        #region [ 선언자 ]
        ...
        #region [ Form Load ]
        private void BM0210_EXCEL_Load(object sender, EventArgs e)
        {
            #region Grid 셋팅
            //GRID1 품목
            ...
            _GridUtil.InitColumnUltraGrid(grid1, "CUSTCODE", "고객사코드", true, GridColDataType_emu.VarChar, 90, 100, Infragistics.Win.HAlign.Center, true, false);
            _GridUtil.InitColumnUltraGrid(grid1, "CUSTNAME", "고객사명", true, GridColDataType_emu.VarChar, 150, 100, Infragistics.Win.HAlign.Left, true, false);
            ...
            _GridUtil.InitColumnUltraGrid(grid1, "CUSTITEMCODE", "고객사 품목", true, GridColDataType_emu.VarChar, 120, 100, Infragistics.Win.HAlign.Left, true, false);
            _GridUtil.InitColumnUltraGrid(grid1, "CUSTITEMNAME", "고객사 품명", true, GridColDataType_emu.VarChar, 200, 100, Infragistics.Win.HAlign.Left, true, false);
            _GridUtil.InitColumnUltraGrid(grid1, "PACKQTY", "포장수", true, GridColDataType_emu.VarChar, 90, 100, Infragistics.Win.HAlign.Left, true, false);
            ...
            //거래처
            gridManager.PopUpAdd("CUSTCODE", "CUSTNAME", "BM0030", new string[] { "PLANTCODE", "CT", "Y" });
        ...
        private void INSERT_SAVE()
        {
            ...
            string sCustItemCode = string.Empty;
            string sCustItemName = string.Empty;
            ...
            try
            {
                for (int i = 0; i < grid1.Rows.Count; i++)
                {
                    ...
                    sCustItemCode = Convert.ToString(grid1.Rows[i].Cells["CUSTITEMCODE"].Value);
                    sCustItemName = Convert.ToString(grid1.Rows[i].Cells["CUSTITEMNAME"].Value);
                    ...
                    {
                        MessageBox.Show(Common.getLangText(Convert.ToString(i + 1) + "번째 열의 고객사를 확인하세요.", "MSG"));
                        return;
                    }
                ...
                DataTable excelUpType = helper_Excel.FillTable("USP_EXCELUPLOADTYPE_S1", CommandType.StoredProcedure
                                                         , helper.CreateParameter("AS_FORMID", "BM0210", DbType.String, ParameterDirection.Input));
                ...
                if (excelUploadType == "R")
                {
                    ...
                    for (int i = 0; i < grid1.Rows.Count; i++)
                    {
                        helper.ExecuteNoneQuery("USP_BM0210_I1"
                                                    ...
                                                    , helper.CreateParameter("AS_PACKQTY", Convert.ToString(grid1.Rows[i].Cells["PACKQTY"].Value), DbType.String, ParameterDirection.Input)
                                                    , helper.CreateParameter("AS_CUSTITEMCODE", Convert.ToString(grid1.Rows[i].Cells["CUSTITEMCODE"].Value), DbType.String, ParameterDirection.Input)
                                                    , helper.CreateParameter("AS_CUSTITEMNAME", Convert.ToString(grid1.Rows[i].Cells["CUSTITEMNAME"].Value), DbType.String, ParameterDirection.Input)
            ...
            try
            {
                ...
                if (openfiledialog.ShowDialog() == System.Windows.Forms.DialogResult.OK)
                {
                    ...
                    for (int i = 1; i < dtExcelImport.Rows.Count; i++)
                    {
                        ...
                        drTarget["CUSTNAME"] = Convert.ToString(dtExcelImport.Rows[i][2]);
                        drTarget["ITEMCODE"] = Convert.ToString(dtExcelImport.Rows[i][3]);
                        drTarget["ITEMNAME"] = Convert.ToString(dtExcelImport.Rows[i][4]);
                        drTarget["CUSTITEMCODE"] = Convert.ToString(dtExcelImport.Rows[i][5]);
                        drTarget["CUSTITEMNAME"] = Convert.ToString(dtExcelImport.Rows[i][6]);
                        drTarget["PACKQTY"] = Convert.ToString(dtExcelImport.Rows[i][7]);
