...
namespace WIZ.PopUp
{
    public partial class POP_TBM1200_EXCEL : WIZ.Forms.BasePopupForm
    {
        #region [ 선언자 ]
        ...
        #region [ Form Load ]
        private void POP_TBM1200_EXCEL_Load(object sender, EventArgs e)
        {
            #region Grid 셋팅
            _GridUtil.InitializeGrid(this.grid1, true, true, false, "", false);

            _GridUtil.InitializeGrid(this.grid1, true, true, false, "", false);
            _GridUtil.InitColumnUltraGrid(grid1, "PLANTCODE", "사업장", true, GridColDataType_emu.VarChar, 190, 120, Infragistics.Win.HAlign.Center, true, false, null, null, null, null, null);
        ...
        private void INSERT_SAVE()
        {
            ...
            string BASEQTY = string.Empty;
            string UNITCODE = string.Empty;
            string COMPONENTQTY = string.Empty;
            string COMPONENTUNIT = string.Empty;
            string LGORT_IN = string.Empty;
            string LGORT_OUT = string.Empty;
            ...
            try
            {
                for (int i = 0; i < grid1.Rows.Count; i++)
                {
                    ...
                    COMPONENT = Convert.ToString(grid1.Rows[i].Cells["COMPONENT"].Value);
                    BASEQTY = Convert.ToString(grid1.Rows[i].Cells["BASEQTY"].Value);
                    UNITCODE = Convert.ToString(grid1.Rows[i].Cells["UNITCODE"].Value);
                    COMPONENTQTY = Convert.ToString(grid1.Rows[i].Cells["COMPONENTQTY"].Value);
                    COMPONENTUNIT = Convert.ToString(grid1.Rows[i].Cells["COMPONENTUNIT"].Value);
                    //LGORT_IN = Convert.ToString(grid1.Rows[i].Cells["LGORT_IN"].Value);
                    //LGORT_OUT = Convert.ToString(grid1.Rows[i].Cells["LGORT_OUT"].Value);
                ...
                DataTable excelUpType = helper_Excel.FillTable("USP_EXCELUPLOADTYPE_S1", CommandType.StoredProcedure
                                                         , helper.CreateParameter("AS_FORMID", "TBM1200", DbType.String, ParameterDirection.Input));
                ...
                if (excelUploadType == "R")
                {
                    ...
                    for (int i = 0; i < grid1.Rows.Count; i++)
                    {
                        helper.ExecuteNoneQuery("USP_POP_BM1200_I1"
                                          , CommandType.StoredProcedure
                                               ...
                                               , helper.CreateParameter("AS_COMPONENT", Convert.ToString(grid1.Rows[i].Cells["COMPONENT"].Value), DbType.String, ParameterDirection.Input)
                                               , helper.CreateParameter("AS_BASEQTY", Convert.ToString(grid1.Rows[i].Cells["BASEQTY"].Value), DbType.String, ParameterDirection.Input)
                                               ...
                                               , helper.CreateParameter("AS_COMPONENTQTY", Convert.ToString(grid1.Rows[i].Cells["COMPONENTQTY"].Value), DbType.String, ParameterDirection.Input)
                                               , helper.CreateParameter("AS_COMPONENTUNIT", Convert.ToString(grid1.Rows[i].Cells["COMPONENTUNIT"].Value), DbType.String, ParameterDirection.Input)
                                               //, helper.CreateParameter("AS_LGORT_IN",        Convert.ToString(grid1.Rows[i].Cells["AS_LGORT_IN"].Value),    DbType.String, ParameterDirection.Input)
                                               //, helper.CreateParameter("AS_LGORT_OUT",       Convert.ToString(grid1.Rows[i].Cells["AS_LGORT_OUT"].Value),   DbType.String, ParameterDirection.Input)
                                               //, helper.CreateParameter("AS_USEFLAG",         Convert.ToString(grid1.Rows[i].Cells["AS_USEFLAG"].Value),     DbType.String, ParameterDirection.Input)
                    ...
                    for (int i = 0; i < grid1.Rows.Count; i++)
                    {
                        helper.ExecuteNoneQuery("USP_POP_BM1200_I1"
                                          , CommandType.StoredProcedure
                                               ...
                                               , helper.CreateParameter("AS_ITEMCODE", Convert.ToString(grid1.Rows[i].Cells["ITEMCODE"].Value), DbType.String, ParameterDirection.Input)
                                               , helper.CreateParameter("AS_COMPONENT", Convert.ToString(grid1.Rows[i].Cells["COMPONENT"].Value), DbType.String, ParameterDirection.Input)
                                               , helper.CreateParameter("AS_BASEQTY", Convert.ToDouble(grid1.Rows[i].Cells["BASEQTY"].Value), DbType.String, ParameterDirection.Input)
                                               , helper.CreateParameter("AS_UNITCODE", Convert.ToString(grid1.Rows[i].Cells["UNITCODE"].Value), DbType.String, ParameterDirection.Input)
                                               , helper.CreateParameter("AS_COMPONENTQTY", Convert.ToDouble(grid1.Rows[i].Cells["COMPONENTQTY"].Value), DbType.String, ParameterDirection.Input)
                                               , helper.CreateParameter("AS_COMPONENTUNIT", Convert.ToString(grid1.Rows[i].Cells["COMPONENTUNIT"].Value), DbType.String, ParameterDirection.Input)
                                               //, helper.CreateParameter("AS_LGORT_IN",      Convert.ToString(grid1.Rows[i].Cells["AS_LGORT_IN"].Value),   DbType.String, ParameterDirection.Input)
                                               //, helper.CreateParameter("AS_LGORT_OUT",     Convert.ToString(grid1.Rows[i].Cells["AS_LGORT_OUT"].Value),  DbType.String, ParameterDirection.Input)
                                               //, helper.CreateParameter("AS_USEFLAG",       Convert.ToString(grid1.Rows[i].Cells["AS_USEFLAG"].Value),    DbType.String, ParameterDirection.Input)
            ...
            try
            {
                ...
                if (openfiledialog.ShowDialog() == System.Windows.Forms.DialogResult.OK)
                {
                    ...
                    for (int i = 1; i < dtExcelImport.Rows.Count; i++)
                    {
                        ...
                        drTarget["BASEQTY"] = Convert.ToString(dtExcelImport.Rows[i][2]);
                        drTarget["UNITCODE"] = Convert.ToString(dtExcelImport.Rows[i][3]);
                        drTarget["COMPONENT"] = Convert.ToString(dtExcelImport.Rows[i][4]);
                        drTarget["COMPONENTQTY"] = Convert.ToString(dtExcelImport.Rows[i][5]);
                        drTarget["COMPONENTUNIT"] = Convert.ToString(dtExcelImport.Rows[i][6]);
                        //drTarget["LGORT_IN"]      = Convert.ToString(dtExcelImport.Rows[i][7]);
                        //drTarget["LGORT_OUT"]     = Convert.ToString(dtExcelImport.Rows[i][8]);
