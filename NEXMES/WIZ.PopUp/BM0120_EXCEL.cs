...
namespace WIZ.PopUp
{
    public partial class BM0120_EXCEL : WIZ.Forms.BasePopupForm
    {
        #region [ 선언자 ]
        ...
        #region [ Form Load ]
        private void BM0120_EXCEL_Load(object sender, EventArgs e)
        {
            ...
            _GridUtil.InitColumnUltraGrid(grid1, "BASEQTY", "생산수량", true, GridColDataType_emu.Double, 100, 100, Infragistics.Win.HAlign.Right, true, false);
            ...
            _GridUtil.InitColumnUltraGrid(grid1, "COMPONENT", "하위품목", true, GridColDataType_emu.VarChar, 120, 120, Infragistics.Win.HAlign.Left, true, false);
            _GridUtil.InitColumnUltraGrid(grid1, "COMPONENTNAME", "하위품명", true, GridColDataType_emu.VarChar, 200, 150, Infragistics.Win.HAlign.Left, true, false);
            _GridUtil.InitColumnUltraGrid(grid1, "COMPONENTQTY", "하위품목수량", true, GridColDataType_emu.Double, 100, 100, Infragistics.Win.HAlign.Right, true, false);
            ...
            _GridUtil.InitColumnUltraGrid(grid1, "USEFLAG", "사용여부", true, GridColDataType_emu.VarChar, 90, 90, Infragistics.Win.HAlign.Left, true, false);
            _GridUtil.InitColumnUltraGrid(grid1, "REMARK", "비고", true, GridColDataType_emu.VarChar, 500, 200, Infragistics.Win.HAlign.Left, true, false);
        ...
        private void INSERT_SAVE()
        {
            ...
            string sBaseQty = string.Empty;
            string sUnitCode = string.Empty;
            string sCompoNent = string.Empty;
            string sCompoNentName = string.Empty;
            string sCompoNentQty = string.Empty;
            string sCompoNentUnit = string.Empty;
            ...
            try
            {
                for (int i = 0; i < grid1.Rows.Count; i++)
                {
                    ...
                    sBaseQty = Convert.ToString(grid1.Rows[i].Cells["BASEQTY"].Value);
                    sUnitCode = Convert.ToString(grid1.Rows[i].Cells["UNITCODE"].Value);
                    sCompoNent = Convert.ToString(grid1.Rows[i].Cells["COMPONENT"].Value);
                    sCompoNentName = Convert.ToString(grid1.Rows[i].Cells["COMPONENTNAME"].Value);
                    sCompoNentQty = Convert.ToString(grid1.Rows[i].Cells["COMPONENTQTY"].Value);
                    sCompoNentUnit = Convert.ToString(grid1.Rows[i].Cells["COMPONENTUNIT"].Value);
                    ...
                    if (sCompoNent == string.Empty)
                    {
                        MessageBox.Show(Common.getLangText(Convert.ToString(i + 1) + "번째 열의 하위품목을 확인하세요.", "MSG"));
                        return;
                    }
                ...
                DataTable excelUpType = helper_Excel.FillTable("USP_EXCELUPLOADTYPE_S1", CommandType.StoredProcedure
                                                         , helper.CreateParameter("AS_FORMID", "BM0120", DbType.String, ParameterDirection.Input));
                ...
                if (excelUploadType == "R")
                {
                    ...
                    for (int i = 0; i < grid1.Rows.Count; i++)
                    {
                        helper.ExecuteNoneQuery("USP_BM0120_EXCEL_I1", CommandType.StoredProcedure
                                               ...
                                               , helper.CreateParameter("AF_BASEQTY", Convert.ToString(grid1.Rows[i].Cells["BASEQTY"].Value), DbType.String, ParameterDirection.Input)
                                               , helper.CreateParameter("AF_COMPONENTQTY", Convert.ToString(grid1.Rows[i].Cells["COMPONENTQTY"].Value), DbType.String, ParameterDirection.Input)
            ...
            try
            {
                ...
                if (openfiledialog.ShowDialog() == System.Windows.Forms.DialogResult.OK)
                {
                    ...
                    for (int i = 1; i < dtExcelImport.Rows.Count; i++)
                    {
                        ...
                        drTarget["BASEQTY"] = Convert.ToString(dtExcelImport.Rows[i][3]);
                        drTarget["UNITCODE"] = Convert.ToString(dtExcelImport.Rows[i][4]);
                        drTarget["COMPONENT"] = Convert.ToString(dtExcelImport.Rows[i][5]);
                        drTarget["COMPONENTNAME"] = Convert.ToString(dtExcelImport.Rows[i][6]);
                        drTarget["COMPONENTQTY"] = Convert.ToString(dtExcelImport.Rows[i][7]);
                        drTarget["COMPONENTUNIT"] = Convert.ToString(dtExcelImport.Rows[i][8]);
