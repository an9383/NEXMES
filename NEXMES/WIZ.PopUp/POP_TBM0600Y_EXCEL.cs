...
namespace WIZ.PopUp
{
    public partial class POP_TBM0600Y_EXCEL : WIZ.Forms.BasePopupForm
    {
        #region [ 선언자 ]
        ...
        #region [ Form Load ]
        private void POP_TBM0600Y_EXCEL_Load(object sender, EventArgs e)
        {
            #region Grid 셋팅
            ...
            _GridUtil.InitColumnUltraGrid(grid1, "OPCODE", "공정ⓒ", true, GridColDataType_emu.VarChar, 100, 100, Infragistics.Win.HAlign.Left, true, false, null, null, null, null, null);
            _GridUtil.InitColumnUltraGrid(grid1, "WORKCENTERCODE", "작업장ⓒ", true, GridColDataType_emu.VarChar, 206, 40, Infragistics.Win.HAlign.Left, true, true, null, null, null, null, null);
            _GridUtil.InitColumnUltraGrid(grid1, "WORKCENTERNAME", "작업장명", true, GridColDataType_emu.VarChar, 120, 100, Infragistics.Win.HAlign.Left, true, true, null, null, null, null, null);
            //     _GridUtil.InitColumnUltraGrid(grid1, "LINECODE",       "라인ⓒ",           true, GridColDataType_emu.VarChar, 100, 100, Infragistics.Win.HAlign.Left,   true, false, null, null, null, null, null);
            _GridUtil.InitColumnUltraGrid(grid1, "ZBFLAG", "지그비사용유무ⓒ", true, GridColDataType_emu.VarChar, 90, 100, Infragistics.Win.HAlign.Center, true, true, null, null, null, null, null);
        ...
        private void INSERT_SAVE()
        {
            ...
            try
            {
                for (int i = 0; i < grid1.Rows.Count; i++)
                {
                    ...
                    OPCODE = Convert.ToString(grid1.Rows[i].Cells["OPCODE"].Value);
                    //                    LINECODE       = Convert.ToString(grid1.Rows[i].Cells["LINECODE"].Value);
                    WORKCENTERCODE = Convert.ToString(grid1.Rows[i].Cells["WORKCENTERCODE"].Value);
                    WORKCENTERNAME = Convert.ToString(grid1.Rows[i].Cells["WORKCENTERNAME"].Value);
                    ZBFLAG = Convert.ToString(grid1.Rows[i].Cells["ZBFLAG"].Value);
                    ...
                    {
                        MessageBox.Show(Convert.ToString(i + 1) + "번째 열의 공정ⓒ를 확인하세요.");
                        return;
                    }
                    if (WORKCENTERCODE == string.Empty)
                    {
                        MessageBox.Show(Convert.ToString(i + 1) + "번째 열의 작업장ⓒ를 확인하세요.");
                        return;
                    }
                }
                ...
                if (excelUploadType == "R")
                {
                    ...
                    for (int i = 0; i < grid1.Rows.Count; i++)
                    {
                        helper.ExecuteNoneQuery("USP_POP_BM0600Y_I1"
                                          , CommandType.StoredProcedure
                                               ...
                                               , helper.CreateParameter("AS_ZBFLAG", Convert.ToString(grid1.Rows[i].Cells["ZBFLAG"].Value), DbType.String, ParameterDirection.Input)
            ...
            try
            {
                ...
                if (openfiledialog.ShowDialog() == System.Windows.Forms.DialogResult.OK)
                {
                    ...
                    for (int i = 1; i < dtExcelImport.Rows.Count; i++)
                    {
                        ...
                        drTarget["OPCODE"] = Convert.ToString(dtExcelImport.Rows[i][1]);
                        drTarget["WORKCENTERCODE"] = Convert.ToString(dtExcelImport.Rows[i][2]);
                        drTarget["WORKCENTERNAME"] = Convert.ToString(dtExcelImport.Rows[i][3]);
                        drTarget["ZBFLAG"] = Convert.ToString(dtExcelImport.Rows[i][4]);
                        drTarget["USEFLAG"] = Convert.ToString(dtExcelImport.Rows[i][5]);
                        //           drTarget["LINECODE"]       = Convert.ToString(dtExcelImport.Rows[i][2]);
