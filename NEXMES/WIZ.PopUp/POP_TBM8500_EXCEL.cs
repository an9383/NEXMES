...
namespace WIZ.PopUp
{
    public partial class POP_TBM8500_EXCEL : WIZ.Forms.BasePopupForm
    {
        #region [ 선언자 ]
        ...
        #region [ Form Load ]
        private void POP_TBM8500_EXCEL_Load(object sender, EventArgs e)
        {
            #region Grid 셋팅
            ...
            _GridUtil.InitColumnUltraGrid(grid1, "PALLETNO", "Pallet No", true, GridColDataType_emu.VarChar, 140, 100, Infragistics.Win.HAlign.Center, true, false, null, null, null, null, null);
            _GridUtil.InitColumnUltraGrid(grid1, "MAPPINGNO", "Mapping No", true, GridColDataType_emu.VarChar, 160, 100, Infragistics.Win.HAlign.Center, true, false, null, null, null, null, null);
            _GridUtil.InitColumnUltraGrid(grid1, "STATUS", "Pallet 상태", true, GridColDataType_emu.VarChar, 100, 50, Infragistics.Win.HAlign.Center, true, false, null, null, null, null, null);
            _GridUtil.InitColumnUltraGrid(grid1, "STORAGELOCCODE", "Pallet 위치", true, GridColDataType_emu.VarChar, 140, 30, Infragistics.Win.HAlign.Center, true, false, null, null, null, null, null);
            _GridUtil.InitColumnUltraGrid(grid1, "LOADRATE", "적재율(%)", true, GridColDataType_emu.VarChar, 80, 100, Infragistics.Win.HAlign.Right, true, false, null, null, null, null, null);
            _GridUtil.InitColumnUltraGrid(grid1, "ORDER_LOC", "지시위치", true, GridColDataType_emu.VarChar, 140, 100, Infragistics.Win.HAlign.Center, true, false, null, null, null, null, null);
            _GridUtil.InitColumnUltraGrid(grid1, "USEFLAG", "사용여부ⓒ", true, GridColDataType_emu.VarChar, 90, 100, Infragistics.Win.HAlign.Center, true, true, null, null, null, null, null);

            _GridUtil.SetInitUltraGridBind(grid1);

            grid1.DisplayLayout.Bands[0].Columns["PLANTCODE"].Header.Appearance.ForeColor = Color.SkyBlue;
            grid1.DisplayLayout.Bands[0].Columns["PALLETNO"].Header.Appearance.ForeColor = Color.SkyBlue;
        ...
        private void INSERT_SAVE()
        {
            ...
            string MAPPINGNO = string.Empty;
            string STATUS = string.Empty;
            string STORAGELOCCODE = string.Empty;
            string LOADRATE = string.Empty;
            string ORDER_LOC = string.Empty;
            ...
            try
            {
                for (int i = 0; i < grid1.Rows.Count; i++)
                {
                    ...
                    PALLETNO = Convert.ToString(grid1.Rows[i].Cells["PALLETNO"].Value);
                    MAPPINGNO = Convert.ToString(grid1.Rows[i].Cells["MAPPINGNO"].Value);
                    STATUS = Convert.ToString(grid1.Rows[i].Cells["STATUS"].Value);
                    STORAGELOCCODE = Convert.ToString(grid1.Rows[i].Cells["STORAGELOCCODE"].Value);
                    LOADRATE = Convert.ToString(grid1.Rows[i].Cells["LOADRATE"].Value);
                    ORDER_LOC = Convert.ToString(grid1.Rows[i].Cells["ORDER_LOC"].Value);
                    ...
                    if (PALLETNO == string.Empty)
                    {
                        MessageBox.Show(Convert.ToString(i + 1) + "번째 열의 PALLET NO를 확인하세요.");
                        return;
                    }
                ...
                if (excelUploadType == "R")
                {
                    ...
                    for (int i = 0; i < grid1.Rows.Count; i++)
                    {
                        helper.ExecuteNoneQuery("USP_POP_BM8500_I1"
                                          , CommandType.StoredProcedure
                                               ...
                                               , helper.CreateParameter("AS_MAPPINGNO", Convert.ToString(grid1.Rows[i].Cells["MAPPINGNO"].Value), DbType.String, ParameterDirection.Input)
                                               ...
                                               , helper.CreateParameter("AS_LOADRATE", Convert.ToString(grid1.Rows[i].Cells["LOADRATE"].Value), DbType.String, ParameterDirection.Input)
                                               , helper.CreateParameter("AS_ORDER_LOC", Convert.ToString(grid1.Rows[i].Cells["ORDER_LOC"].Value), DbType.String, ParameterDirection.Input)
            ...
            try
            {
                ...
                if (openfiledialog.ShowDialog() == System.Windows.Forms.DialogResult.OK)
                {
                    ...
                    for (int i = 1; i < dtExcelImport.Rows.Count; i++)
                    {
                        ...
                        drTarget["PALLETNO"] = Convert.ToString(dtExcelImport.Rows[i][1]);
                        drTarget["MAPPINGNO"] = Convert.ToString(dtExcelImport.Rows[i][2]);
                        drTarget["STATUS"] = Convert.ToString(dtExcelImport.Rows[i][3]);
                        drTarget["STORAGELOCCODE"] = Convert.ToString(dtExcelImport.Rows[i][4]);
                        drTarget["LOADRATE"] = Convert.ToString(dtExcelImport.Rows[i][5]);
                        drTarget["ORDER_LOC"] = Convert.ToString(dtExcelImport.Rows[i][6]);
