...
namespace WIZ.PopUp
{
    public partial class BM0600_EXCEL : WIZ.Forms.BasePopupForm
    {
        #region [ 선언자 ]
        ...
        #region [ Form Load ]
        private void BM0600_EXCEL_Load(object sender, EventArgs e)
        {
            #region Grid 셋팅
            //GRID1
            ...
            _GridUtil.InitColumnUltraGrid(grid1, "STDTIME", "표준작업시간", true, GridColDataType_emu.Double, 100, 80, Infragistics.Win.HAlign.Right, true, false);
            _GridUtil.InitColumnUltraGrid(grid1, "CYCLETIME", "CYCLE시간", true, GridColDataType_emu.Double, 90, 80, Infragistics.Win.HAlign.Right, true, false);
            _GridUtil.InitColumnUltraGrid(grid1, "SETUPTIME", "SETUP시간", true, GridColDataType_emu.Double, 90, 80, Infragistics.Win.HAlign.Right, true, false);
            _GridUtil.InitColumnUltraGrid(grid1, "STOPTIME", "정지시간", true, GridColDataType_emu.Double, 90, 80, Infragistics.Win.HAlign.Right, true, false);
            _GridUtil.InitColumnUltraGrid(grid1, "BASERUNTIME", "기본가동시간", true, GridColDataType_emu.Double, 100, 80, Infragistics.Win.HAlign.Right, true, false);
            _GridUtil.InitColumnUltraGrid(grid1, "CAVITY", "CAVITY", true, GridColDataType_emu.Double, 90, 80, Infragistics.Win.HAlign.Right, true, false);
        ...
        private void INSERT_SAVE()
        {
            ...
            string sStdTime = string.Empty;
            string sCycleTime = string.Empty;
            string sSetupTime = string.Empty;
            string sStopTime = string.Empty;
            string sBaseRunTime = string.Empty;
            ...
            try
            {
                for (int i = 0; i < grid1.Rows.Count; i++)
                {
                    ...
                    sStdTime = Convert.ToString(grid1.Rows[i].Cells["STDTIME"].Value);
                    sCycleTime = Convert.ToString(grid1.Rows[i].Cells["CYCLETIME"].Value);
                    sSetupTime = Convert.ToString(grid1.Rows[i].Cells["SETUPTIME"].Value);
                    sBaseRunTime = Convert.ToString(grid1.Rows[i].Cells["BASERUNTIME"].Value);
                    ...
                    {
                        MessageBox.Show(Common.getLangText(Convert.ToString(i + 1) + "번째 열의 공정을 확인하세요.", "MSG"));
                        return;
                    }
                ...
                DataTable excelUpType = helper_Excel.FillTable("USP_EXCELUPLOADTYPE_S1", CommandType.StoredProcedure
                                                         , helper.CreateParameter("AS_FORMID", "BM0600", DbType.String, ParameterDirection.Input));
                ...
                if (excelUploadType == "R")
                {
                    ...
                    for (int i = 0; i < grid1.Rows.Count; i++)
                    {
                        helper.ExecuteNoneQuery("USP_BM0600_I1"
                                                    ...
                                                    , helper.CreateParameter("AF_STDTIME", DBHelper.nvlString(grid1.Rows[i].Cells["STDTIME"].Value), DbType.Double, ParameterDirection.Input)
                                                    , helper.CreateParameter("AF_CYCLETIME", DBHelper.nvlString(grid1.Rows[i].Cells["CYCLETIME"].Value), DbType.Double, ParameterDirection.Input)
                                                    , helper.CreateParameter("AF_SETUPTIME", DBHelper.nvlString(grid1.Rows[i].Cells["SETUPTIME"].Value), DbType.Double, ParameterDirection.Input)
                                                    , helper.CreateParameter("AD_STARTDATE", DateTime.Now, DbType.DateTime, ParameterDirection.Input)
                                                    , helper.CreateParameter("AD_ENDDATE", DateTime.Now, DbType.DateTime, ParameterDirection.Input)
                                                    , helper.CreateParameter("AF_STOPTIME", DBHelper.nvlString(grid1.Rows[i].Cells["STOPTIME"].Value), DbType.Double, ParameterDirection.Input)
                                                    , helper.CreateParameter("AF_BASERUNTIME", DBHelper.nvlString(grid1.Rows[i].Cells["BASERUNTIME"].Value), DbType.Double, ParameterDirection.Input)
                                                    , helper.CreateParameter("AF_CAVITY", DBHelper.nvlString(grid1.Rows[i].Cells["CAVITY"].Value), DbType.Double, ParameterDirection.Input)
            ...
            try
            {
                ...
                if (openfiledialog.ShowDialog() == System.Windows.Forms.DialogResult.OK)
                {
                    ...
                    for (int i = 1; i < dtExcelImport.Rows.Count; i++)
                    {
                        ...
                        drTarget["OPCODE"] = Convert.ToString(dtExcelImport.Rows[i][3]);
                        drTarget["OPNAME"] = Convert.ToString(dtExcelImport.Rows[i][4]);
                        drTarget["STDTIME"] = Convert.ToString(dtExcelImport.Rows[i][5]);
                        drTarget["CYCLETIME"] = Convert.ToString(dtExcelImport.Rows[i][6]);
                        drTarget["SETUPTIME"] = Convert.ToString(dtExcelImport.Rows[i][7]);
                        drTarget["STOPTIME"] = Convert.ToString(dtExcelImport.Rows[i][8]);
                        drTarget["BASERUNTIME"] = Convert.ToString(dtExcelImport.Rows[i][9]);
                        drTarget["CAVITY"] = Convert.ToString(dtExcelImport.Rows[i][10]);
