...
namespace WIZ.PopUp
{
    public partial class BM0200_EXCEL : WIZ.Forms.BasePopupForm
    {
        #region [ 선언자 ]
        ...
        #region [ Form Load ]
        private void BM0200_EXCEL_Load(object sender, EventArgs e)
        {
            #region Grid 셋팅
            ...
            _GridUtil.InitColumnUltraGrid(grid1, "OUTCOSTFLAG", "유무상구분", true, GridColDataType_emu.VarChar, 90, 100, Infragistics.Win.HAlign.Left, true, false);
            _GridUtil.InitColumnUltraGrid(grid1, "CUSTITEMCODE", "업체품목", true, GridColDataType_emu.VarChar, 120, 120, Infragistics.Win.HAlign.Left, true, false);
            _GridUtil.InitColumnUltraGrid(grid1, "CUSTITEMNAME", "업체품명", true, GridColDataType_emu.VarChar, 200, 150, Infragistics.Win.HAlign.Left, true, false);
            ...
            //거래처
            gridManager.PopUpAdd("CUSTCODE", "CUSTNAME", "BM0030", new string[] { "PLANTCODE", "VD", "Y" });
        ...
        private void INSERT_SAVE()
        {
            ...
            try
            {
                ...
                DataTable excelUpType = helper_Excel.FillTable("USP_EXCELUPLOADTYPE_S1", CommandType.StoredProcedure
                                                         , helper.CreateParameter("AS_FORMID", "BM0200", DbType.String, ParameterDirection.Input));
                ...
                if (excelUploadType == "R")
                {
                    ...
                    for (int i = 0; i < grid1.Rows.Count; i++)
                    {
                        helper.ExecuteNoneQuery("USP_BM0200_EXCEL_I1"
            ...
            try
            {
                ...
                if (openfiledialog.ShowDialog() == System.Windows.Forms.DialogResult.OK)
                {
                    ...
                    for (int i = 1; i < dtExcelImport.Rows.Count; i++)
                    {
                        ...
                        drTarget["UNITCOST"] = Convert.ToString(dtExcelImport.Rows[i][5]);
                        drTarget["OUTCOSTFLAG"] = Convert.ToString(dtExcelImport.Rows[i][6]);
                        drTarget["CUSTITEMCODE"] = Convert.ToString(dtExcelImport.Rows[i][7]);
                        drTarget["CUSTITEMNAME"] = Convert.ToString(dtExcelImport.Rows[i][8]);
