...
namespace WIZ.PopUp
{
    public partial class PD4030_POP : Form
    {
        #region < MEMBER AREA >
        ...
        #region < CONSTRUCTOR >
        public PD4030_POP(string sItemCode, string sInspCode, string sValueType)
        {
            InitializeComponent();
        ...
        #region < FORM LOAD >
        private void PD4030_POP_Load(object sender, EventArgs e)
        {
            ScanLogic();
        ...
        private void ScanLogic()
        {
            try
            {
                ...
                _dt = USP_PD4021_S1(_sItemCode, _sInspCode, ref RS_CODE, ref RS_MSG);
                ...
                if (RS_CODE == "S")
                {
                    ...
                    {
                        MessageBox.Show(Common.getLangText("조회 결과가 없습니다.", "MSG"), "제품검사 등록");
                    }
                }
                else
                {
                    MessageBox.Show(RS_MSG, "제품검사 등록");
                }
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.ToString(), "제품검사 등록");
            }
        }

        private DataTable USP_PD4021_S1(string sItemCode, string sInspCode, ref string RS_CODE, ref string RS_MSG)
        {
            DBHelper helper = new DBHelper(false);

            try
            {
                rtnDtTemp = helper.FillTable("USP_PD4031_S1"
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("값을 입력한 후, 진행하세요.", "제품검사 등록");
                return;
            }
            ...
            {
                MessageBox.Show("입력값을 확인하세요.", "제품검사 등록");
                return;
            }

            DialogResult result = MessageBox.Show("[" + _sInspCode + "] " + Common.getLangText("검사결과를 등록하시겠습니까?", "MSG"), "제품검사 등록", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1);
            ...
            {
                MessageBox.Show(ex.Message, "제품검사 등록");
            }
        }
