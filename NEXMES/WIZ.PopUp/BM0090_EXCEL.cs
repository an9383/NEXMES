...
namespace WIZ.PopUp
{
    public partial class BM0090_EXCEL : WIZ.Forms.BasePopupForm
    {
        #region [ 선언자 ]
        ...
        #region [ Form Load ]
        private void BM0090_EXCEL_Load(object sender, EventArgs e)
        {
            ...
            _GridUtil.InitColumnUltraGrid(grid1, "STORAGELOCCODE", "저장위치코드", true, GridColDataType_emu.VarChar, 150, 120, Infragistics.Win.HAlign.Left, true, false);
            _GridUtil.InitColumnUltraGrid(grid1, "NAME", "저장위치명", true, GridColDataType_emu.VarChar, 150, 120, Infragistics.Win.HAlign.Left, true, false);
            _GridUtil.InitColumnUltraGrid(grid1, "WHCODE", "창고코드", true, GridColDataType_emu.VarChar, 90, 120, Infragistics.Win.HAlign.Left, true, false);
            _GridUtil.InitColumnUltraGrid(grid1, "OPCODE", "공정코드", true, GridColDataType_emu.VarChar, 90, 100, Infragistics.Win.HAlign.Left, true, false);
        ...
        private void INSERT_SAVE()
        {
            ...
            string sStoragelocType = string.Empty;
            ...
            try
            {
                for (int i = 0; i < grid1.Rows.Count; i++)
                {
                    ...
                    sWhName = Convert.ToString(grid1.Rows[i].Cells["WHNAME"].Value);
                    sStoragelocName = Convert.ToString(grid1.Rows[i].Cells["STORAGELOCNAME"].Value);
                    sStoragelocType = Convert.ToString(grid1.Rows[i].Cells["STORAGELOCTYPE"].Value);
                    ...
                    {
                        MessageBox.Show(Common.getLangText(Convert.ToString(i + 1) + "번째 열의 창고코드를 확인하세요.", "MSG"));
                        return;
                    }
                }

                DataTable excelUpType = helper_Excel.FillTable("USP_EXCELUPLOADTYPE_S1", CommandType.StoredProcedure
                                                         , helper.CreateParameter("AS_FORMID", "BM0090", DbType.String, ParameterDirection.Input));
                ...
                if (excelUploadType == "R")
                {
                    ...
                    for (int i = 0; i < grid1.Rows.Count; i++)
                    {
                        helper.ExecuteNoneQuery("USP_BM0090_EXCEL_I1", CommandType.StoredProcedure
                                               ...
                                               , helper.CreateParameter("AS_STORAGELOCNAME", Convert.ToString(grid1.Rows[i].Cells["STORAGELOCNAME"].Value), DbType.String, ParameterDirection.Input)
                                               , helper.CreateParameter("AS_STORAGELOCTYPE", Convert.ToString(grid1.Rows[i].Cells["STORAGELOCTYPE"].Value), DbType.String, ParameterDirection.Input)
            ...
            try
            {
                ...
                if (openfiledialog.ShowDialog() == System.Windows.Forms.DialogResult.OK)
                {
                    ...
                    for (int i = 1; i < dtExcelImport.Rows.Count; i++)
                    {
                        ...
                        drTarget["STORAGELOCNAME"] = Convert.ToString(dtExcelImport.Rows[i][1]);
                        drTarget["STORAGELOCTYPE"] = Convert.ToString(dtExcelImport.Rows[i][2]);
                        drTarget["WHCODE"] = Convert.ToString(dtExcelImport.Rows[i][3]);
                        drTarget["WHNAME"] = Convert.ToString(dtExcelImport.Rows[i][4]);
