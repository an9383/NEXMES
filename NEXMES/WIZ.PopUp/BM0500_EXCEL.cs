...
namespace WIZ.PopUp
{
    public partial class BM0500_EXCEL : WIZ.Forms.BasePopupForm
    {
        #region [ 선언자 ]
        ...
        #region [ Form Load ]
        private void BM0500_EXCEL_Load(object sender, EventArgs e)
        {
            ...
            _GridUtil.InitColumnUltraGrid(grid1, "CARDESC", "차량내역", true, GridColDataType_emu.VarChar, 120, 100, Infragistics.Win.HAlign.Left, true, false);
            _GridUtil.InitColumnUltraGrid(grid1, "CARGUBUN", "자차구분", true, GridColDataType_emu.VarChar, 120, 100, Infragistics.Win.HAlign.Left, true, false);
            _GridUtil.InitColumnUltraGrid(grid1, "CARWGT", "차량중량", true, GridColDataType_emu.VarChar, 120, 100, Infragistics.Win.HAlign.Left, true, false);
            _GridUtil.InitColumnUltraGrid(grid1, "CAREMPWGT", "공차중량", true, GridColDataType_emu.VarChar, 120, 100, Infragistics.Win.HAlign.Left, true, false);
            _GridUtil.InitColumnUltraGrid(grid1, "CARMAXWGT", "최대적재중량", true, GridColDataType_emu.VarChar, 120, 100, Infragistics.Win.HAlign.Left, true, false);
            _GridUtil.InitColumnUltraGrid(grid1, "CUSTCODE", "용차업체", true, GridColDataType_emu.VarChar, 120, 100, Infragistics.Win.HAlign.Left, true, false);
            _GridUtil.InitColumnUltraGrid(grid1, "CARCLASS", "차종", true, GridColDataType_emu.VarChar, 120, 100, Infragistics.Win.HAlign.Left, true, false);
            _GridUtil.InitColumnUltraGrid(grid1, "CARMAKER", "제조사", true, GridColDataType_emu.VarChar, 120, 100, Infragistics.Win.HAlign.Left, true, false);
            ...
            rtnDtTemp = _Common.GET_BM0000_CODE("CARGUBUN");  //차량구분                                                                                                                            
        ...
        private void INSERT_SAVE()
        {
            ...
            string sCarWgt = string.Empty;
            string sCarempWgt = string.Empty;
            string sCarmaxWgt = string.Empty;
            string sCustCode = string.Empty;
            string sCarClass = string.Empty;
            ...
            try
            {
                for (int i = 0; i < grid1.Rows.Count; i++)
                {
                    ...
                    sCarDesc = Convert.ToString(grid1.Rows[i].Cells["CARDESC"].Value);
                    sCarGubun = Convert.ToString(grid1.Rows[i].Cells["CARGUBUN"].Value);
                    sCarWgt = Convert.ToString(grid1.Rows[i].Cells["CARWGT"].Value);
                    sCarempWgt = Convert.ToString(grid1.Rows[i].Cells["CAREMPWGT"].Value);
                    sCarmaxWgt = Convert.ToString(grid1.Rows[i].Cells["CARMAXWGT"].Value);
                    sCustCode = Convert.ToString(grid1.Rows[i].Cells["CUSTCODE"].Value);
                    sCarClass = Convert.ToString(grid1.Rows[i].Cells["CARCLASS"].Value);
                    sCarMaker = Convert.ToString(grid1.Rows[i].Cells["CARMAKER"].Value);
                ...
                DataTable excelUpType = helper_Excel.FillTable("USP_EXCELUPLOADTYPE_S1", CommandType.StoredProcedure
                                                         , helper.CreateParameter("AS_FORMID", "BM0500", DbType.String, ParameterDirection.Input));
                ...
                if (excelUploadType == "R")
                {
                    ...
                    for (int i = 0; i < grid1.Rows.Count; i++)
                    {
                        helper.ExecuteNoneQuery("USP_BM0500_EXCEL_I1", CommandType.StoredProcedure
                                               ...
                                               , helper.CreateParameter("AS_CARDESC", Convert.ToString(grid1.Rows[i].Cells["CARDESC"].Value), DbType.String, ParameterDirection.Input)
                                               , helper.CreateParameter("AS_CARGUBUN", Convert.ToString(grid1.Rows[i].Cells["CARGUBUN"].Value), DbType.String, ParameterDirection.Input)
                                               , helper.CreateParameter("AS_CARWGT", Convert.ToString(grid1.Rows[i].Cells["CARWGT"].Value), DbType.String, ParameterDirection.Input)
                                               , helper.CreateParameter("AS_CAREMPWGT", Convert.ToString(grid1.Rows[i].Cells["CAREMPWGT"].Value), DbType.String, ParameterDirection.Input)
                                               , helper.CreateParameter("AS_CARMAXWGT", Convert.ToString(grid1.Rows[i].Cells["CARMAXWGT"].Value), DbType.String, ParameterDirection.Input)
                                               ...
                                               , helper.CreateParameter("AS_CARCLASS", Convert.ToString(grid1.Rows[i].Cells["CARCLASS"].Value), DbType.String, ParameterDirection.Input)
                                               , helper.CreateParameter("AS_CARMAKER", Convert.ToString(grid1.Rows[i].Cells["CARMAKER"].Value), DbType.String, ParameterDirection.Input)
            ...
            try
            {
                ...
                if (openfiledialog.ShowDialog() == System.Windows.Forms.DialogResult.OK)
                {
                    ...
                    for (int i = 1; i < dtExcelImport.Rows.Count; i++)
                    {
                        ...
                        drTarget["CARDESC"] = Convert.ToString(dtExcelImport.Rows[i][1]);
                        drTarget["CARGUBUN"] = Convert.ToString(dtExcelImport.Rows[i][2]);
                        drTarget["CARWGT"] = Convert.ToString(dtExcelImport.Rows[i][3]);
                        drTarget["CAREMPWGT"] = Convert.ToString(dtExcelImport.Rows[i][4]);
                        drTarget["CARMAXWGT"] = Convert.ToString(dtExcelImport.Rows[i][5]);
                        drTarget["CUSTCODE"] = Convert.ToString(dtExcelImport.Rows[i][6]);
                        drTarget["CARCLASS"] = Convert.ToString(dtExcelImport.Rows[i][7]);
                        drTarget["CARMAKER"] = Convert.ToString(dtExcelImport.Rows[i][8]);
