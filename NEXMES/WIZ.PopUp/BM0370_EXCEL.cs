...
namespace WIZ.PopUp
{
    public partial class BM0370_EXCEL : WIZ.Forms.BasePopupForm
    {
        #region [ 선언자 ]
        ...
        #region [ Form Load ]
        private void BM0370_EXCEL_Load(object sender, EventArgs e)
        {
            #region Grid 셋팅
            ...
            _GridUtil.InitColumnUltraGrid(grid1, "FAILURETYPE", "고장분류", true, GridColDataType_emu.VarChar, 100, 100, Infragistics.Win.HAlign.Left, true, false);
            ...
            grid1.DisplayLayout.Bands[0].Columns["FAILURECODE"].Header.Appearance.ForeColor = Color.SkyBlue;
            ...
            gridManager.PopUpAdd("FAILURECODE", "FAILURENAME", "BM0370", new string[] { "PLANTCODE", "", "Y" });
        ...
        private void INSERT_SAVE()
        {
            ...
            try
            {
                for (int i = 0; i < grid1.Rows.Count; i++)
                {
                    ...
                    sFailUreType = Convert.ToString(grid1.Rows[i].Cells["FAILURETYPE"].Value);
                    sFailUreCode = Convert.ToString(grid1.Rows[i].Cells["FAILURECODE"].Value);
                    sFailUreName = Convert.ToString(grid1.Rows[i].Cells["FAILURENAME"].Value);
                    ...
                    if (sFailUreType == string.Empty)
                    {
                        MessageBox.Show(Common.getLangText(Convert.ToString(i + 1) + "번째 열의 고장분류를 확인하세요.", "MSG"));
                        return;
                    }
                    if (sFailUreCode == string.Empty)
                    {
                        MessageBox.Show(Common.getLangText(Convert.ToString(i + 1) + "번째 열의 고장코드를 확인하세요.", "MSG"));
                        return;
                    }
                }

                DataTable excelUpType = helper_Excel.FillTable("USP_EXCELUPLOADTYPE_S1", CommandType.StoredProcedure
                                                         , helper.CreateParameter("AS_FORMID", "BM0370", DbType.String, ParameterDirection.Input));
                ...
                if (excelUploadType == "R")
                {
                    ...
                    for (int i = 0; i < grid1.Rows.Count; i++)
                    {
                        helper.ExecuteNoneQuery("USP_BM0370_I1"
                                                    ...
                                                    , helper.CreateParameter("AS_FAILURETYPE", Convert.ToString(grid1.Rows[i].Cells["FAILURETYPE"].Value), DbType.String, ParameterDirection.Input)
                                                    , helper.CreateParameter("AS_FAILURECODE", Convert.ToString(grid1.Rows[i].Cells["FAILURECODE"].Value), DbType.String, ParameterDirection.Input)
                                                    , helper.CreateParameter("AS_FAILURENAME", Convert.ToString(grid1.Rows[i].Cells["FAILURENAME"].Value), DbType.String, ParameterDirection.Input)
            ...
            try
            {
                ...
                if (openfiledialog.ShowDialog() == System.Windows.Forms.DialogResult.OK)
                {
                    ...
                    for (int i = 1; i < dtExcelImport.Rows.Count; i++)
                    {
                        ...
                        drTarget["FAILURECODE"] = Convert.ToString(dtExcelImport.Rows[i][1]);
                        drTarget["FAILURENAME"] = Convert.ToString(dtExcelImport.Rows[i][2]);
                        drTarget["FAILURETYPE"] = Convert.ToString(dtExcelImport.Rows[i][3]);
                        drTarget["USEFLAG"] = Convert.ToString(dtExcelImport.Rows[i][4]).Trim();
                        drTarget["REMARK"] = Convert.ToString(dtExcelImport.Rows[i][5]);
