...
namespace WIZ.PopUp
{
    public partial class BM0070_POP : WIZ.Forms.BasePopupForm
    {
        #region < MEMBER AREA >
        ...
        public BM0070_POP(string[] param)
        {
            InitializeComponent();
            ...
            for (int i = 0; i < param.Length; i++)
            {
                ...
                switch (i)
                {
                    ...
                    case 1:
                        cbo_MACHTYPE_H.Text = argument[1].ToUpper() == "" ? "ALL" : argument[1].ToUpper(); //설비타입
                        break;

                    case 2:
                        txt_MACHCODE_H.Text = argument[2].ToUpper(); //설비코드
                        break;

                    case 3:
                        txt_MACHNAME_H.Text = argument[3].ToUpper(); //설비명
                        break;
                    case 4:
        ...
        private void InitGrid()
        {
            try
            {
                ...
                _GridUtil.InitColumnUltraGrid(grid1, "MACHTYPE", "설비분류", false, GridColDataType_emu.VarChar, 130, 100, Infragistics.Win.HAlign.Left, true, false);
                _GridUtil.InitColumnUltraGrid(grid1, "MACHCODE", "설비코드", false, GridColDataType_emu.VarChar, 100, 100, Infragistics.Win.HAlign.Center, true, false);
        ...
        private void Search()
        {
            ...
            sMachType = Convert.ToString(cbo_MACHTYPE_H.Value);
            sMachCode = txt_MACHCODE_H.Text.Trim();
            sMachName = txt_MACHNAME_H.Text.Trim();
            sUseFlag = Convert.ToString(cbo_USEFLAG_H.Value);

            _DtTemp = _biz.SEL_BM0070(sPlantCode, sMachType, sMachCode, sMachName, sUseFlag);
        ...
        private void grid1_DoubleClickRow(object sender, Infragistics.Win.UltraWinGrid.DoubleClickRowEventArgs e)
        {
            ...
            TmpDt.Rows.Add(new object[] { e.Row.Cells["MACHCODE"].Value, e.Row.Cells["MACHNAME"].Value });

            this.Tag = TmpDt;
            this.Close();
        }

        private void txt_MACHCODE_H_KeyPress(object sender, KeyPressEventArgs e)
        {
        ...
        private void txt_MACHNAME_H_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (e.KeyChar == (char)Keys.Enter)
            {
                Search();
