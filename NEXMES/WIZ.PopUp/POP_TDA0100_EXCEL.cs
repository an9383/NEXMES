...
namespace WIZ.PopUp
{
    public partial class POP_TDA0100_EXCEL : WIZ.Forms.BasePopupForm
    {
        #region [ 선언자 ]
        ...
        #region [ Form Load ]
        private void POP_TDA0100_EXCEL_Load(object sender, EventArgs e)
        {
            #region Grid 셋팅
            ...
            _GridUtil.InitColumnUltraGrid(grid1, "ID", "지그비ID", true, GridColDataType_emu.VarChar, 100, 4, Infragistics.Win.HAlign.Center, true, false, null, null, null, null, null);
            _GridUtil.InitColumnUltraGrid(grid1, "WORKCENTERCODE", "작업장ⓒ", true, GridColDataType_emu.VarChar, 120, 90, Infragistics.Win.HAlign.Center, true, false, null, null, null, null, null);
            _GridUtil.InitColumnUltraGrid(grid1, "GATEWAYID", "게이트웨이ID", true, GridColDataType_emu.VarChar, 120, 110, Infragistics.Win.HAlign.Center, true, false, null, null, null, null, null);
            _GridUtil.InitColumnUltraGrid(grid1, "VALUE", "COUNT 값", true, GridColDataType_emu.VarChar, 80, 80, Infragistics.Win.HAlign.Right, true, false, null, null, null, null, null);
        ...
        private void INSERT_SAVE()
        {
            ...
            try
            {
                for (int i = 0; i < grid1.Rows.Count; i++)
                {
                    ...
                    ID = Convert.ToString(grid1.Rows[i].Cells["ID"].Value);
                    WORKCENTERCODE = Convert.ToString(grid1.Rows[i].Cells["WORKCENTERCODE"].Value);
                    GATEWAYID = Convert.ToString(grid1.Rows[i].Cells["GATEWAYID"].Value);
                    VALUE = Convert.ToString(grid1.Rows[i].Cells["VALUE"].Value);
                    ...
                    if (ID == string.Empty)
                    {
                        MessageBox.Show(Convert.ToString(i + 1) + "번째 열의 지그비ID를 확인하세요.");
                        return;
                    }
                    if (WORKCENTERCODE == string.Empty)
                    {
                        MessageBox.Show(Convert.ToString(i + 1) + "번째 열의 작업장ⓒ을 확인하세요.");
                        return;
                    }
                }
                ...
                if (excelUploadType == "R")
                {
                    ...
                    for (int i = 0; i < grid1.Rows.Count; i++)
                    {
                        helper.ExecuteNoneQuery("USP_POP_BM4700_I1"
                                          , CommandType.StoredProcedure
                                               ...
                                               , helper.CreateParameter("AS_VALUE", Convert.ToString(grid1.Rows[i].Cells["VALUE"].Value), DbType.String, ParameterDirection.Input)
            ...
            try
            {
                ...
                if (openfiledialog.ShowDialog() == System.Windows.Forms.DialogResult.OK)
                {
                    ...
                    for (int i = 1; i < dtExcelImport.Rows.Count; i++)
                    {
                        ...
                        drTarget["GATEWAYID"] = Convert.ToString(dtExcelImport.Rows[i][3]);
                        drTarget["VALUE"] = Convert.ToString(dtExcelImport.Rows[i][4]);
