...
namespace WIZ.PopUp
{
    public partial class POP_TBM1100_EXCEL : WIZ.Forms.BasePopupForm
    {
        #region [ 선언자 ]
        ...
        #region [ Form Load ]
        private void POP_TBM1100_EXCEL_Load(object sender, EventArgs e)
        {
            #region Grid 셋팅
            ...
            _GridUtil.InitColumnUltraGrid(grid1, "STOPTYPE", "비가동구분ⓒ", true, GridColDataType_emu.VarChar, 120, 100, Infragistics.Win.HAlign.Left, true, false, null, null, null, null, null);
            ...
            _GridUtil.InitColumnUltraGrid(grid1, "STOPDESC", "비가동명", true, GridColDataType_emu.VarChar, 200, 200, Infragistics.Win.HAlign.Left, true, true, null, null, null, null, null);
            _GridUtil.InitColumnUltraGrid(grid1, "VIEWFLAG", "현장단말기표시유무ⓒ", true, GridColDataType_emu.VarChar, 240, 30, Infragistics.Win.HAlign.Left, true, true, null, null, null, null, null);
        ...
        private void INSERT_SAVE()
        {
            ...
            try
            {
                for (int i = 0; i < grid1.Rows.Count; i++)
                {
                    ...
                    STOPTYPE = Convert.ToString(grid1.Rows[i].Cells["STOPTYPE"].Value);
                    STOPCODE = Convert.ToString(grid1.Rows[i].Cells["STOPCODE"].Value);
                    STOPDESC = Convert.ToString(grid1.Rows[i].Cells["STOPDESC"].Value);
                    VIEWFLAG = Convert.ToString(grid1.Rows[i].Cells["VIEWFLAG"].Value);
                    ...
                    if (STOPTYPE == string.Empty)
                    {
                        MessageBox.Show(Convert.ToString(i + 1) + "번째 열의 비가동 구분을 확인하세요.");
                        return;
                    }
                    if (STOPCODE == string.Empty)
                    {
                        MessageBox.Show(Convert.ToString(i + 1) + "번째 열의 비가동 코드를 확인하세요.");
                        return;
                    }
                ...
                if (excelUploadType == "R")
                {
                    ...
                    for (int i = 0; i < grid1.Rows.Count; i++)
                    {
                        helper.ExecuteNoneQuery("USP_POP_BM1100_I1"
            ...
            try
            {
                ...
                if (openfiledialog.ShowDialog() == System.Windows.Forms.DialogResult.OK)
                {
                    ...
                    for (int i = 1; i < dtExcelImport.Rows.Count; i++)
                    {
                        ...
                        drTarget["STOPCODE"] = Convert.ToString(dtExcelImport.Rows[i][2]);
                        drTarget["STOPDESC"] = Convert.ToString(dtExcelImport.Rows[i][3]);
                        drTarget["VIEWFLAG"] = Convert.ToString(dtExcelImport.Rows[i][4]);
