...
namespace WIZ.PopUp
{
    public partial class BM0060_POP : WIZ.Forms.BasePopupForm
    {
        #region < MEMBER AREA >
        ...
        public BM0060_POP(string[] param)
        {
            InitializeComponent();
            ...
            for (int i = 0; i < param.Length; i++)
            {
                ...
                switch (i)
                {
                    ...
                    case 1:
                        txt_WORKCENTERCODE_H.Text = argument[1].ToUpper(); //작업장코드
                        break;

                    case 2:
                        txt_WORKCENTERNAME_H.Text = argument[2].ToUpper(); //작업장명
                        break;
                    case 3:
                        txt_LINECODE_H.Text = argument[3].ToUpper(); //라인코드
                        break;
                    case 4:
                        txt_OPCODE_H.Text = argument[4].ToUpper(); //공정코드
                        break;
                    case 5:
        ...
        private void InitGrid()
        {
            try
            {
                ...
                _GridUtil.InitColumnUltraGrid(grid1, "WORKCENTERCODE", "작업장코드", false, GridColDataType_emu.VarChar, 90, 100, Infragistics.Win.HAlign.Center, true, false);
                ...
                _GridUtil.InitColumnUltraGrid(grid1, "LINECODE", "라인", false, GridColDataType_emu.VarChar, 120, 100, Infragistics.Win.HAlign.Left, true, false);
                _GridUtil.InitColumnUltraGrid(grid1, "OPCODE", "공정", false, GridColDataType_emu.VarChar, 120, 100, Infragistics.Win.HAlign.Left, true, false);
        ...
        private void Search()
        {
            ...
            sWorkCenterCode = txt_WORKCENTERCODE_H.Text.Trim();
            sWorkCenterName = txt_WORKCENTERNAME_H.Text.Trim();
            sOpCode = txt_OPCODE_H.Text.Trim();
            sLineCode = txt_LINECODE_H.Text.Trim();
            sUseFlag = Convert.ToString(cbo_USEFLAG_H.Value);

            _DtTemp = _biz.SEL_BM0060(sPlantCode, sWorkCenterCode, sWorkCenterName, sOpCode, sLineCode, sUseFlag);
        ...
        private void txt_WORKCENTERCODE_H_KeyPress(object sender, KeyPressEventArgs e)
        {
        ...
        private void txt_WORKCENTERNAME_H_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (e.KeyChar == (char)Keys.Enter)
            {
                Search();
        ...
        private void txt_LINECODE_H_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (e.KeyChar == (char)Keys.Enter)
            {
                Search();
