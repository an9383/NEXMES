...
namespace WIZ.PopUp
{
    public partial class BM0610_EXCEL : WIZ.Forms.BasePopupForm
    {
        #region [ 선언자 ]
        ...
        #region [ Form Load ]
        private void BM0610_EXCEL_Load(object sender, EventArgs e)
        {
            ...
            _GridUtil.InitColumnUltraGrid(grid1, "DASPOINTID", "하드웨어 ID", false, GridColDataType_emu.VarChar, 100, 100, Infragistics.Win.HAlign.Center, true, false);
            _GridUtil.InitColumnUltraGrid(grid1, "DASPOINTNAME", "하드웨어 명", false, GridColDataType_emu.VarChar, 200, 100, Infragistics.Win.HAlign.Left, true, false);
            _GridUtil.InitColumnUltraGrid(grid1, "DASTYPE", "하드웨어 종류", false, GridColDataType_emu.VarChar, 120, 100, Infragistics.Win.HAlign.Left, true, false);
            _GridUtil.InitColumnUltraGrid(grid1, "IPADDRESS", "할당 I.P", false, GridColDataType_emu.VarChar, 160, 100, Infragistics.Win.HAlign.Center, true, false);
            _GridUtil.InitColumnUltraGrid(grid1, "LOCATION", "설치 장소", false, GridColDataType_emu.VarChar, 120, 100, Infragistics.Win.HAlign.Left, true, false);
            _GridUtil.InitColumnUltraGrid(grid1, "SERIALNO", "시리얼번호", false, GridColDataType_emu.VarChar, 120, 100, Infragistics.Win.HAlign.Center, true, false);
            ...
            grid1.DisplayLayout.Bands[0].Columns["DASPOINTID"].Header.Appearance.ForeColor = Color.SkyBlue;
            grid1.DisplayLayout.Bands[0].Columns["DASTYPE"].Header.Appearance.ForeColor = Color.SkyBlue;
            ...
            rtnDtTemp = _Common.GET_BM0000_CODE("DASTYPE");     //하드웨어 유형
            WIZ.UltraGridUtil.SetComboUltraGrid(this.grid1, "DASTYPE", rtnDtTemp, "CODE_ID", "CODE_NAME");
        ...
        private void INSERT_SAVE()
        {
            ...
            string sDaspointId = string.Empty;
            string sDaspointName = string.Empty;
            string sDasType = string.Empty;
            ...
            try
            {
                for (int i = 0; i < grid1.Rows.Count; i++)
                {
                    ...
                    sDaspointId = Convert.ToString(grid1.Rows[i].Cells["DASPOINTID"].Value);
                    sDaspointName = Convert.ToString(grid1.Rows[i].Cells["DASPOINTNAME"].Value);
                    sDasType = Convert.ToString(grid1.Rows[i].Cells["DASTYPE"].Value);
                    sIpAddress = Convert.ToString(grid1.Rows[i].Cells["IPADDRESS"].Value);
                    sLocaTion = Convert.ToString(grid1.Rows[i].Cells["LOCATION"].Value);
                    ...
                    if (sDaspointId == string.Empty)
                    {
                        MessageBox.Show(Common.getLangText(Convert.ToString(i + 1) + "번째 열의 하드웨어ID를 확인하세요.", "MSG"));
                        return;
                    }

                    if (sDasType == string.Empty)
                    {
                        MessageBox.Show(Common.getLangText(Convert.ToString(i + 1) + "번째 열의 하드웨어 유형을 확인하세요.", "MSG"));
                        return;
                    }
                }

                DataTable excelUpType = helper_Excel.FillTable("USP_EXCELUPLOADTYPE_S1", CommandType.StoredProcedure
                                                         , helper.CreateParameter("AS_FORMID", "BM0610", DbType.String, ParameterDirection.Input));
                ...
                if (excelUploadType == "R")
                {
                    ...
                    for (int i = 0; i < grid1.Rows.Count; i++)
                    {
                        helper.ExecuteNoneQuery("USP_BM0610_EXCEL_I1", CommandType.StoredProcedure
                                               ...
                                               , helper.CreateParameter("AS_DASPOINTID", Convert.ToString(grid1.Rows[i].Cells["DASPOINTID"].Value), DbType.String, ParameterDirection.Input)
                                               , helper.CreateParameter("AS_DASPOINTNAME", Convert.ToString(grid1.Rows[i].Cells["DASPOINTNAME"].Value), DbType.String, ParameterDirection.Input)
                                               , helper.CreateParameter("AS_DASTYPE", Convert.ToString(grid1.Rows[i].Cells["DASTYPE"].Value), DbType.String, ParameterDirection.Input)
                                               ...
                                               , helper.CreateParameter("AS_LOCATION", Convert.ToString(grid1.Rows[i].Cells["LOCATION"].Value), DbType.String, ParameterDirection.Input)
                                               , helper.CreateParameter("AS_SERIALNO", Convert.ToString(grid1.Rows[i].Cells["SERIALNO"].Value), DbType.String, ParameterDirection.Input)
            ...
            try
            {
                ...
                if (openfiledialog.ShowDialog() == System.Windows.Forms.DialogResult.OK)
                {
                    ...
                    for (int i = 1; i < dtExcelImport.Rows.Count; i++)
                    {
                        ...
                        drTarget["DASPOINTID"] = Convert.ToString(dtExcelImport.Rows[i][1]);
                        drTarget["DASPOINTNAME"] = Convert.ToString(dtExcelImport.Rows[i][2]);
                        drTarget["DASTYPE"] = Convert.ToString(dtExcelImport.Rows[i][3]);
                        drTarget["IPADDRESS"] = Convert.ToString(dtExcelImport.Rows[i][4]);
                        drTarget["LOCATION"] = Convert.ToString(dtExcelImport.Rows[i][5]);
