...
namespace WIZ.PopUp
{
    public partial class POP_TBM9300_EXCEL : WIZ.Forms.BasePopupForm
    {
        #region [ 선언자 ]
        ...
        #region [ Form Load ]
        private void POP_TBM9300_EXCEL_Load(object sender, EventArgs e)
        {
            #region Grid 셋팅
            ...
            _GridUtil.InitColumnUltraGrid(grid1, "STOPCODE", "비가동코드ⓒ", true, GridColDataType_emu.VarChar, 120, 100, Infragistics.Win.HAlign.Center, true, false, null, null, null, null, null);
            ...
            _GridUtil.InitColumnUltraGrid(grid1, "STMM", "시작(분)", true, GridColDataType_emu.VarChar, 80, 30, Infragistics.Win.HAlign.Center, true, true, null, null, null, null, null);
        ...
        private void INSERT_SAVE()
        {
            ...
            string STHH = string.Empty;
            string STMM = string.Empty;
            string ENHH = string.Empty;
            string ENMM = string.Empty;
            ...
            try
            {
                for (int i = 0; i < grid1.Rows.Count; i++)
                {
                    ...
                    STHH = Convert.ToString(grid1.Rows[i].Cells["STHH"].Value);
                    STMM = Convert.ToString(grid1.Rows[i].Cells["STMM"].Value);
                    ENHH = Convert.ToString(grid1.Rows[i].Cells["ENHH"].Value);
                    ENMM = Convert.ToString(grid1.Rows[i].Cells["ENMM"].Value);
                    ...
                    {
                        MessageBox.Show(Convert.ToString(i + 1) + "번째 열의 비가동코드를 확인하세요.");
                        return;
                    }
                ...
                if (excelUploadType == "R")
                {
                    ...
                    for (int i = 0; i < grid1.Rows.Count; i++)
                    {
                        helper.ExecuteNoneQuery("USP_POP_BM9300_I1"
            ...
            try
            {
                ...
                if (openfiledialog.ShowDialog() == System.Windows.Forms.DialogResult.OK)
                {
                    ...
                    for (int i = 1; i < dtExcelImport.Rows.Count; i++)
                    {
                        ...
                        drTarget["STHH"] = Convert.ToString(dtExcelImport.Rows[i][2]);
                        drTarget["STMM"] = Convert.ToString(dtExcelImport.Rows[i][3]);
                        drTarget["ENHH"] = Convert.ToString(dtExcelImport.Rows[i][4]);
                        drTarget["ENMM"] = Convert.ToString(dtExcelImport.Rows[i][5]);
