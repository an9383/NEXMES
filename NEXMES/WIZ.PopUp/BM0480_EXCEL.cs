...
namespace WIZ.PopUp
{
    public partial class BM0480_EXCEL : WIZ.Forms.BasePopupForm
    {
        #region [ 선언자 ]
        ...
        #region [ Form Load ]
        private void BM0480_EXCEL_Load(object sender, EventArgs e)
        {
            #region Grid 셋팅
            //GRID1 품목
            ...
            _GridUtil.InitColumnUltraGrid(grid1, "MACHKCYCLE", "점검주기", true, GridColDataType_emu.VarChar, 120, 100, Infragistics.Win.HAlign.Center, true, false);
            ...
            rtnDtTemp = _Common.GET_BM0000_CODE("MACHKCYCLE"); //검사값구분              
            ...
            //점검항목
            gridManager.PopUpAdd("MACHKCODE", "MACHKNAME", "BM0460", new string[] { "PLANTCODE", "Y" });
            #endregion
        }
        #endregion
        ...
        private void INSERT_SAVE()
        {
            ...
            string sMachkCode = string.Empty;
            string sWorkcentercode = string.Empty;
            string sMachkCycle = string.Empty;
            ...
            try
            {
                for (int i = 0; i < grid1.Rows.Count; i++)
                {
                    ...
                    sMachkCode = Convert.ToString(grid1.Rows[i].Cells["MACHKCODE"].Value);
                    sWorkcentercode = Convert.ToString(grid1.Rows[i].Cells["WORKCENTERCODE"].Value);
                    sMachkCycle = Convert.ToString(grid1.Rows[i].Cells["MACHKCYCLE"].Value);
                    ...
                    if (sMachkCode == string.Empty)
                    {
                        MessageBox.Show(Common.getLangText(Convert.ToString(i + 1) + "번째 열의 점검항목을 확인하세요.", "MSG"));
                        return;
                    }
                    if (sMachkCycle == string.Empty)
                    {
                        MessageBox.Show(Common.getLangText(Convert.ToString(i + 1) + "번째 열의 점검주기를 확인하세요.", "MSG"));
                        return;
                    }
                }

                DataTable excelUpType = helper_Excel.FillTable("USP_EXCELUPLOADTYPE_S1", CommandType.StoredProcedure
                                                         , helper.CreateParameter("AS_FORMID", "BM0480", DbType.String, ParameterDirection.Input));
                ...
                if (excelUploadType == "R")
                {
                    ...
                    for (int i = 0; i < grid1.Rows.Count; i++)
                    {
                        helper.ExecuteNoneQuery("USP_BM0480_I1"
                                                    ...
                                                    , helper.CreateParameter("AS_MACHKCODE", DBHelper.nvlString(grid1.Rows[i].Cells["MACHKCODE"].Value), DbType.String, ParameterDirection.Input)
                                                    , helper.CreateParameter("AS_MACHKCYCLE", DBHelper.nvlString(grid1.Rows[i].Cells["MACHKCYCLE"].Value), DbType.String, ParameterDirection.Input)
            ...
            try
            {
                ...
                if (openfiledialog.ShowDialog() == System.Windows.Forms.DialogResult.OK)
                {
                    ...
                    for (int i = 1; i < dtExcelImport.Rows.Count; i++)
                    {
                        ...
                        drTarget["WORKCENTERNAME"] = Convert.ToString(dtExcelImport.Rows[i][2]);
                        drTarget["MACHKCODE"] = Convert.ToString(dtExcelImport.Rows[i][3]);
                        drTarget["MACHKNAME"] = Convert.ToString(dtExcelImport.Rows[i][4]);
                        drTarget["MACHKCYCLE"] = Convert.ToString(dtExcelImport.Rows[i][5]);
