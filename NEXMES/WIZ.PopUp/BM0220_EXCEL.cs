...
namespace WIZ.PopUp
{
    public partial class BM0220_EXCEL : WIZ.Forms.BasePopupForm
    {
        #region [ 선언자 ]
        ...
        #region [ Form Load ]
        private void BM0220_EXCEL_Load(object sender, EventArgs e)
        {
            #region Grid 셋팅
            //GRID1 품목
            ...
            _GridUtil.InitColumnUltraGrid(grid1, "WORKCENTERCODE", "작업장코드", true, GridColDataType_emu.VarChar, 120, 100, Infragistics.Win.HAlign.Center, true, false);
            _GridUtil.InitColumnUltraGrid(grid1, "MACHCODE", "설비코드", true, GridColDataType_emu.VarChar, 120, 100, Infragistics.Win.HAlign.Center, true, false);
            _GridUtil.InitColumnUltraGrid(grid1, "DISPLAYNO", "표시순번", true, GridColDataType_emu.Integer, 80, 100, Infragistics.Win.HAlign.Right, true, false);
            _GridUtil.InitColumnUltraGrid(grid1, "WCPRODFLAG", "작업장실적반영여부", true, GridColDataType_emu.VarChar, 150, 100, Infragistics.Win.HAlign.Left, true, false);
            ...
            rtnDtTemp = _Common.GET_BM0000_CODE("WCPRODFLAG"); //작업장실적반영여부
            ...
            //설비
            gridManager.PopUpAdd("MACHCODE", "MACHNAME", "BM0070", new string[] { "PLANTCODE", "", "Y" });
            #endregion
        }
        #endregion
        ...
        private void INSERT_SAVE()
        {
            ...
            string sWcProdFlag = string.Empty;
            ...
            try
            {
                for (int i = 0; i < grid1.Rows.Count; i++)
                {
                    ...
                    sWcProdFlag = Convert.ToString(grid1.Rows[i].Cells["WCPRODFLAG"].Value);
                    ...
                    if (sWorkcenterCode == string.Empty)
                    {
                        MessageBox.Show(Common.getLangText(Convert.ToString(i + 1) + "번째 열의 작업장을 확인하세요.", "MSG"));
                        return;
                    }
                    if (sMachCode == string.Empty)
                    {
                        MessageBox.Show(Common.getLangText(Convert.ToString(i + 1) + "번째 열의 설비를 확인하세요.", "MSG"));
                        return;
                    }
                ...
                DataTable excelUpType = helper_Excel.FillTable("USP_EXCELUPLOADTYPE_S1", CommandType.StoredProcedure
                                                         , helper.CreateParameter("AS_FORMID", "BM0220", DbType.String, ParameterDirection.Input));
                ...
                if (excelUploadType == "R")
                {
                    ...
                    for (int i = 0; i < grid1.Rows.Count; i++)
                    {
                        helper.ExecuteNoneQuery("USP_BM0220_I1"
                                                    ...
                                                    , helper.CreateParameter("AI_DISPLAYNO", Convert.ToString(grid1.Rows[i].Cells["DISPLAYNO"].Value), DbType.Int32, ParameterDirection.Input)
                                                    , helper.CreateParameter("AS_WCPRODFLAG", Convert.ToString(grid1.Rows[i].Cells["WCPRODFLAG"].Value), DbType.String, ParameterDirection.Input)
            ...
            try
            {
                ...
                if (openfiledialog.ShowDialog() == System.Windows.Forms.DialogResult.OK)
                {
                    ...
                    for (int i = 1; i < dtExcelImport.Rows.Count; i++)
                    {
                        ...
                        drTarget["WORKCENTERCODE"] = Convert.ToString(dtExcelImport.Rows[i][1]);
                        drTarget["MACHCODE"] = Convert.ToString(dtExcelImport.Rows[i][2]);
                        drTarget["DISPLAYNO"] = Convert.ToString(dtExcelImport.Rows[i][3]);
                        drTarget["WCPRODFLAG"] = Convert.ToString(dtExcelImport.Rows[i][4]);
                        drTarget["USEFLAG"] = Convert.ToString(dtExcelImport.Rows[i][5]).Trim();
                        drTarget["REMARK"] = Convert.ToString(dtExcelImport.Rows[i][6]);
