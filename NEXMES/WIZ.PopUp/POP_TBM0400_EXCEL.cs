...
namespace WIZ.PopUp
{
    public partial class POP_TBM0400_EXCEL : WIZ.Forms.BasePopupForm
    {
        #region [ 선언자 ]
        ...
        #region [ Form Load ]
        private void POP_TBM0400_EXCEL_Load(object sender, EventArgs e)
        {
            #region Grid 셋팅
            ...
            _GridUtil.InitColumnUltraGrid(grid1, "OPNAME", "공정명", true, GridColDataType_emu.VarChar, 206, 40, Infragistics.Win.HAlign.Left, true, true, null, null, null, null, null);
            _GridUtil.InitColumnUltraGrid(grid1, "WBTFLAG", "단말기 사용여부", true, GridColDataType_emu.VarChar, 120, 100, Infragistics.Win.HAlign.Left, true, true, null, null, null, null, null);
            _GridUtil.InitColumnUltraGrid(grid1, "DISPLAYNO", "표시순번", true, GridColDataType_emu.VarChar, 100, 100, Infragistics.Win.HAlign.Right, true, true, null, null, null, null, null);
        ...
        private void INSERT_SAVE()
        {
            ...
            try
            {
                for (int i = 0; i < grid1.Rows.Count; i++)
                {
                    ...
                    OPNAME = Convert.ToString(grid1.Rows[i].Cells["OPNAME"].Value);
                    WBTFLAG = Convert.ToString(grid1.Rows[i].Cells["WBTFLAG"].Value);
                    DISPLAYNO = Convert.ToString(grid1.Rows[i].Cells["DISPLAYNO"].Value);
                    ...
                    {
                        MessageBox.Show(Convert.ToString(i + 1) + "번째 열의 공정코드를 확인하세요.");
                        return;
                    }
                ...
                if (excelUploadType == "R")
                {
                    ...
                    for (int i = 0; i < grid1.Rows.Count; i++)
                    {
                        helper.ExecuteNoneQuery("USP_POP_BM0400_I1"
                                          , CommandType.StoredProcedure
                                               ...
                                               , helper.CreateParameter("AS_DISPLAYNO", Convert.ToString(grid1.Rows[i].Cells["DISPLAYNO"].Value), DbType.String, ParameterDirection.Input)
            ...
            try
            {
                ...
                if (openfiledialog.ShowDialog() == System.Windows.Forms.DialogResult.OK)
                {
                    ...
                    for (int i = 1; i < dtExcelImport.Rows.Count; i++)
                    {
                        ...
                        drTarget["OPNAME"] = Convert.ToString(dtExcelImport.Rows[i][2]);
                        drTarget["WBTFLAG"] = Convert.ToString(dtExcelImport.Rows[i][3]);
                        drTarget["DISPLAYNO"] = Convert.ToString(dtExcelImport.Rows[i][4]);
