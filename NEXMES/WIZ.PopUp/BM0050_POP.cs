...
namespace WIZ.PopUp
{
    public partial class BM0050_POP : WIZ.Forms.BasePopupForm
    {
        #region < MEMBER AREA >
        ...
        public BM0050_POP(string[] param)
        {
            InitializeComponent();
            ...
            for (int i = 0; i < param.Length; i++)
            {
                ...
                switch (i)
                {
                    ...
                    case 1:
                        txt_LINECODE_H.Text = argument[1].ToUpper(); //라인코드
                        break;

                    case 2:
                        txt_LINENAME_H.Text = argument[2].ToUpper(); //라인명
                        break;
                    case 3:
                        txt_OPCODE_H.Text = argument[3].ToUpper(); //공정코드
                        break;
                    case 4:
        ...
        private void InitGrid()
        {
            try
            {
                ...
                _GridUtil.InitColumnUltraGrid(grid1, "LINECODE", "라인코드", false, GridColDataType_emu.VarChar, 100, 100, Infragistics.Win.HAlign.Center, true, false);
                _GridUtil.InitColumnUltraGrid(grid1, "LINENAME", "라인명", false, GridColDataType_emu.VarChar, 130, 100, Infragistics.Win.HAlign.Left, true, false);
        ...
        private void Search()
        {
            ...
            sLineName = txt_LINENAME_H.Text.Trim();
            sUseFlag = Convert.ToString(cbo_USEFLAG_H.Value);

            _DtTemp = _biz.SEL_BM0050(sPlantCode, sLineCode, sLineName, sOpCode, sUseFlag);
        ...
        private void txt_LINENAME_H_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (e.KeyChar == (char)Keys.Enter)
            {
                Search();
