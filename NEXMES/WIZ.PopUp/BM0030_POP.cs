...
namespace WIZ.PopUp
{
    public partial class BM0030_POP : WIZ.Forms.BasePopupForm
    {
        #region < MEMBER AREA >
        ...
        public BM0030_POP(string[] param)
        {
            InitializeComponent();
            ...
            for (int i = 0; i < param.Length; i++)
            {
                ...
                switch (i)
                {
                    ...
                    case 1:
                        cbo_CUSTTYPE_H.Value = argument[1].ToUpper() == "" ? "ALL" : argument[1].ToUpper(); //거래처구분
                        break;
                    ...
                    case 3:
                        txt_CUSTNAME_H.Text = argument[3].ToUpper(); //거래처명
                        break;
                    case 4:
                        cbo_USEFLAG_H.Text = argument[4].ToUpper() == "" ? "ALL" : argument[4].ToUpper(); //사용여부
                        break;
                    case 5:
                        cbo_CUSTTYPE_H.Enabled = argument[5].ToUpper() == "FALSE" ? cbo_CUSTTYPE_H.Enabled = false : cbo_CUSTTYPE_H.Enabled = true; // 거래처 구분 콤보박스 enable 유무
                        break;
                }
                #endregion
            }
        ...
        private void InitGrid()
        {
            try
            {
                ...
                _GridUtil.InitColumnUltraGrid(grid1, "CUSTTYPE", "거래처구분", false, GridColDataType_emu.VarChar, 130, 100, Infragistics.Win.HAlign.Left, true, false);
                ...
                rtnDtTemp = _Common.GET_BM0000_CODE("CUSTTYPE"); //품목유형
        ...
        private void Search()
        {
            ...
            sCustType = Convert.ToString(cbo_CUSTTYPE_H.Value);
            sCustCode = txt_CUSTCODE_H.Text.Trim();
            sCustName = txt_CUSTNAME_H.Text.Trim();
            sUseFlag = Convert.ToString(cbo_USEFLAG_H.Value);

            _DtTemp = _biz.SEL_BM0030(sPlantCode, sCustType, sCustCode, sCustName, sUseFlag);
