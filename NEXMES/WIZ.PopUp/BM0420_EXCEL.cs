...
namespace WIZ.PopUp
{
    public partial class BM0420_EXCEL : WIZ.Forms.BasePopupForm
    {
        #region [ 선언자 ]
        ...
        #region [ Form Load ]
        private void BM0420_EXCEL_Load(object sender, EventArgs e)
        {
            #region Grid 셋팅
            ...
            //검사항목
            gridManager.PopUpAdd("INSPCODE", "INSPNAME", "BM0180", new string[] { "PLANTCODE", "Y" });
            #endregion
        }
        #endregion
            ...
            try
            {
                for (int i = 0; i < grid1.Rows.Count; i++)
                {
                    ...
                    if (sValueType != "J")
                    {
                        ...
                        {
                            MessageBox.Show(Common.getLangText(Convert.ToString(i + 1) + "번째 열의 하한값은 상한값보다 클 수 없습니다.", "MSG")); return;
                        }
                    }
                    else
                    {
                ...
                DataTable excelUpType = helper_Excel.FillTable("USP_EXCELUPLOADTYPE_S1", CommandType.StoredProcedure
                                                         , helper.CreateParameter("AS_FORMID", "BM0420", DbType.String, ParameterDirection.Input));
