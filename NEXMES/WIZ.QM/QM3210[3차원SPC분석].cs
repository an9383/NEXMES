...
namespace WIZ.QM
{
    public partial class QM3210 : WIZ.Forms.BaseMDIChildForm
    {
        #region < MEMBER AREA >
        ...
        #region < FORM LOAD >
        private void QM3210_Load(object sender, EventArgs e)
        {
            GridInitialize();
            ...
            //품목
            btbManager.PopUpAdd(txtItemCode_H, txtItemName_H, "BM0010", new object[] { cboPlantCode_H, "" });
        ...
        public override void DoInquire()
        {
            ...
            try
            {
                ...
                string sItemCode = DBHelper.nvlString(txtItemCode_H.Value);

                rtnDtTemp = helper.FillTable("USP_QM3210_S1", CommandType.StoredProcedure
                ...
                {
                    tvInsp.Nodes.Clear();
                    QueryResultToTreeView(tvInsp, rtnDtTemp);
                }
                else
                {
        ...
        private void btnConfirm_Click(object sender, EventArgs e)
        {
            if (rdbAuto.Checked == true)
            {
                ...
                bStop = false;
                _timer.Interval = Convert.ToInt32(nupSearchTime.Value) * 1000 * 60;
        ...
        {
            if (lblCnt.Text.Substring(0, 1) == "1")
            {
            ...
            {
                lblCnt.Text = (Convert.ToInt32(lblCnt.Text.Substring(0, 1)) - 1).ToString() + lblCnt.Text.Remove(0, 1);
        ...
        {
            if (lblCnt.Text.Substring(0, 1) == lblCnt.Text.Substring(2, 1))
            {
        ...
        {
            if (Convert.ToDateTime(cboStartDate_H.Value) > Convert.ToDateTime(cboEndDate_H.Value) && Uc_Spc.Length > 0)
            {
                this.ShowDialog(Common.getLangText("시작일자를 종료일자보다 이전으로 선택해주십시오.", "MSG"), WIZ.Forms.DialogForm.DialogType.OK);
                return;
            }
            ...
            for (int i = 0; i < Uc_Spc.Length; i++)
            {
                Uc_Spc[i].StartDate = Convert.ToString(cboStartDate_H.Value).Substring(0, 10);
                Uc_Spc[i].EndDate = Convert.ToString(cboEndDate_H.Value).Substring(0, 10);
            }
        }
        ...
        private void tvInsp_NodeMouseDoubleClick(object sender, TreeNodeMouseClickEventArgs e)
        {
            if (e.Node.Level == 1)
            {
            ...
            if (e.Node.Level == 2)
            {
                ...
                for (int i = 1; i < sPathSplit.Length; i++)
                {
                    ...
                    sPathCode[ArrayIndex + 1] = sPathSplit2[0].Trim().Trim();
        ...
        private void Binding_UC_SPC()
        {
            ...
            lblCnt.Text = "1/" + ((Chk_Cnt / 2) + (Chk_Cnt % 2)).ToString();
            iDisplayNum = 0;
            ...
            for (int j = 0; j < DtChk.Rows.Count; j++)
            {
                ...
                uc.StartDate = Convert.ToString(cboStartDate_H.Value).Substring(0, 10);
                uc.EndDate = Convert.ToString(cboEndDate_H.Value).Substring(0, 10);
                uc.InspType = "A";
