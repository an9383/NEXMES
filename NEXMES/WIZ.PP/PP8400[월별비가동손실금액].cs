...
namespace WIZ.PP
{
    public partial class PP8400 : WIZ.Forms.BaseMDIChildForm
    {
        #region<MEMBER AREA>
        ...
        public override void DoInquire()
        {
            ...
            try
            {
                ...
                string SYyyy1 = Convert.ToDateTime(this.cboYear_H.Value).ToString("yyyy");              // 해당년도
                string SYyyy2 = Convert.ToDateTime(this.cboYear_H.Value).AddYears(-1).ToString("yyyy"); // 전년도

                rtnDtTemp = helper.FillTable("USP_PP8400_S1", CommandType.StoredProcedure
                    ...
                    if (grid1.Rows.Count != 0)
                    {
                        for (int i = 0; i < this.rtnDtTemp.Rows.Count; i++)
                        {
                            table.Rows.Add(new object[] {
                              ...
                              Convert.ToDouble(rtnDtTemp.Rows[i]["OldSA"]),
                              Convert.ToDouble(rtnDtTemp.Rows[i]["SA01"]),
                              Convert.ToDouble(rtnDtTemp.Rows[i]["SA02"]),
                              Convert.ToDouble(rtnDtTemp.Rows[i]["SA03"]),
                              Convert.ToDouble(rtnDtTemp.Rows[i]["SA04"]),
                              Convert.ToDouble(rtnDtTemp.Rows[i]["SA05"]),
                              Convert.ToDouble(rtnDtTemp.Rows[i]["SA06"]),
                              Convert.ToDouble(rtnDtTemp.Rows[i]["SA07"]),
                              Convert.ToDouble(rtnDtTemp.Rows[i]["SA08"]),
                              Convert.ToDouble(rtnDtTemp.Rows[i]["SA09"]),
                              Convert.ToDouble(rtnDtTemp.Rows[i]["SA10"]),
                              Convert.ToDouble(rtnDtTemp.Rows[i]["SA11"]),
                              Convert.ToDouble(rtnDtTemp.Rows[i]["SA12"])
                             });
                        }
                    }
        ...
        #region 폼 로더
        private void PP8400_Load(object sender, EventArgs e)
        {
            #region Grid 셋팅
            //그리드 객체 생성                                                                                                                                                                                    
