...
// *---------------------------------------------------------------------------------------------*
//   Form ID      : PP0060
//   Form Name    : 원인별 비가동 현황
//   Name Space   : WIZ.PP
//   Created Date : 2018-01-17
//   Made By      : WIZCORE
//   Edited Date  : 
//   Edit By      :
//   Description  : 원인별 비가동 현황 조회
// *---------------------------------------------------------------------------------------------*
#endregion
...
namespace WIZ.PP
{
    public partial class PP0060 : WIZ.Forms.BaseMDIChildForm
    {
        ...
        #region < FORM LOAD >
        private void PP0060_Load(object sender, EventArgs e)
        {
            try
            {
                #region GRID SETTING
                ...
                _GridUtil.InitColumnUltraGrid(grid1, "STOPCODE", "비가동코드", false, GridColDataType_emu.VarChar, 180, 100, Infragistics.Win.HAlign.Left, true, false);
                _GridUtil.InitColumnUltraGrid(grid1, "STOPCNT", "비가동건수", false, GridColDataType_emu.Double, 100, 100, Infragistics.Win.HAlign.Right, true, false);
                _GridUtil.InitColumnUltraGrid(grid1, "STATUSTIME", "비가동시간(분)", false, GridColDataType_emu.Double, 120, 100, Infragistics.Win.HAlign.Right, true, false);
                _GridUtil.InitColumnUltraGrid(grid1, "WORKAMT", "비가동금액(원)", false, GridColDataType_emu.Double, 120, 100, Infragistics.Win.HAlign.Right, false, false);

                grid1.Columns["STOPCNT"].Format = "#,##0";
                grid1.Columns["STATUSTIME"].Format = "#,##0.00";
                grid1.Columns["WORKAMT"].Format = "#,###,###,##0";

                //grid1.DisplayLayout.Bands[0].Override.SummaryFooterCaptionVisible = DefaultableBoolean.False;
                //grid1.DisplayLayout.Bands[0].Summaries.Add(Common.getLangText("합계", "TEXT"), SummaryPosition.UseSummaryPositionColumn, this.grid1.DisplayLayout.Bands[0].Columns["STOPTYPE"]);
                //grid1.DisplayLayout.Bands[0].Summaries[0].SourceColumn = this.grid1.DisplayLayout.Bands[0].Columns["STOPTYPE"];
                //grid1.DisplayLayout.Bands[0].Summaries[0].DisplayFormat = Common.getLangText("합계","TEXT");
                //grid1.DisplayLayout.Bands[0].Summaries[0].Key = "STOPTYPE";
                ...
                //GRID2 원인별 비가동 현황 (상세)
                _GridUtil.InitializeGrid(this.grid2, false, true, false, "", true);
                ...
                _GridUtil.InitColumnUltraGrid(grid2, "RECDATE", "일자", false, GridColDataType_emu.VarChar, 120, 100, Infragistics.Win.HAlign.Center, true, false);
                ...
                _GridUtil.InitColumnUltraGrid(grid2, "STOPCODE", "비가동코드", false, GridColDataType_emu.VarChar, 180, 100, Infragistics.Win.HAlign.Left, true, false);
                ...
                _GridUtil.InitColumnUltraGrid(grid2, "WORKAMT", "비가동금액(원)", false, GridColDataType_emu.Double, 120, 100, Infragistics.Win.HAlign.Right, false, false);

                grid2.Columns["STATUSTIME"].Format = "#,##0.00";
                grid2.Columns["WORKERCNT"].Format = "#,##0";
                grid2.Columns["WORKERGS"].Format = "#,##0.00";
                grid2.Columns["WORKAMT"].Format = "#,###,###,##0";
        ...
        public override void DoInquire()
        {
            ...
            try
            {
                ...
                rtnDtTemp = helper.FillTable("USP_PP0060_S1", CommandType.StoredProcedure
                ...
                if (tabControl1.SelectedTab.Index == 0)
                {
                    if (helper.RSCODE == "S")
                    {
                        ...
                        {
                            _GridUtil.Grid_Clear(grid1);
                            grid1.DataSource = UltraGridUtil.SetSubTotalUltraGrid(grid1, rtnDtTemp, "STOPTYPE", Common.getLangText("[ 구분별 합계 ]", "TEXT"), "STOPTYPE", "STOPCNT,STATUSTIME", "SUM,SUM");
                            grid1.DataBinds();
                    ...
                    {
                        this.ShowDialog(Common.getLangText(helper.RSMSG, "MSG"), Forms.DialogForm.DialogType.OK);
                    }
                }
                else if (tabControl1.SelectedTab.Index == 1)
                {
                    if (helper.RSCODE == "S")
                    {
                        if (rtnDtTemp.Rows.Count > 0)
                        {
                            ...
                            grid2.Columns["STOPTYPE"].MergedCellContentArea = MergedCellContentArea.VisibleRect;
                            grid2.Columns["STOPTYPE"].MergedCellEvaluationType = MergedCellEvaluationType.MergeSameValue;
                            grid2.Columns["STOPTYPE"].MergedCellStyle = MergedCellStyle.Always;
        ...
        private void grid1_DoubleClickCell_1(object sender, DoubleClickCellEventArgs e)
        {
            ...
            if (dt == null)
            {
                cbo_STOPTYPE_H.Value = this.grid1.ActiveRow.Cells["STOPTYPE"].Value.ToString();
