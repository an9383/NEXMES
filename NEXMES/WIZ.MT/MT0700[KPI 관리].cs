...
namespace WIZ.MT
{
    public partial class MT0700 : WIZ.Forms.BaseMDIChildForm
    {
        #region < MEMBER AREA >
        ...
        #region < FORM LOAD >
        private void MT0700_Load(object sender, EventArgs e)
        {
            GridInitialize();
        ...
        public override void DoSave()
        {
            ...
            string sMO_DAY_PROD_AVG = string.Empty;
            string sMO_DAY_WORK_AVG = string.Empty;
            string sMO_DAY_ERROR_COUNT = string.Empty;
            string sUV_DAY_PROD_AVG = string.Empty;
            string sUV_DAY_WORK_AVG = string.Empty;
            string sUV_DAY_ERROR_COUNT = string.Empty;
                ...
                foreach (DataRow drChange in dtChange.Rows)
                {
                    switch (drChange.RowState)
                    {
                        case DataRowState.Deleted:
                            ...
                            sMO_DAY_PROD_AVG = DBHelper.nvlString(drChange["MO_DAY_PROD_AVG"]);
                            sMO_DAY_WORK_AVG = DBHelper.nvlString(drChange["MO_DAY_WORK_AVG"]);
                            sMO_DAY_ERROR_COUNT = DBHelper.nvlString(drChange["MO_DAY_ERROR_COUNT"]);
                            sUV_DAY_PROD_AVG = DBHelper.nvlString(drChange["UV_DAY_PROD_AVG"]);
                            sUV_DAY_WORK_AVG = DBHelper.nvlString(drChange["UV_DAY_WORK_AVG"]);
                            sUV_DAY_ERROR_COUNT = DBHelper.nvlString(drChange["UV_DAY_ERROR_COUNT"]);
                            sRECDATE = string.Format("{0:yyyy-MM}", drChange["RECDATE"]);

                            helper.ExecuteNoneQuery("USP_MT0700_I1", CommandType.StoredProcedure
                          , helper.CreateParameter("AS_PLANTCODE", sPlantCode, DbType.String, ParameterDirection.Input)
                          ...
                          , helper.CreateParameter("AS_MO_DAY_PROD_AVG", sMO_DAY_PROD_AVG, DbType.String, ParameterDirection.Input)
                          , helper.CreateParameter("AS_MO_DAY_WORK_AVG", sMO_DAY_WORK_AVG, DbType.String, ParameterDirection.Input)
                          , helper.CreateParameter("AS_MO_DAY_ERROR_COUNT", sMO_DAY_ERROR_COUNT, DbType.String, ParameterDirection.Input)
                          , helper.CreateParameter("AS_UV_DAY_PROD_AVG", sUV_DAY_PROD_AVG, DbType.String, ParameterDirection.Input)
                          , helper.CreateParameter("AS_UV_DAY_WORK_AVG", sUV_DAY_WORK_AVG, DbType.String, ParameterDirection.Input)
                          , helper.CreateParameter("AS_UV_DAY_ERROR_COUNT", sUV_DAY_ERROR_COUNT, DbType.String, ParameterDirection.Input));
                            #endregion
                            break;
