...
namespace WIZ.MM
{
    public partial class PD2045 : WIZ.Forms.BaseMDIChildForm
    {
        #region < MEMBER AREA >
        ...
        #region < FORM LOAD >
        private void PD2045_Load(object sender, EventArgs e)
        {
        ...
        private void ScanLogic()
        {
            try
            {
                ...
                _dt = USP_PD2045_S1(gBarCode, ref RS_CODE, ref RS_MSG);
        ...
        private DataTable USP_PD2045_S1(string sBarCode, ref string RS_CODE, ref string RS_MSG)
        {
            DBHelper helper = new DBHelper(false);

            try
            {
                rtnDtTemp = helper.FillTable("USP_PD2045_S1"
        ...
        private void USP_PD2045_I1(string sBarCode, string sItemCode, string sCustCode, ref string RS_CODE, ref string RS_MSG)
        {
            DBHelper helper = new DBHelper(false);

            try
            {
                helper.ExecuteNoneQuery("USP_PD2045_I1"
                                       , CommandType.StoredProcedure
                                       , helper.CreateParameter("AS_PLANTCODE", LoginInfo.PlantCode, DbType.String, ParameterDirection.Input)
                                       , helper.CreateParameter("AS_BARCODE", sBarCode, DbType.String, ParameterDirection.Input)
                                       , helper.CreateParameter("AS_ITEMCODE", sItemCode, DbType.String, ParameterDirection.Input)
                                       , helper.CreateParameter("AS_ITEMCNT", txtQty.Text, DbType.Int32, ParameterDirection.Input)
        ...
        private bool AskRelease(string sLotNo)
        {
            ...
            {
                DataTable rtnDtTemp = new DataTable();
                rtnDtTemp = helper.FillTable("USP_PD2045_S2", CommandType.StoredProcedure
                                                            , helper.CreateParameter("AS_PLANTCODE", LoginInfo.PlantCode, DbType.String, ParameterDirection.Input)
                                                            , helper.CreateParameter("AS_LOTNO", sLotNo, DbType.String, ParameterDirection.Input));

                if (rtnDtTemp.Rows.Count == 0)
                {
                    DialogResult result = MessageBox.Show("[" + gBarCode + "] 자재 검사 이력이 없습니다. 그래도 자재출고 처리 하시겠습니까?", "자재출고 등록", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1);
                ...
                {
                    DialogResult result = MessageBox.Show("[" + gBarCode + "] 자재출고 처리 하시겠습니까?", "자재출고 등록", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1);
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                USP_PD2045_I1(gBarCode, sItemCode, sCustCode, ref RS_CODE, ref RS_MSG);

                if (RS_CODE == "S")
                {
                    ControlClear();
                    ShowDialog(Common.getLangText("자재출고 완료", "MSG"), DialogForm.DialogType.OK);
                }
                else if (RS_CODE == "E")
                {
