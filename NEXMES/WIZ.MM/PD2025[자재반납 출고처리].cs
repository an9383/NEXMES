...
namespace WIZ.MM
{
    public partial class PD2025 : WIZ.Forms.BaseMDIChildForm
    {
        #region < MEMBER AREA >
        ...
        #region < FORM LOAD >
        private void PD2025_Load(object sender, EventArgs e)
        {
            //타이틀 설정
        ...
        private void ScanLogic()
        {
            try
            {
                ...
                _dt = USP_PD2025_S1(gBarCode, ref RS_CODE, ref RS_MSG);
                if (RS_CODE == "S")
                {
                    if (_dt.Rows.Count > 0)
                    {
        ...
        //자재반납 출고대상 조회
        private DataTable USP_PD2025_S1(string sBarCode, ref string RS_CODE, ref string RS_MSG)
        {
            DBHelper helper = new DBHelper(false);

            try
            {
                rtnDtTemp = helper.FillTable("USP_PD2025_S1",
        ...
        //자재반납 출고처리
        private void USP_PD2025_I1(string sBarCode, ref string RS_CODE, ref string RS_MSG)
        {
            DBHelper helper = new DBHelper(false);

            try
            {
                helper.ExecuteNoneQuery("USP_PD2025_I1",
                                             CommandType.StoredProcedure,
                                             helper.CreateParameter("AS_PLANTCODE", LoginInfo.PlantCode, DbType.String, ParameterDirection.Input),
                                             helper.CreateParameter("AS_BARCODE", sBarCode, DbType.String, ParameterDirection.Input),
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            DialogResult result = MessageBox.Show("[" + gBarCode + "]" + Common.getLangText("자재반납 하시겠습니까?", "MSG"), "", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1);
            ...
            try
            {
                USP_PD2025_I1(gBarCode, ref RS_CODE, ref RS_MSG);

                if (RS_CODE == "S")
                {
                    ControlClear();
                    this.ShowDialog(Common.getLangText("[자재반납] 자재반납 완료", "MSG"), DialogForm.DialogType.OK);
                }
                else if (RS_CODE == "E")
                {
                    this.ShowDialog(Common.getLangText("[자재반납] ", "MSG") + RS_MSG, DialogForm.DialogType.OK);
                }

            }
            catch (Exception ex)
            {
                this.ShowDialog(Common.getLangText("[자재반납] ", "MSG") + ex.Message, DialogForm.DialogType.OK);
            }
        }
        #endregion
