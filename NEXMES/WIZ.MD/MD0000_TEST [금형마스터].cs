...
namespace WIZ.MD
{
    public partial class MD0000_TEST : WIZ.Forms.BaseMDIChildForm
    {
        #region < MEMBER AREA >
        ...
        public void SetMoldInfo(DataTable DTinfo)
        {
            if (DTinfo.Rows.Count > 0)
            {
                ...
                txtLINKCODE1.Text = CModule.ToString(DTinfo.Rows[0]["CODELINK1"]);
                txtLINKCODE2.Text = CModule.ToString(DTinfo.Rows[0]["CODELINK2"]);
                txtLINKCODE3.Text = CModule.ToString(DTinfo.Rows[0]["CODELINK3"]);
                txtLINKCODE4.Text = CModule.ToString(DTinfo.Rows[0]["CODELINK4"]);
                txtLINKCODE5.Text = CModule.ToString(DTinfo.Rows[0]["CODELINK5"]);
        ...
        private void BtnProd_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    txtMOLDPRODLIST.Text = mbp.CheckItems.Remove(mbp.CheckItems.Length-1);
                }
            }
            catch
            {
